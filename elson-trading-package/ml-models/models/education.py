...
content_prerequisites = Table(
    ...
    Base.metadata,
    Column(
        "content_id", Integer, ForeignKey("educational_content.id"), primary_key=True
    ),
    Column(
        "prerequisite_id",
        Integer,
...
class EducationalContent(Base):
    ...
    completion_requirement = Column(
        Enum(CompletionRequirement), nullable=False, default=CompletionRequirement.NONE
    )
    ...
    associated_quiz_id = Column(
        Integer, ForeignKey("educational_content.id"), nullable=True
    )
    associated_quiz = relationship(
        "EducationalContent",
        remote_side=[id],
...
class UserProgress(Base):
    ...
    class Meta:
        ...
        __table_args__ = ({"unique_constraint": ("user_id", "content_id")},)

    def __repr__(self):
        status = (
            "completed"
            if self.is_completed
            else f"in-progress ({self.progress_percent}%)"
        )
...
class LearningPathItem(Base):
    ...
    )  # Whether this item is required for completion
    ...
    class Meta:
        ...
        __table_args__ = ({"unique_constraint": ("learning_path_id", "content_id")},)
...
class UserPermission(Base):
    ...
    permission_id = Column(
        Integer, ForeignKey("trading_permissions.id"), nullable=False
    )
    ...
    )  # Guardian who granted
    override_reason = Column(
        Text, nullable=True
    )  # If granted without meeting requirements

    # Relationships
    user = relationship(
        "User", foreign_keys=[user_id], back_populates="trading_permissions"
    )
    ...
    class Meta:
        ...
        __table_args__ = ({"unique_constraint": ("user_id", "permission_id")},)
