...
from ...models.trade import InvestmentType, Trade, TradeSource, TradeStatus
...
async def get_user_micro_invest_settings(
    ...
    if not user_settings:
        ...
        user_settings = service.create_user_settings(settings_data)
...
async def get_roundup_transactions(
    ...
    # Apply sorting
    sort_column = getattr(
        Trade.RoundupTransaction, sort_by, Trade.RoundupTransaction.transaction_date
    )
    if sort_desc:
    ...
    return {"transactions": query.all(), "total": total}
...
async def create_micro_investment(
    ...
    current_user: User = Depends(
        get_current_user_with_permissions(["fractional_shares"])
    ),
    service: MicroInvestService = Depends(get_micro_invest_service),
):
...
async def complete_micro_invest_education(
    ...
    user_settings.completed_micro_invest_education = True
    db.commit()
