...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('quantum_evaluation.log')
    ]
)
...
async def run_evaluation(
    ...
    feature_columns = [
        'Close', 'Volume', 'SMA_10', 'SMA_30', 'EMA_10', 
        'ATR', 'Volatility', 'RSI', 'Price_to_SMA10', 'Price_to_SMA30'
    ]
    ...
    if volatility_regime_test:
        ...
        logger.info(f"Testing across {len(volatility_regimes)} volatility regimes: {', '.join(volatility_regimes.keys())}")
    ...
    for symbol in symbols:
        ...
        try:
            ...
            df['Volatility_20d'] = df['Return'].rolling(window=20).std() * np.sqrt(252) * 100  # Annualized
            ...
            if volatility_regime_test:
                ...
                for regime_name, thresholds in volatility_regimes.items():
                    ...
                    regime_df = df[(df['Volatility_20d'] >= thresholds['threshold_min']) & 
                    ...
                    if len(regime_df) >= 60:  # Minimum 60 days of data required
                        ...
                        regime_df['date_diff'] = regime_df.index.to_series().diff().dt.days
                        period_start = None
                        periods = []
                        
                        for idx, row in regime_df.iterrows():
                            if period_start is None or row['date_diff'] > 1:
                                if period_start is not None and (idx - period_start).days >= 30:
                                    periods.append((period_start, idx))
                                period_start = idx
                        
                        # Add the last period if it's long enough
                        if period_start is not None and (regime_df.index[-1] - period_start).days >= 30:
                            periods.append((period_start, regime_df.index[-1]))
                            
                        regime_periods[regime_name] = periods
                        
                        logger.info(f"Found {len(regime_df)} days in {regime_name} volatility regime "
                                   f"with {len(periods)} continuous periods of 30+ days")
                
                logger.info(f"Testing {symbol} across {len(regime_dfs)} volatility regimes "
        ...
        for regime_name, regime_df in regime_dfs.items():
            ...
            if not periods:
                logger.warning(f"No valid test periods for {symbol} in {regime_name} regime")
                continue
                
            logger.info(f"Testing {symbol} in {regime_name} volatility regime with {len(regime_df)} data points")
            ...
            for model_config in models_to_test:
                try:
                    ...
                    if model_config["name"] == "QuantumHybrid" and regime_name in ["HIGH", "EXTREME"]:
                    ...
                    logger.info(f"Testing {model_config['name']} on {symbol} in {regime_name} regime")
                    
                    # For each continuous period in this regime, run a separate backtest
                    period_results = []
                    for period_start, period_end in periods:
                        logger.debug(f"Testing period {period_start.date()} to {period_end.date()}")
                    ...
                    if period_results:
                        aggregated_result = {
                            "test_metrics": {
                                "accuracy": np.mean([r["test_metrics"]["accuracy"] for r in period_results]),
                                "precision": np.mean([r["test_metrics"]["precision"] for r in period_results]),
                                "recall": np.mean([r["test_metrics"]["recall"] for r in period_results]),
                                "f1": np.mean([r["test_metrics"]["f1"] for r in period_results]),
                            },
                            "financial_impact": {
                                "total_return": np.mean([r["financial_impact"]["total_return"] for r in period_results]),
                                "sharpe_ratio": np.mean([r["financial_impact"]["sharpe_ratio"] for r in period_results]),
                                "max_drawdown": np.mean([r["financial_impact"]["max_drawdown"] for r in period_results]),
                                "win_rate": np.mean([r["financial_impact"]["win_rate"] for r in period_results]),
                            },
                            ...
                            "individual_periods": [{
                                ...
                                "data_points": len(regime_df[(regime_df.index >= p_start) & (regime_df.index <= p_end)]),
                                "win_rate": r["financial_impact"]["win_rate"],
                                "total_return": r["financial_impact"]["total_return"],
                        ...
                        if all("benchmark_comparison" in r for r in period_results):
                            ...
                            for benchmark in benchmark_names:
                                ...
                                if all(benchmark in r["benchmark_comparison"] for r in period_results):
                                    benchmark_comparison[benchmark] = {
                                        "accuracy": np.mean([r["benchmark_comparison"][benchmark]["accuracy"] 
                                                           for r in period_results if benchmark in r["benchmark_comparison"]]),
                                        "f1": np.mean([r["benchmark_comparison"][benchmark]["f1"] 
                                                     for r in period_results if benchmark in r["benchmark_comparison"]]),
                                    }
                            
                            aggregated_result["benchmark_comparison"] = benchmark_comparison
                        
                        # Store results for this model in this regime
                        symbol_results[regime_name][model_config["name"]] = aggregated_result
                        
                        logger.info(f"Completed testing {model_config['name']} on {symbol} in {regime_name} regime. "
                        ...
                        logger.warning(f"No periods were successfully evaluated for {symbol} with {model_config['name']} "
                                    f"in {regime_name} regime")
                        symbol_results[regime_name][model_config["name"]] = {"error": "No valid periods"}
                    
                except Exception as e:
                    logger.error(f"Error testing {model_config['name']} on {symbol} in {regime_name} regime: {str(e)}")
                    symbol_results[regime_name][model_config["name"]] = {"error": str(e)}
...
async def create_summary_report(
    ...
    for model_name in model_names:
        ...
        for symbol, symbol_results in results.items():
            for regime, regime_results in symbol_results.items():
                if model_name in regime_results and "error" not in regime_results[model_name]:
                    ...
                    if "financial_impact" in regime_results[model_name]:
                        ...
                        regime_metrics[regime]["total_return"].append(impact["total_return"])
                        regime_metrics[regime]["sharpe_ratio"].append(impact["sharpe_ratio"])
                        regime_metrics[regime]["max_drawdown"].append(impact["max_drawdown"])
                    ...
                    if "sample_count" in regime_results[model_name]:
                        regime_metrics[regime]["sample_count"].append(regime_results[model_name]["sample_count"])
                    
                    if "period_count" in regime_results[model_name]:
                        regime_metrics[regime]["period_count"].append(regime_results[model_name]["period_count"])
        
        # Calculate overall model averages
        summary["models"][model_name] = {
            "avg_accuracy": np.mean(model_metrics["accuracy"]) if model_metrics["accuracy"] else None,
            "avg_f1": np.mean(model_metrics["f1"]) if model_metrics["f1"] else None,
            "avg_total_return": np.mean(model_metrics["total_return"]) if model_metrics["total_return"] else None,
            "avg_sharpe_ratio": np.mean(model_metrics["sharpe_ratio"]) if model_metrics["sharpe_ratio"] else None,
            "avg_max_drawdown": np.mean(model_metrics["max_drawdown"]) if model_metrics["max_drawdown"] else None,
            "avg_win_rate": np.mean(model_metrics["win_rate"]) if model_metrics["win_rate"] else None,
        ...
        for regime in regimes:
            ...
            if r_metrics["accuracy"]:
                ...
                summary["volatility_regimes"][regime]["sample_count"] += int(np.sum(r_metrics["sample_count"]))
                summary["volatility_regimes"][regime]["period_count"] += int(np.sum(r_metrics["period_count"]))
    ...
    for model_name in model_names:
        if "Quantum" in model_name:  # Only compare quantum models
            ...
            # Track by regime
            regime_stats = {regime: {"wins": 0, "losses": 0, "ties": 0, "total": 0} for regime in regimes}
            
            for symbol, symbol_results in results.items():
                for regime, regime_results in symbol_results.items():
                    if (model_name in regime_results and 
                        "benchmark_comparison" in regime_results[model_name]):
                        ...
                        for classical_model, metrics in benchmark.items():
                            if "Random" in classical_model or "Logistic" in classical_model:
                                classical_acc = metrics.get("accuracy", 0)
                                total += 1
                                regime_stats[regime]["total"] += 1
                                
                                if quantum_acc > classical_acc + 0.02:  # 2% better is a win
                                    wins += 1
                                    regime_stats[regime]["wins"] += 1
                                elif classical_acc > quantum_acc + 0.02:  # 2% worse is a loss
                                    losses += 1
    ...
    for model_name in model_names:
        ...
        win_rates = []
        for regime in regimes:
            if regime in summary["volatility_regimes"] and model_name in summary["volatility_regimes"][regime]["models"]:
                win_rate = summary["volatility_regimes"][regime]["models"][model_name]["avg_win_rate"]
        ...
        if len(win_rates) >= 2:
            ...
            volatility_robustness[model_name] = {
                ...
                "performance_differential": robustness * 100,  # Convert to percentage points
                "is_robust": robustness <= 0.1  # Less than 10 percentage points difference is considered robust
            }
    ...
    for model_name, metrics in summary["models"].items():
        if metrics["avg_total_return"] is not None and metrics["avg_total_return"] > best_return:
    ...
    for regime in regimes:
        if regime in summary["volatility_regimes"]:
            ...
            for model_name, metrics in summary["volatility_regimes"][regime]["models"].items():
            ...
            if regime_best_model:
                best_by_regime[regime] = {
                    ...
                    "win_rate": summary["volatility_regimes"][regime]["models"][regime_best_model]["avg_win_rate"]
                }
...
def create_comparison_charts(summary: Dict[str, Any], output_dir: str):
    ...
    for model_name, metrics in summary["models"].items():
        if metrics["avg_total_return"] is not None:
            ...
            accuracies.append(metrics["avg_accuracy"] if metrics["avg_accuracy"] is not None else 0)
    ...
    if "classical_comparison" in summary:
        ...
        win_rates = [summary["classical_comparison"][m]["win_rate"] for m in quantum_models]
    ...
    if "volatility_regimes" in summary and len(summary["volatility_regimes"]) > 1:
        ...
        if quantum_models and regimes:
            ...
            model_colors = {model: colors[i % len(colors)] for i, model in enumerate(quantum_models)}
                ...
                if regime in summary["volatility_regimes"]:
                    ...
                    for model in quantum_models:
                        if model in regime_models:
                            model_win_rates[model].append(regime_models[model]["avg_win_rate"])
                            model_returns[model].append(regime_models[model]["avg_return"])
                        else:
            ...
            for model in quantum_models:
                plt.plot(regimes, model_win_rates[model], marker='o', label=model, color=model_colors[model])
            ...
            # Plot lines for each model
            for model in quantum_models:
                plt.plot(regimes, model_returns[model], marker='o', label=model, color=model_colors[model])
            ...
            if "volatility_robustness" in summary:
                ...
                if robust_models:
                    ...
                    plt.title('Volatility Robustness - Win Rate Consistency Across Regimes')
                    plt.ylabel('Performance Differential (pp)')
                    plt.axhline(y=10, color='green', linestyle='--', label='Target (10pp)')
                    plt.xticks(rotation=45)
                    plt.grid(axis='y', alpha=0.3)
                    plt.legend()
                    
                    plt.tight_layout()
                    plt.savefig(os.path.join(output_dir, "volatility_robustness.png"), dpi=300)

async def main():
    """Main entry point for the evaluation script"""
    parser = argparse.ArgumentParser(description='Evaluate quantum models on market data across volatility regimes')
    
    parser.add_argument('--symbols', type=str, nargs='+', default=['AAPL', 'MSFT', 'GOOGL', 'SPY', 'QQQ'],
    ...
    parser.add_argument('--no_volatility_test', action='store_true',
                        help='Skip testing across volatility regimes')
    parser.add_argument('--extended_symbols', action='store_true',
    ...
    # Run evaluation
    try:
        logger.info(f"Starting evaluation of {len(args.symbols)} symbols from {args.start_date} to {args.end_date}")
        logger.info(f"Volatility regime testing: {'ENABLED' if not args.no_volatility_test else 'DISABLED'}")
