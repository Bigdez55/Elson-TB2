...
from Elson.trading_engine.backtesting.hybrid_model_evaluation import HybridModelBacktester
from Elson.trading_engine.ai_model_engine.hybrid_models import HybridMachineLearningModel
from Elson.trading_engine.volatility_regime.regime_specific_models import RegimeSpecificModelSelector
from Elson.trading_engine.feature_engineering.volatility_features import VolatilityFeatureEngineer
...
async def main():
    ...
    print("Evaluating hybrid model performance across volatility regimes. This may take some time...")
    result = backtester.run_backtest(test_name="hybrid_model_backtest", generate_plots=True)
    ...
    results = {
        'summary': {
            ...
            'min_win_rate': min(
                result.low_volatility.win_rate if result.low_volatility.sample_count > 0 else 1.0,
                result.normal_volatility.win_rate if result.normal_volatility.sample_count > 0 else 1.0,
                result.high_volatility.win_rate if result.high_volatility.sample_count > 0 else 1.0,
                result.extreme_volatility.win_rate if result.extreme_volatility.sample_count > 0 else 1.0
            ),
            'max_win_rate': max(
                result.low_volatility.win_rate if result.low_volatility.sample_count > 0 else 0.0,
                result.normal_volatility.win_rate if result.normal_volatility.sample_count > 0 else 0.0,
                result.high_volatility.win_rate if result.high_volatility.sample_count > 0 else 0.0,
                result.extreme_volatility.win_rate if result.extreme_volatility.sample_count > 0 else 0.0
            )
        },
        'markets': {
            'SYNTHETIC': {
    ...
    if os.path.exists(backtest_file):
        with open(backtest_file, 'r') as f:
            ...
            success_criteria_met = backtest_data.get('success_criteria', {}).get('met', False)
            target_achieved = success_criteria_met
    else:
        target_achieved = False
        
    target_status = "✅ SUCCESS CRITERIA MET" if target_achieved else "❌ SUCCESS CRITERIA NOT MET"
    
    print(f"\nTarget Status: {target_status}")
    print(f"Average Win Rate: {summary.get('average_win_rate', 0):.2%}")
    print(f"Markets Meeting 70% Target: {summary.get('markets_meeting_target', 0)} / {summary.get('total_markets_tested', 0)}")
    print(f"Average Return: {summary.get('average_return', 0):.2%}")
    print(f"Average Sharpe Ratio: {summary.get('average_sharpe', 0):.2f}")
    print(f"Win Rate Range: {summary.get('min_win_rate', 0):.2%} - {summary.get('max_win_rate', 0):.2%}")
    
    print("\nDetailed Results by Market:")
    print("-" * 80)
    print(f"{'Market':<10} {'Win Rate':<12} {'Return':<12} {'Sharpe':<12} {'Target Met':<10}")
    print("-" * 80)
    
    for symbol, result in results['markets'].items():
        if 'error' in result:
            print(f"{symbol:<10} {'ERROR':<12} {'ERROR':<12} {'ERROR':<12} {'ERROR':<10}")
            continue
        ...
        print(f"{symbol:<10} {win_rate_str:<12} {total_return_str:<12} {sharpe_str:<12} {target_met:<10}")
    ...
    if 'backtest_data' in locals() and backtest_data:
        ...
        print(f"Low Volatility:     Win Rate: {low_vol.get('win_rate', 0):.2%}, Samples: {low_vol.get('sample_count', 0)}")
        print(f"Normal Volatility:  Win Rate: {normal_vol.get('win_rate', 0):.2%}, Samples: {normal_vol.get('sample_count', 0)}")
        print(f"High Volatility:    Win Rate: {high_vol.get('win_rate', 0):.2%}, Samples: {high_vol.get('sample_count', 0)}")
        print(f"Extreme Volatility: Win Rate: {extreme_vol.get('win_rate', 0):.2%}, Samples: {extreme_vol.get('sample_count', 0)}")
        
        # Get volatility robustness
        volatility_robustness = performance.get('overall', {}).get('volatility_robustness', 0)
    ...
    print(f"1. High Volatility Win Rate ≥ 65%: {'✅' if high_vol_target else '❌'} ({high_vol_win_rate:.2%})")
    print(f"2. Extreme Volatility Win Rate ≥ 60%: {'✅' if extreme_vol_target else '❌'} ({extreme_vol_win_rate:.2%})")
    print(f"3. Volatility Robustness ≤ 10pp: {'✅' if robustness_target else '❌'} ({volatility_robustness:.2f}pp)")
    
    print("\nRecommendations:")
    if target_achieved:
        print("✅ The hybrid model has successfully met all success criteria for Phase 2!")
        print("✅ The implementation has demonstrated volatility robustness across regimes.")
        ...
        if not extreme_vol_target:
            ...
            print("   2. Consider more conservative position sizing in extreme conditions")
            print("   3. Adjust model weights to favor more robust models in extreme regimes")
        ...
        if not robustness_target:
            ...
            print("   3. Adjust parameter adaptation speed to ensure more consistent performance")
