...
class AuthDatabaseTester:
    ...
    def test_database_connectivity(self):
        ...
        try:
            ...
            self.log_test("database_tests", "basic_connectivity", "PASSED", 
            ...
            if os.path.exists(db_path):
                ...
                self.log_test("database_tests", "file_access", "PASSED", 
                             f"Database file exists, size: {file_size} bytes")
            else:
                self.log_test("database_tests", "file_access", "FAILED", 
    ...
    def test_database_schema(self):
        ...
        try:
            with engine.connect() as connection:
                ...
                if tables:
                    self.log_test("database_tests", "users_table_exists", "PASSED", 
                                 "Users table found")
                    ...
                    if missing_columns:
                        self.log_test("database_tests", "schema_validation", "FAILED", 
                                     f"Missing columns: {missing_columns}")
                    else:
                        self.log_test("database_tests", "schema_validation", "PASSED", 
                                     f"All required columns present: {list(columns.keys())}")
                else:
                    self.log_test("database_tests", "users_table_exists", "FAILED", 
    ...
    def test_password_hashing(self):
        ...
        try:
            ...
            if hashed and len(hashed) > 50:  # bcrypt hashes are typically ~60 chars
                self.log_test("auth_tests", "password_hashing", "PASSED", 
                             f"Password hashed successfully, length: {len(hashed)}")
            else:
                self.log_test("auth_tests", "password_hashing", "FAILED", 
                             "Password hash appears invalid")
            
            # Test verification
            if verify_password(test_password, hashed):
                self.log_test("auth_tests", "password_verification", "PASSED", 
                             "Password verification successful")
            else:
                self.log_test("auth_tests", "password_verification", "FAILED", 
                             "Password verification failed")
                
            # Test wrong password
            if not verify_password("WrongPassword", hashed):
                self.log_test("auth_tests", "password_rejection", "PASSED", 
                             "Wrong password correctly rejected")
            else:
                self.log_test("auth_tests", "password_rejection", "FAILED", 
    ...
    def test_jwt_tokens(self):
        ...
        try:
            ...
            if token and len(token) > 50:
                self.log_test("auth_tests", "jwt_creation", "PASSED", 
                             f"JWT token created, length: {len(token)}")
            else:
                self.log_test("auth_tests", "jwt_creation", "FAILED", 
            ...
            if payload and payload.get("sub") == test_email:
                self.log_test("auth_tests", "jwt_verification", "PASSED", 
                             f"JWT token verified, payload: {payload}")
            else:
                self.log_test("auth_tests", "jwt_verification", "FAILED", 
            ...
            if not expired_payload:
                self.log_test("auth_tests", "jwt_expiration", "PASSED", 
                             "Expired token correctly rejected")
            else:
                self.log_test("auth_tests", "jwt_expiration", "FAILED", 
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "access_token" in data and "user" in data:
                    self.log_test("auth_tests", "user_registration", "PASSED", 
                                 f"User registered successfully, user_id: {data['user']['id']}")
                    
                    # Test duplicate registration
                    duplicate_response = self.client.post("/api/v1/auth/register", json=test_user_data)
                    if duplicate_response.status_code == 400:
                        self.log_test("auth_tests", "duplicate_registration_prevention", "PASSED", 
                                     "Duplicate registration correctly prevented")
                    else:
                        self.log_test("auth_tests", "duplicate_registration_prevention", "FAILED", 
                                     f"Duplicate registration not prevented: {duplicate_response.status_code}")
                else:
                    self.log_test("auth_tests", "user_registration", "FAILED", 
    ...
    def test_user_login(self):
        ...
        try:
            ...
            reg_response = self.client.post("/api/v1/auth/register", json=test_user_data)
            if reg_response.status_code != 200:
                self.log_test("auth_tests", "login_test_setup", "FAILED", 
            ...
            if login_response.status_code == 200:
                ...
                if "access_token" in data and "user" in data:
                    self.log_test("auth_tests", "user_login", "PASSED", 
                    ...
                    if me_response.status_code == 200:
                        self.log_test("auth_tests", "token_usage", "PASSED", 
                                     "Token successfully used for authenticated request")
                    else:
                        self.log_test("auth_tests", "token_usage", "FAILED", 
                                     f"Token usage failed: {me_response.status_code}")
                else:
                    self.log_test("auth_tests", "user_login", "FAILED", 
            ...
            wrong_response = self.client.post("/api/v1/auth/login", json=wrong_login_data)
            if wrong_response.status_code == 401:
                self.log_test("auth_tests", "failed_login_handling", "PASSED", 
                             "Wrong password correctly rejected")
            else:
                self.log_test("auth_tests", "failed_login_handling", "FAILED", 
    ...
    def test_redis_connectivity(self):
        ...
        try:
            if redis_client:
                ...
                redis_client.ping()
                self.log_test("database_tests", "redis_connectivity", "PASSED", 
                ...
                if value and value.decode() == "test_value":
                    self.log_test("database_tests", "redis_operations", "PASSED", 
                                 "Redis read/write operations successful")
                else:
                    self.log_test("database_tests", "redis_operations", "FAILED", 
                                 "Redis read/write operations failed")
                
                # Cleanup
                redis_client.delete(test_key)
            else:
                self.log_test("database_tests", "redis_connectivity", "WARNING", 
    ...
    def test_rate_limiting(self):
        ...
        try:
            ...
            if reg_response.status_code != 200:
                self.log_test("security_tests", "rate_limiting_setup", "FAILED", 
            ...
            for i in range(10):  # Try 10 failed logins
                ...
                if response.status_code == 429:  # Too Many Requests
                    self.log_test("security_tests", "rate_limiting", "PASSED", 
            ...
            if failed_attempts >= 10:
                self.log_test("security_tests", "rate_limiting", "WARNING", 
    ...
    def test_database_connections_pool(self):
        ...
        try:
            ...
            for i in range(5):
                try:
                    ...
                    if result[0] == 1:
                        ...
                        db.close()
                        self.log_test("database_tests", "connection_pool", "FAILED", 
            ...
            self.log_test("database_tests", "connection_pool", "PASSED", 
    ...
    def test_security_configurations(self):
        ...
        try:
            ...
            if len(settings.SECRET_KEY) >= 32:
                self.log_test("security_tests", "secret_key_strength", "PASSED", 
                             f"Secret key length: {len(settings.SECRET_KEY)}")
            else:
                self.log_test("security_tests", "secret_key_strength", "FAILED", 
                             f"Secret key too short: {len(settings.SECRET_KEY)} chars")
            
            # Check default secret key
            if settings.SECRET_KEY == "your-secret-key-change-in-production":
                self.log_test("security_tests", "default_secret_key", "FAILED", 
                             "Using default secret key - security risk!")
            else:
                self.log_test("security_tests", "default_secret_key", "PASSED", 
                             "Custom secret key configured")
            
            # Check token expiration
            if settings.ACCESS_TOKEN_EXPIRE_MINUTES > 0:
                self.log_test("security_tests", "token_expiration", "PASSED", 
                             f"Token expiration: {settings.ACCESS_TOKEN_EXPIRE_MINUTES} minutes")
            else:
                self.log_test("security_tests", "token_expiration", "FAILED", 
            ...
            if "*" in settings.ALLOWED_ORIGINS:
                ...
                self.log_test("security_tests", "cors_configuration", "PASSED", 
    ...
    def check_existing_users(self):
        ...
        try:
            ...
            self.log_test("database_tests", "existing_users_count", "INFO", 
            ...
            if user_count > 0:
                ...
                for user in users:
                    if user.email and user.hashed_password and user.id:
                        ...
                        self.log_test("database_tests", "user_data_integrity", "WARNING", 
                                     f"User {user.id} has missing critical data")
                
                if valid_users == user_count:
                    self.log_test("database_tests", "user_data_integrity", "PASSED", 
                                 f"All {user_count} users have valid data")
                else:
                    self.log_test("database_tests", "user_data_integrity", "FAILED", 
    ...
    def test_two_factor_auth_readiness(self):
        ...
        try:
            ...
            if 'two_factor_enabled' in columns:
                self.log_test("security_tests", "2fa_database_ready", "PASSED", 
                             "Two-factor authentication field exists in database")
                
                # Check if any users have 2FA enabled
                users_with_2fa = db.execute(text("SELECT COUNT(*) FROM users WHERE two_factor_enabled = 1")).fetchone()[0]
                self.log_test("security_tests", "2fa_usage", "INFO", 
                             f"{users_with_2fa} users have 2FA enabled")
            else:
                self.log_test("security_tests", "2fa_database_ready", "FAILED", 
    ...
    def generate_recommendations(self):
        ...
        if any("FAILED" in test.get("status", "") for test in self.test_results.get("database_tests", {}).values()):
            recommendations.append("Database connectivity issues detected - check connection strings and database file permissions")
        
        if any("FAILED" in test.get("status", "") for test in self.test_results.get("auth_tests", {}).values()):
            recommendations.append("Authentication system issues detected - review JWT configuration and password hashing")
        ...
        if not redis_client:
            recommendations.append("Configure Redis for better session management and rate limiting")
        
        # Database-specific recommendations
        if settings.DATABASE_URL.startswith("sqlite"):
            recommendations.append("Consider migrating to PostgreSQL for production use")
