    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connection_metrics": {
                ...
                "avg_time_ms": (
                    statistics.mean(self.connection_times) * 1000
                    if self.connection_times
                    else 0
                ),
                "min_time_ms": (
                    min(self.connection_times) * 1000 if self.connection_times else 0
                ),
                "max_time_ms": (
                    max(self.connection_times) * 1000 if self.connection_times else 0
                ),
            },
            "message_latency_metrics": {
                "count": len(self.message_latencies),
                "avg_latency_ms": (
                    statistics.mean(self.message_latencies) * 1000
                    if self.message_latencies
                    else 0
                ),
                "min_latency_ms": (
                    min(self.message_latencies) * 1000 if self.message_latencies else 0
                ),
                "max_latency_ms": (
                    max(self.message_latencies) * 1000 if self.message_latencies else 0
                ),
                "p95_latency_ms": (
                    statistics.quantiles(self.message_latencies, n=20)[18] * 1000
            ...
            "reconnection_metrics": {
                ...
                "avg_time_ms": (
                    statistics.mean(self.reconnection_times) * 1000
                    if self.reconnection_times
                    else 0
                ),
            },
            "system_metrics": {
...
class ComprehensiveWebSocketTester:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                results["quote_endpoint"] = {
                    ...
                    in [200, 404],  # 404 is acceptable if no data
                    "status_code": response.status_code,
                    "data": response.json() if response.status_code == 200 else None,
                }
    ...
    async def test_subscription_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            subscription_message = {"action": "subscribe", "symbols": test_symbols}
            ...
            while True:
                try:
                    ...
                    if message_type == "subscribed":
                        ...
                        logger.info(
                            f"Subscription confirmed for: {data.get('symbols', [])}"
                        )
                        break
            ...
            # Test unsubscription
            unsubscription_message = {"action": "unsubscribe", "symbols": ["AAPL"]}
    ...
    async def test_multiple_connections(
        ...
        try:
            ...
            logger.info(
                f"Multiple connections test: {results['successful_connections']}/{num_connections} successful"
            )
    ...
    async def test_error_handling(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                message = await asyncio.wait_for(websocket.recv(), timeout=5.0)
                data = json.loads(message)
                if data.get("type") == "error" and "Invalid JSON" in data.get(
                    "message", ""
                ):
            ...
            try:
                ...
                message = await asyncio.wait_for(websocket.recv(), timeout=5.0)
                data = json.loads(message)
                if data.get("type") == "error" and "Unknown action" in data.get(
                    "message", ""
                ):
        ...
        try:
            ...
            if self.metrics.message_latencies:
                results["avg_latency_ms"] = (
                    statistics.mean(self.metrics.message_latencies) * 1000
                )
    ...
    async def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        test_results["tests"]["multiple_connections"] = (
            await self.test_multiple_connections(5)
        )
        ...
        test_results["overall_health_score"] = self._calculate_health_score(
            test_results
        )
...
async def main():
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                "http://localhost:8000/api/v1/streaming/status", timeout=5.0
            )
            if response.status_code != 200:
                logger.error(
                    "Server is not responding correctly. Please ensure the FastAPI server is running."
                )
                return
    except Exception as e:
    ...
    try:
        ...
        print(
            f"Overall Health Score: {health.get('total_score', 0)}/{health.get('max_score', 0)} ({health.get('percentage', 0):.1f}%)"
        )
        ...
        print(
            f"  Average Message Latency: {latency_metrics.get('avg_latency_ms', 0):.1f}ms"
        )
