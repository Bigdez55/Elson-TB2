...
from app.services.market_integration import (
    MarketIntegrationService,
    get_market_integration_service,
)
...
async def provider_health(current_user: User = Depends(get_current_active_user)):
...
async def available_providers(current_user: User = Depends(get_current_active_user)):
...
async def external_historical(
    ...
    end_date: Optional[str] = Query(None, description="End date (YYYY-MM-DD)"),
    interval: str = Query(
        "daily", description="Data interval (daily, weekly, monthly)"
    ),
    current_user: User = Depends(get_current_active_user),
):
...
async def reconcile_orders(
    max_age_days: int = Query(
        30, ge=1, le=90, description="Maximum age of orders to reconcile"
    ),
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db),
    integration: MarketIntegrationService = Depends(get_integration_service),
