...
from app.services.market_data_streaming_enhanced import StreamingQuote, get_streaming_service
...
@router.get("/streaming/quote/{symbol}", response_model=Dict)
async def get_streaming_quote(
    ...
    if not quote:
        return {
            "symbol": symbol,
            "error": "No streaming quote available",
            "timestamp": time.time(),
        }
...
async def get_quote(
    ...
    try:
        ...
        logger.info(
            f"Quote request for {symbol} from user {current_user.id if current_user else 'anonymous'}"
        )
        return quote
...
async def get_batch_quotes(
    ...
    try:
        ...
        if len(symbols) > 20:
            raise HTTPException(
                status_code=400, detail="Maximum 20 symbols per request"
            )

        quotes = await market_data.get_batch_quotes(symbols, force_refresh)
        logger.info(
            f"Batch quote request for {len(symbols)} symbols from user {current_user.id if current_user else 'anonymous'}"
        )
        return quotes
    except Exception as e:
        logger.error(f"Error getting batch quotes: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Error getting batch quotes: {str(e)}"
        )
...
async def get_market_hours(
    market: str = Path(..., description="Market to get hours for"),
    market_data: MarketDataService = Depends(get_market_data_service),
    current_user: Optional[User] = Depends(get_current_active_user_optional),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting market hours: {str(e)}"
        )
