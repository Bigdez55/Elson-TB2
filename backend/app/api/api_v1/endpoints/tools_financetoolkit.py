...
async def get_financial_statements(
    ...
    cache_key = _cache_key("statements", {"symbol": symbol, "period": period, "limit": limit})
    ...
    if toolkit:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to fetch statements: {e}")
    else:
        # Mock data
        base_year = 2025
        income_statements = []
...
async def get_ratio_summary(
    ...
    profitability_ratios = [
        {"name": "return_on_equity", "value": "0.285", "formula": RATIO_FORMULAS["return_on_equity"], "interpretation": RATIO_INTERPRETATIONS["return_on_equity"], "benchmark": "0.15"},
        {"name": "return_on_assets", "value": "0.128", "formula": RATIO_FORMULAS["return_on_assets"], "interpretation": RATIO_INTERPRETATIONS["return_on_assets"], "benchmark": "0.05"},
        {"name": "gross_margin", "value": "0.448", "formula": RATIO_FORMULAS["gross_margin"], "interpretation": RATIO_INTERPRETATIONS["gross_margin"], "benchmark": "0.35"},
        {"name": "operating_margin", "value": "0.302", "formula": RATIO_FORMULAS["operating_margin"], "interpretation": RATIO_INTERPRETATIONS["operating_margin"], "benchmark": "0.15"},
        {"name": "net_profit_margin", "value": "0.252", "formula": RATIO_FORMULAS["net_profit_margin"], "interpretation": RATIO_INTERPRETATIONS["net_profit_margin"], "benchmark": "0.10"},
    ]
    categories.append({"category": "profitability", "ratios": profitability_ratios})

    # Liquidity ratios
    liquidity_ratios = [
        {"name": "current_ratio", "value": "1.60", "formula": RATIO_FORMULAS["current_ratio"], "interpretation": RATIO_INTERPRETATIONS["current_ratio"], "benchmark": "1.50"},
        {"name": "quick_ratio", "value": "1.52", "formula": RATIO_FORMULAS["quick_ratio"], "interpretation": RATIO_INTERPRETATIONS["quick_ratio"], "benchmark": "1.00"},
        {"name": "cash_ratio", "value": "0.60", "formula": RATIO_FORMULAS["cash_ratio"], "interpretation": "Higher is better - measures ability to pay with cash", "benchmark": "0.20"},
    ]
    categories.append({"category": "liquidity", "ratios": liquidity_ratios})

    # Solvency ratios
    solvency_ratios = [
        {"name": "debt_to_equity", "value": "1.22", "formula": RATIO_FORMULAS["debt_to_equity"], "interpretation": RATIO_INTERPRETATIONS["debt_to_equity"], "benchmark": "1.00"},
        {"name": "debt_to_assets", "value": "0.55", "formula": RATIO_FORMULAS["debt_to_assets"], "interpretation": "Lower is generally better - measures leverage", "benchmark": "0.50"},
        {"name": "interest_coverage", "value": "29.5", "formula": RATIO_FORMULAS["interest_coverage"], "interpretation": RATIO_INTERPRETATIONS["interest_coverage"], "benchmark": "3.00"},
    ]
    categories.append({"category": "solvency", "ratios": solvency_ratios})

    # Valuation ratios
    valuation_ratios = [
        {"name": "pe_ratio", "value": "28.5", "formula": RATIO_FORMULAS["pe_ratio"], "interpretation": RATIO_INTERPRETATIONS["pe_ratio"], "benchmark": "20.0"},
        {"name": "price_to_book", "value": "35.2", "formula": RATIO_FORMULAS["price_to_book"], "interpretation": RATIO_INTERPRETATIONS["price_to_book"], "benchmark": "3.0"},
        {"name": "price_to_sales", "value": "7.2", "formula": RATIO_FORMULAS["price_to_sales"], "interpretation": "Lower may indicate value - compare to peers", "benchmark": "2.0"},
        {"name": "ev_to_ebitda", "value": "20.5", "formula": RATIO_FORMULAS["ev_to_ebitda"], "interpretation": RATIO_INTERPRETATIONS["ev_to_ebitda"], "benchmark": "12.0"},
        {"name": "dividend_yield", "value": "0.005", "formula": RATIO_FORMULAS["dividend_yield"], "interpretation": RATIO_INTERPRETATIONS["dividend_yield"], "benchmark": "0.02"},
    ]
    categories.append({"category": "valuation", "ratios": valuation_ratios})

    # Growth ratios
    growth_ratios = [
        {"name": "revenue_growth", "value": "0.08", "formula": RATIO_FORMULAS["revenue_growth"], "interpretation": RATIO_INTERPRETATIONS["revenue_growth"], "benchmark": "0.05"},
        {"name": "earnings_growth", "value": "0.12", "formula": RATIO_FORMULAS["earnings_growth"], "interpretation": "Positive is good - shows profit expansion", "benchmark": "0.10"},
        {"name": "dividend_growth", "value": "0.05", "formula": RATIO_FORMULAS["dividend_growth"], "interpretation": "Positive indicates increasing shareholder returns", "benchmark": "0.03"},
    ]
...
async def get_ratio_category(
    ...
    cache_key = _cache_key("ratios_category", {
        "symbol": symbol,
    ...
    category_ratios = {
        RatioCategoryEnum.PROFITABILITY: ["return_on_equity", "return_on_assets", "gross_margin", "operating_margin", "net_profit_margin"],
        RatioCategoryEnum.LIQUIDITY: ["current_ratio", "quick_ratio", "cash_ratio", "operating_cash_flow_ratio"],
        RatioCategoryEnum.SOLVENCY: ["debt_to_equity", "debt_to_assets", "interest_coverage", "debt_to_ebitda"],
        RatioCategoryEnum.VALUATION: ["pe_ratio", "price_to_book", "price_to_sales", "ev_to_ebitda", "dividend_yield"],
        RatioCategoryEnum.GROWTH: ["revenue_growth", "earnings_growth", "book_value_growth", "dividend_growth"],
        RatioCategoryEnum.EFFICIENCY: ["asset_turnover", "inventory_turnover", "receivables_turnover", "days_sales_outstanding"],
    }
    ...
    return ToolResponse(
        tool="financetoolkit_category",
        request_params={"symbol": symbol, "category": category.value, "periods": periods},
...
async def compare_ratios(
    ...
    ratios_to_compare = {
        RatioCategoryEnum.PROFITABILITY: ["return_on_equity", "return_on_assets", "net_profit_margin"],
        ...
        RatioCategoryEnum.ALL: ["return_on_equity", "pe_ratio", "debt_to_equity", "revenue_growth"],
    }
