...
class RoundupTransactionBase(BaseModel):
    ...
    source: Optional[str] = Field(
        None, description="Source of transaction (e.g., 'card', 'bank')"
    )


class UserSettingsBase(BaseModel):
    micro_investing_enabled: bool = Field(
        False, description="Whether micro-investing is enabled"
    )
    roundup_enabled: bool = Field(False, description="Whether roundups are enabled")
    roundup_multiplier: float = Field(
        1.0, description="Multiplier for roundups (1x, 2x, 3x)"
    )
    roundup_frequency: RoundupFrequencyEnum = Field(
        RoundupFrequencyEnum.WEEKLY,
        description="How often to invest collected roundups",
    )
    roundup_threshold: float = Field(
        5.0, description="Minimum amount to trigger investment"
    )
    micro_invest_target_type: MicroInvestTargetEnum = Field(
        MicroInvestTargetEnum.DEFAULT_PORTFOLIO,
        description="Where to invest micro-investments",
    )
    micro_invest_portfolio_id: Optional[int] = Field(
        None, description="Target portfolio ID for micro-investments"
    )
    micro_invest_symbol: Optional[str] = Field(
        None, description="Target symbol for micro-investments"
    )
    notify_on_roundup: bool = Field(
        True, description="Whether to notify on roundup collection"
    )
    notify_on_investment: bool = Field(
        True, description="Whether to notify on investment"
    )
    max_weekly_roundup: float = Field(
        100.0, description="Maximum weekly roundup amount"
    )
    max_monthly_micro_invest: float = Field(
        500.0, description="Maximum monthly micro-investment amount"
    )
...
class MicroInvestmentBase(BaseModel):
    ...
    investment_amount: float = Field(
        ..., gt=0, description="Amount to invest in dollars"
    )
    description: Optional[str] = Field(
        None, description="Optional description for the investment"
    )
