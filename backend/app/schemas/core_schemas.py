...
class FinancialGoal(BaseModel):
    ...
    current_amount: Decimal = Field(default=Decimal("0"), description="Current progress")
    ...
    monthly_contribution: Optional[Decimal] = Field(None, description="Suggested monthly contribution")
...
class ExpenseCategory(BaseModel):
    ...
    reduction_potential: Optional[Decimal] = Field(None, description="Possible monthly savings")
...
class FinancialPlan(BaseModel):
    ...
    planning_period_months: int = Field(default=12, description="Planning period in months")
    ...
    emergency_fund_months: Decimal = Field(..., description="Months of expenses covered")
...
class AllocationTarget(BaseModel):
    ...
    min_percent: Decimal = Field(..., ge=0, le=100, description="Rebalancing lower bound")
    max_percent: Decimal = Field(..., ge=0, le=100, description="Rebalancing upper bound")
...
class RebalancingRule(BaseModel):
    ...
    calendar_frequency: Optional[Literal["monthly", "quarterly", "semi_annual", "annual"]] = None
    threshold_percent: Optional[Decimal] = Field(None, description="Drift threshold to trigger")
    tax_aware: bool = True
    notes: Optional[str] = None
...
class PortfolioPolicy(BaseModel):
    ...
    risk_capacity: Literal["low", "medium", "high"] = Field(..., description="Ability to take risk")
    time_horizon: TimeHorizon

    # Investment objectives
    primary_objective: Literal["growth", "income", "preservation", "balanced"]
    return_target: Optional[str] = Field(None, description="Target return description, not guarantee")
    income_requirement: Optional[Decimal] = Field(None, description="Annual income need")
    ...
    concentration_limits: Dict[str, Decimal] = Field(
        ...
        description="Max allocation to any single security/sector"
    )
...
class ScenarioCase(BaseModel):
    ...
    description: str
    probability: Optional[Decimal] = Field(None, ge=0, le=100, description="Probability %")
...
class ScenarioAnalysis(BaseModel):
    ...
    analysis_type: Literal["projection", "comparison", "risk_assessment", "optimization"]
...
class EntryLogic(BaseModel):
    ...
    setup_description: str = Field(..., description="What conditions trigger entry consideration")
...
class TradePlan(BaseModel):
    ...
    instrument_type: str = Field(..., description="Type of instrument (stock, ETF, etc.)")
    ...
    position_sizing_method: str = Field(..., description="How position size is calculated")
    ...
    review_criteria: List[str] = Field(
        default_factory=list,
        description="How to review the trade after completion"
    )
...
class ComplianceCheck(BaseModel):
    ...
    request_summary: str = Field(..., description="Summary of the request being checked")

    # Overall status
    overall_status: Literal["approved", "approved_with_conditions", "blocked", "needs_review"]
    ...
    allowed_response_types: List[str] = Field(
        default_factory=list,
        description="Types of responses allowed given the request"
    )
    prohibited_content: List[str] = Field(
        default_factory=list,
        description="Content that must not be included"
    )
    ...
    professional_type: Optional[str] = Field(
        None,
        description="Type of professional to refer to (CPA, attorney, CFP, etc.)"
    )
    ...
    tone_guidance: Optional[str] = Field(
        None,
        description="Guidance on appropriate tone for response"
    )
    depth_limit: Optional[str] = Field(
        None,
        description="Limit on response depth for compliance"
    )
...
def get_schema(schema_name: str) -> type:
    ...
    if schema_name not in CORE_SCHEMAS:
        raise ValueError(f"Unknown schema: {schema_name}. Available: {list(CORE_SCHEMAS.keys())}")
