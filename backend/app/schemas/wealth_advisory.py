...
class AdvisoryMode(str, Enum):
    ...
    RETIREMENT_PLANNING = (
        "retirement_planning"  # NEW: Retirement income, Social Security, pensions
    )
...
class ProfessionalRecommendation(BaseModel):
    ...
    credentials: list[str] = Field(
        default_factory=list, description="Relevant credentials"
    )
    responsibilities: list[str] = Field(
        default_factory=list, description="Key responsibilities"
    )
    why_recommended: str = Field(
        ..., description="Why this professional is recommended"
    )
...
class ActionItem(BaseModel):
    ...
    professional_needed: Optional[str] = Field(
        None, description="Professional to consult"
    )
...
class WealthAdvisoryRequest(BaseModel):
    ...
    query: str = Field(
        ..., min_length=5, max_length=2000, description="The advisory question"
    )
    ...
    wealth_tier: Optional[WealthTier] = Field(
        None, description="Client's wealth tier for context-appropriate advice"
    )
    include_citations: bool = Field(
        default=True, description="Include knowledge base citations"
    )
    include_professionals: bool = Field(
        default=True, description="Include professional recommendations"
    )
    family_context: Optional[dict[str, Any]] = Field(
        None, description="Additional family or situation context"
    )
...
class EstatePlanningRequest(BaseModel):
    ...
    situation: str = Field(
        ..., min_length=10, description="Description of estate planning needs"
    )
    estimated_estate_value: Optional[float] = Field(
        None, ge=0, description="Estimated estate value"
    )
    family_members: Optional[int] = Field(
        None, ge=1, description="Number of family members"
    )
    has_business: bool = Field(
        default=False, description="Whether client owns a business"
    )
    charitable_intent: bool = Field(
        default=False, description="Charitable giving interest"
    )
    state_of_residence: Optional[str] = Field(
        None, description="State for jurisdiction-specific advice"
    )
    existing_documents: Optional[list[str]] = Field(
        None, description="List of existing estate documents"
    )
...
class SuccessionPlanningRequest(BaseModel):
    ...
    estimated_value: Optional[float] = Field(
        None, ge=0, description="Estimated business value"
    )
    owner_age: Optional[int] = Field(
        None, ge=18, le=120, description="Business owner's age"
    )
    years_until_exit: Optional[int] = Field(
        None, ge=0, description="Target years until exit"
    )
    potential_successors: Optional[list[str]] = Field(
        None, description="Potential successor types (family, employee, external)"
    )
    exit_preferences: Optional[list[str]] = Field(
        None, description="Preferred exit options"
    )
...
class TeamCoordinationRequest(BaseModel):
    ...
    situation_details: str = Field(
        ..., min_length=20, description="Details of the situation"
    )
    wealth_tier: WealthTier = Field(..., description="Client's wealth tier")
    existing_advisors: Optional[list[str]] = Field(
        None, description="List of existing advisor types"
    )
    budget_constraints: Optional[str] = Field(
        None, description="Any budget constraints for advisory team"
    )
...
class FinancialLiteracyRequest(BaseModel):
    ...
    specific_questions: Optional[list[str]] = Field(
        None, description="Specific questions about the topic"
    )
    current_financial_situation: Optional[str] = Field(
        None, description="Brief description of current financial situation"
    )
    learning_goals: Optional[list[str]] = Field(
        None, description="What the user wants to achieve"
    )
...
class CredentialInfoRequest(BaseModel):
    ...
    credential_type: CredentialType = Field(
        ..., description="The credential to learn about"
    )
    include_study_resources: bool = Field(
        default=True, description="Include study provider information"
    )
    include_career_path: bool = Field(
        default=True, description="Include career path information"
    )
...
class RoleInfoRequest(BaseModel):
    ...
    wealth_tier_context: Optional[WealthTier] = Field(
        None, description="Wealth tier for context-specific information"
    )
...
class RetirementPlanningRequest(BaseModel):
    ...
    target_retirement_age: int = Field(
        default=65, ge=30, le=100, description="Target retirement age"
    )
    current_retirement_savings: float = Field(
        default=0, ge=0, description="Current retirement savings"
    )
    annual_income: float = Field(..., ge=0, description="Current annual income")
    employer_401k_match: Optional[float] = Field(
        None, ge=0, le=1, description="Employer 401k match percentage"
    )
    current_contributions: Optional[dict[str, float]] = Field(
        None, description="Current retirement contributions by account type"
    )
    has_pension: bool = Field(default=False, description="Whether client has a pension")
    desired_retirement_income: Optional[float] = Field(
        None, ge=0, description="Desired annual retirement income"
    )
    social_security_estimate: Optional[float] = Field(
        None, ge=0, description="Estimated Social Security benefit"
    )
    ...
    specific_questions: Optional[list[str]] = Field(
        None, description="Specific retirement questions"
    )
...
class CollegePlanningRequest(BaseModel):
    ...
    target_college_start_age: int = Field(
        default=18, ge=16, le=30, description="Age when starting college"
    )
    current_529_balance: float = Field(
        default=0, ge=0, description="Current 529 plan balance"
    )
    monthly_contribution: Optional[float] = Field(
        None, ge=0, description="Current monthly contribution"
    )
    ...
    number_of_children: int = Field(
        default=1, ge=1, description="Number of children to plan for"
    )
    household_income: Optional[float] = Field(
        None, ge=0, description="Household income for financial aid estimates"
    )
    state_of_residence: Optional[str] = Field(
        None, description="State of residence for 529 tax benefits"
    )
    interested_in_financial_aid: bool = Field(
        default=True, description="Interested in financial aid optimization"
    )
    specific_questions: Optional[list[str]] = Field(
        None, description="Specific college planning questions"
    )
...
class GoalPlanningRequest(BaseModel):
    ...
    current_assets: float = Field(
        default=0, ge=0, description="Current total investable assets"
    )
    monthly_savings: Optional[float] = Field(
        None, ge=0, description="Current monthly savings"
    )
    ...
    target_assets: Optional[float] = Field(
        None, ge=0, description="Specific target asset amount"
    )
    target_timeline_years: Optional[int] = Field(
        None, ge=1, description="Target years to achieve goal"
    )
    career_flexibility: str = Field(
        default="medium", description="Career flexibility: low, medium, high"
    )
    geographic_flexibility: bool = Field(
        default=False, description="Willing to relocate for opportunities"
    )
    risk_tolerance: str = Field(
        default="moderate",
        description="Risk tolerance: conservative, moderate, aggressive",
    ...
    constraints: Optional[dict[str, Any]] = Field(
        None, description="Any constraints or preferences"
    )
...
class WealthAdvisoryResponse(BaseModel):
    ...
    wealth_tier: Optional[WealthTier] = Field(
        None, description="The wealth tier context"
    )
    citations: list[Citation] = Field(
        default_factory=list, description="Knowledge base citations"
    )
    recommended_professionals: list[ProfessionalRecommendation] = Field(
        default_factory=list, description="Recommended professionals to consult"
    )
    next_steps: list[ActionItem] = Field(
        default_factory=list, description="Recommended next steps"
    )
    confidence: float = Field(
        default=0.85, ge=0, le=1, description="Confidence score 0-1"
    )
...
class EstatePlanningResponse(BaseModel):
    ...
    recommended_structures: list[dict[str, Any]] = Field(
        default_factory=list, description="Recommended trust and estate structures"
    )
    tax_strategies: list[str] = Field(
        default_factory=list, description="Tax minimization strategies"
    )
    document_checklist: list[str] = Field(
        default_factory=list, description="Required estate planning documents"
    )
    professional_team: list[ProfessionalRecommendation] = Field(
        default_factory=list, description="Professional team recommendations"
    )
    citations: list[Citation] = Field(default_factory=list)
    timeline_recommendation: Optional[str] = Field(
        None, description="Suggested timeline"
    )
...
class SuccessionPlanningResponse(BaseModel):
    ...
    recommended_exit_options: list[dict[str, Any]] = Field(
        default_factory=list, description="Ranked exit options with pros/cons"
    )
    dream_team_structure: list[ProfessionalRecommendation] = Field(
        default_factory=list, description="Dream Team professional coordination"
    )
    valuation_considerations: list[str] = Field(
        default_factory=list, description="Business valuation factors"
    )
    tax_strategies: list[str] = Field(
        default_factory=list, description="Tax optimization strategies"
    )
    timeline_phases: list[dict[str, str]] = Field(
        default_factory=list, description="Succession timeline phases"
    )
    citations: list[Citation] = Field(default_factory=list)
...
class TeamCoordinationResponse(BaseModel):
    ...
    recommended_team: list[ProfessionalRecommendation] = Field(
        default_factory=list, description="Recommended professional team"
    )
    ...
    quarterly_review_agenda: list[str] = Field(
        default_factory=list, description="Quarterly review meeting agenda items"
    )
    citations: list[Citation] = Field(default_factory=list)
...
class FinancialLiteracyResponse(BaseModel):
    ...
    key_concepts: list[str] = Field(
        default_factory=list, description="Key concepts to understand"
    )
    practical_steps: list[str] = Field(
        default_factory=list, description="Practical steps to take"
    )
    common_mistakes: list[str] = Field(
        default_factory=list, description="Common mistakes to avoid"
    )
    resources: list[str] = Field(
        default_factory=list, description="Additional learning resources"
    )
    next_topics: list[str] = Field(
        default_factory=list, description="Suggested next topics to learn"
    )
    citations: list[Citation] = Field(default_factory=list)
...
class CredentialInfoResponse(BaseModel):
    ...
    issuing_organization: str = Field(
        ..., description="Organization that issues credential"
    )
    requirements: dict[str, Any] = Field(
        default_factory=dict, description="Requirements to obtain"
    )
    study_details: dict[str, Any] = Field(
        default_factory=dict, description="Study hours, materials, costs"
    )
    study_providers: Optional[list[dict[str, Any]]] = Field(
        None, description="Study material providers"
    )
    career_applications: list[str] = Field(
        default_factory=list, description="Career applications"
    )
    related_credentials: list[str] = Field(
        default_factory=list, description="Related credentials"
    )
    citations: list[Citation] = Field(default_factory=list)
...
class RoleInfoResponse(BaseModel):
    ...
    typical_credentials: list[str] = Field(
        default_factory=list, description="Typical credentials held"
    )
    ...
    works_with: list[str] = Field(
        default_factory=list, description="Other roles this professional works with"
    )
    when_to_engage: list[str] = Field(
        default_factory=list, description="Situations when to engage this professional"
    )
    citations: list[Citation] = Field(default_factory=list)
...
class RetirementAccountRecommendation(BaseModel):
    ...
    recommended_contribution: float = Field(
        ..., description="Recommended annual contribution"
    )
...
class RetirementPlanningResponse(BaseModel):
    ...
    target_retirement_savings: float = Field(
        ..., description="Target savings at retirement"
    )
    projected_retirement_savings: float = Field(
        ..., description="Projected savings at current rate"
    )
    savings_gap: float = Field(..., description="Gap between target and projected")
    recommended_monthly_savings: float = Field(
        ..., description="Recommended monthly savings"
    )
    account_recommendations: list[RetirementAccountRecommendation] = Field(
        default_factory=list, description="Recommended retirement accounts"
    )
    social_security_strategy: Optional[str] = Field(
        None, description="Social Security claiming strategy"
    )
    withdrawal_strategy: Optional[str] = Field(
        None, description="Recommended withdrawal strategy"
    )
    milestones: list[RetirementMilestone] = Field(
        default_factory=list, description="Key milestones in retirement journey"
    )
    tax_strategies: list[str] = Field(
        default_factory=list, description="Tax optimization strategies"
    )
    risk_factors: list[str] = Field(
        default_factory=list, description="Risks to retirement plan"
    )
    recommended_professionals: list[ProfessionalRecommendation] = Field(
        default_factory=list, description="Professionals to consult"
    )
    citations: list[Citation] = Field(default_factory=list)
...
class CollegeSavingsStrategy(BaseModel):
    ...
    recommended_monthly_contribution: float = Field(
        ..., description="Recommended monthly contribution"
    )
    state_tax_benefit: Optional[str] = Field(
        None, description="State tax deduction benefit"
    )
...
class FinancialAidEstimate(BaseModel):
    ...
    estimated_efc: Optional[float] = Field(
        None, description="Estimated Family Contribution"
    )
    estimated_need: Optional[float] = Field(
        None, description="Estimated financial need"
    )
    likely_aid_types: list[str] = Field(
        default_factory=list, description="Types of aid likely to receive"
    )
    optimization_strategies: list[str] = Field(
        default_factory=list, description="Strategies to maximize aid"
    )
...
class CollegePlanningResponse(BaseModel):
    ...
    years_until_college: int = Field(
        ..., description="Years until child starts college"
    )
    ...
    projected_savings_at_start: float = Field(
        ..., description="Projected savings when college starts"
    )
    funding_gap: float = Field(..., description="Gap between savings and cost")
    savings_strategies: list[CollegeSavingsStrategy] = Field(
        default_factory=list, description="Recommended savings strategies"
    )
    financial_aid_estimate: Optional[FinancialAidEstimate] = Field(
        None, description="Financial aid estimates if applicable"
    )
    alternative_options: list[str] = Field(
        default_factory=list, description="Alternative education funding options"
    )
    tax_strategies: list[str] = Field(
        default_factory=list, description="Tax benefits and strategies"
    )
    timeline_actions: list[dict[str, Any]] = Field(
        default_factory=list, description="Actions by timeline"
    )
    recommended_professionals: list[ProfessionalRecommendation] = Field(
        default_factory=list, description="Professionals to consult"
    )
...
class TierProgressionMilestone(BaseModel):
    ...
    actions: list[str] = Field(
        default_factory=list, description="Key actions for this period"
    )
...
class AccelerationStrategy(BaseModel):
    ...
    difficulty: str = Field(
        default="medium", description="Difficulty: low, medium, high"
    )
...
class GoalPlanningResponse(BaseModel):
    ...
    recommended_timeline_years: int = Field(
        ..., description="Recommended years to achieve goal"
    )
    ...
    projected_assets_at_timeline: float = Field(
        ..., description="Projected assets at timeline end"
    )
    yearly_roadmap: list[TierProgressionMilestone] = Field(
        default_factory=list, description="Year-by-year roadmap"
    )
    acceleration_strategies: list[AccelerationStrategy] = Field(
        default_factory=list, description="Strategies to accelerate progress"
    )
    key_milestones: list[str] = Field(
        default_factory=list, description="Key milestones to celebrate"
    )
    risk_factors: list[str] = Field(
        default_factory=list, description="Potential risks to plan"
    )
    advisors_by_stage: dict[str, list[str]] = Field(
        default_factory=dict, description="Recommended advisors at each stage"
    )
    recommended_professionals: list[ProfessionalRecommendation] = Field(
        default_factory=list, description="Professionals to consult now"
    )
