...
# Note: These enums may need adjustment based on current models
...
class DollarBasedInvestmentBase(BaseModel):
    ...
    symbol: str = Field(
        ..., description="Symbol of the asset to trade", min_length=1, max_length=10
    )
    investment_amount: Decimal = Field(
        ..., description="Dollar amount to invest", ge=1.0
    )
...
class RecurringInvestmentBase(DollarBasedInvestmentBase):
    ...
    frequency: str = Field(
        ...,
        description="Frequency of recurring investment (daily, weekly, monthly, quarterly)",
    )
    start_date: datetime = Field(
        ..., description="Date to start the recurring investment"
    )
    end_date: Optional[datetime] = Field(
        None, description="Date to end the recurring investment (optional)"
    )
    description: Optional[str] = Field(
        None, description="Description or purpose of the recurring investment"
    )
...
class RecurringInvestmentUpdate(BaseModel):
    ...
    investment_amount: Optional[Decimal] = Field(
        None, description="Dollar amount to invest", ge=1.0
    )
    ...
    end_date: Optional[datetime] = Field(
        None, description="Date to end the recurring investment"
    )
    is_active: Optional[bool] = Field(
        None, description="Whether the recurring investment is active"
    )
    description: Optional[str] = Field(
        None, description="Description or purpose of the recurring investment"
    )
...
class TradingConfig(BaseModel):
    ...
    minInvestmentAmount: float = Field(
        ..., description="Minimum investment amount in dollars"
    )
    maxInvestmentAmount: float = Field(
        ..., description="Maximum investment amount in dollars"
    )
    fractionalSharesEnabled: bool = Field(
        ..., description="Whether fractional shares are enabled"
    )
    dollarBasedTradingEnabled: bool = Field(
        ..., description="Whether dollar-based trading is enabled"
    )
    supportedOrderTypes: List[str] = Field(
        ..., description="List of supported order types"
    )
    availableTradingHours: Dict[str, str] = Field(
        ..., description="Available trading hours"
    )
    minFractionalQuantity: float = Field(
        ..., description="Minimum fractional quantity allowed"
    )
    investmentTiers: List[Dict[str, Any]] = Field(
        ..., description="Investment tiers configuration"
    )
