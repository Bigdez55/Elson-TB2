...
class PaperTradingService:
    ...
    def __init__(self, db: Session):
        ...
        self.base_slippage_bps = getattr(
            settings, "PAPER_TRADING_SLIPPAGE_BPS", 2
        )  # 2 basis points
        self.commission_per_share = getattr(
            settings, "PAPER_TRADING_COMMISSION", 0.005
        )  # $0.005 per share
        self.min_commission = getattr(
            settings, "PAPER_TRADING_MIN_COMMISSION", 1.0
        )  # $1 minimum
        self.max_commission = getattr(
            settings, "PAPER_TRADING_MAX_COMMISSION", 10.0
        )  # $10 maximum
    ...
    async def execute_paper_trade(
        self, trade: Trade, simulate_realistic_conditions: bool = True
    ) -> Dict[str, any]:
        """
    ...
    async def _simulate_order_execution(
        self, trade: Trade, current_price: float, realistic: bool
    ) -> Dict[str, any]:
        ...
        if trade.order_type == OrderType.MARKET:
            ...
            return self._create_rejection_result(
                trade, f"Unsupported order type: {trade.order_type}"
            )
    ...
    async def _execute_market_order(
        ...
        if random.random() > self.fill_probability:
            return self._create_rejection_result(
                trade, "Order not filled due to market conditions"
            )
    ...
    async def _calculate_slippage(self, trade: Trade, current_price: float) -> float:
        ...
        volatility_multiplier = random.uniform(
            0.8, 2.0
        ...
        total_slippage = (
            base_slippage * volume_multiplier * volatility_multiplier * time_multiplier
        )
    ...
    def _assess_fill_quality(
        self, slippage_bps: float, fill_ratio: float
    ) -> FillQuality:
    ...
    async def _update_trade_execution(
    ...
    async def get_execution_statistics(
        ...
        try:
            ...
            partial_fills = len(
                [t for t in trades if t.status == TradeStatus.PARTIALLY_FILLED]
            )
            ...
            return {
                ...
                "total_value": sum(
                    (t.filled_quantity or 0.0) * (t.filled_price or 0.0) for t in trades
                ),
    ...
    def _get_top_symbols(self, trades: List[Trade], limit: int = 5) -> List[Dict]:
        ...
        sorted_symbols = sorted(
            symbol_counts.items(), key=lambda x: x[1]["count"], reverse=True
        )
