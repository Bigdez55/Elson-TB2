...
class CoinbaseClient(BaseApiClient):
    ...
    def _add_auth(self, request_kwargs: Dict[str, Any]) -> None:
        ...
        signature = hmac.new(
            key=self.api_secret.encode(), msg=message.encode(), digestmod=hashlib.sha256
        ).hexdigest()
    ...
    async def get_stock_quote(self, symbol: str) -> Dict[str, Any]:
        ...
        if "errors" in response:
            ...
            error_msg = (
                ...
                if errors
                else f"No quote data found for symbol: {symbol}"
            )
    ...
    async def get_historical_data(
        ...
        response = await self.get(
            url=url, params=params, _bypass_base_url=True  # Flag to bypass base URL
        )
    ...
    async def get_company_profile(self, symbol: str) -> Dict[str, Any]:
        ...
        if "errors" in response:
            ...
            error_msg = (
                errors[0].get("message")
                if errors
                else f"No asset data found for symbol: {base_symbol}"
            )
            raise ApiError(error_msg)
    ...
    async def search_symbol(self, keywords: str) -> Dict[str, Any]:
        ...
        if "errors" in response:
            errors = response.get("errors", [])
            error_msg = (
                errors[0].get("message") if errors else "Failed to search currencies"
            )
            raise ApiError(error_msg)
    ...
    async def get_exchange_rates(self, base_currency: str = "USD") -> Dict[str, Any]:
        ...
        if "errors" in response:
            ...
            error_msg = (
                errors[0].get("message")
                if errors
                else f"Failed to get exchange rates for {base_currency}"
            )
            raise ApiError(error_msg)
