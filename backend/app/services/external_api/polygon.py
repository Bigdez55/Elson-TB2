...
class PolygonClient(BaseApiClient):
    ...
    async def get_company_profile(self, symbol: str) -> Dict[str, Any]:
        ...
        if (
            not ticker_details
            or "status" in ticker_details
            and ticker_details["status"] != "OK"
        ):
            error_msg = ticker_details.get(
                "error", f"No company data found for symbol: {symbol}"
            )
            raise ApiError(error_msg)
        ...
        try:
            ...
            financial_data = (
                financials.get("results", [{}])[0] if financials.get("results") else {}
            )
    ...
    async def search_symbol(self, keywords: str) -> Dict[str, Any]:
        ...
        if not response or "status" in response and response["status"] != "OK":
            error_msg = response.get(
                "error", f"No search results found for: {keywords}"
            )
            raise ApiError(error_msg)
