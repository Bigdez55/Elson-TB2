...
from app.models.account import RecurringFrequency, RecurringInvestment
from app.models.portfolio import Portfolio
from app.models.trade import InvestmentType, OrderType, Trade, TradeSource, TradeStatus
...
class InvestmentService:
    ...
    def create_dollar_based_investment(
        self, user_id: int, investment_data: DollarBasedInvestmentCreate
    ) -> Trade:
    ...
    def create_recurring_investment(
        self, user_id: int, investment_data: RecurringInvestmentCreate
        ...
        if start_date.date() <= datetime.utcnow().date():
            ...
            recurring_investment.next_investment_date = (
                self._calculate_next_investment_date(
                    start_date, RecurringFrequency(investment_data.frequency)
                )
            )
    ...
    def get_pending_dollar_investments(
    ...
    def calculate_performance(
        ...
        for investment in investments:
            ...
            try:
                current_price = Decimal(
                    str(self.market_data.get_current_price(investment.symbol))
                )
                ...
                logger.error(
                    f"Failed to get current price for {investment.symbol}: {e}"
                )
    ...
    def get_recurring_investments(self, user_id: int) -> List[RecurringInvestment]:
        ...
        return (
            self.db.query(RecurringInvestment)
            .filter(RecurringInvestment.user_id == user_id)
            .all()
        )

    def get_recurring_investment(
        self, recurring_id: int, user_id: int
    ) -> Optional[RecurringInvestment]:
    ...
    def get_recurring_investments_by_portfolio(
        self, portfolio_id: int
    ) -> List[RecurringInvestment]:
        """Get all recurring investments for a specific portfolio."""
        return (
            self.db.query(RecurringInvestment)
            .filter(RecurringInvestment.portfolio_id == portfolio_id)
            .all()
        )

    def update_recurring_investment(
        self, recurring_id: int, user_id: int, update_data: RecurringInvestmentUpdate
    ) -> Union[RecurringInvestment, Dict[str, Any]]:
    ...
    def get_due_recurring_investments(self) -> List[RecurringInvestment]:
        ...
        due_investments = (
            ...
            .filter(
                ...
                (RecurringInvestment.end_date >= now)
                | (RecurringInvestment.end_date.is_(None)),
            )
            .all()
        )
    ...
    def process_recurring_investments(self) -> Dict[str, Any]:
        ...
        results = {"total_due": 0, "processed": 0, "errors": 0, "trade_ids": []}
        ...
        for investment in due_investments:
            try:
                ...
                # Create the trade
                trade = self.create_dollar_based_investment(
                    investment.user_id, trade_data
                )
                ...
                investment.next_investment_date = self._calculate_next_investment_date(
                    datetime.utcnow(), investment.frequency
                )
