...
class FinGPTSentimentAnalyzer:
    ...
    def __init__(
        ...
        if model_key not in self.FINGPT_MODELS:
            logger.warning(f"Unknown model key: {model_key}. Available: {list(self.FINGPT_MODELS.keys())}")
    ...
    def load_model(self) -> 'FinGPTSentimentAnalyzer':
        ...
        if not self.available or self.model_config is None:
            logger.error("Cannot load model: PEFT not available or invalid model config")
            return self
...
def get_sentiment_analyzer(
    ...
    **kwargs
    ...
    if model_type == 'auto':
        ...
        raise ValueError(f"Unknown model_type: {model_type}. Use 'auto', 'fingpt', or 'transformer'")