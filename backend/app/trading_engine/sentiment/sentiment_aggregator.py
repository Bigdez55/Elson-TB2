...
class SentimentSource:
    ...
    async def analyze_sentiment(
        self, texts: List[str], analyzer=None, **kwargs
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            for text in texts:
                if any(
                    word in text.lower()
                    for word in ["good", "strong", "buy", "bullish", "exceeds"]
                ):
                    sentiment = "positive"
                    score = 0.8
                elif any(
                    word in text.lower()
                    for word in ["bad", "weak", "sell", "bearish", "misses"]
                ):
                    sentiment = "negative"
                    score = -0.8
                else:
...
class NewsAPISentimentSource(SentimentSource):
    ...
    async def fetch_data(
        self, symbols: List[str] = None, days: int = 1, max_articles: int = 100
    ) -> pd.DataFrame:
        """
...
class SocialMediaSentimentSource(SentimentSource):
    ...
    async def fetch_data(
        ...
        for symbol in symbols:
            ...
            for platform in self.platforms:
                ...
                for _ in range(num_posts):
                    ...
                    sentiment_type = np.random.choice(
                        ["bullish", "bearish", "neutral"], p=[0.45, 0.35, 0.2]
                    )
        ...
        if not social_df.empty:
            try:
                # Use our analyze_sentiment method, which has a fallback for tests
                sentiment_df = await self.analyze_sentiment(
                    social_df["content"].tolist()
                )
...
class AnalystRatingSentimentSource(SentimentSource):
    ...
    async def fetch_data(
        self, symbols: List[str] = None, days: int = 30
    ) -> pd.DataFrame:
        """
                ...
                ratings.append(
                    {
                        ...
                        "date": date.strftime("%Y-%m-%d"),
                        "price_target": (
                            round(price_target, 2) if price_target else None
                        ),
                        "current_price": round(current_price, 2),
                        "source": self.name,
                        "score": rating_types[
                            rating
                        ],  # Direct sentiment score from rating
                        "sentiment": (
                            "positive"
                            if rating_types[rating] > 0
                            else ("negative" if rating_types[rating] < 0 else "neutral")
                        ),
        ...
        if (
            ...
            and "price_target" in ratings_df.columns
            and "current_price" in ratings_df.columns
        ):
...
class SentimentAggregator:
    ...
    async def fetch_all_data(
        self, symbols: List[str] = None, days: int = 7
    ...
    async def aggregate(
        self, symbols: List[str] = None, days: int = 7, recency_weight: bool = True
    ) -> Dict[str, Any]:
        """
            ...
            if isinstance(source, AnalystRatingSentimentSource):
                ...
                if not df.empty:
                    ...
                    for symbol, group in df.groupby("symbol"):
                        ...
                        avg_target = (
                            group["price_target"].mean()
                            if "price_target" in group.columns
                            else None
                        )
    ...
    def get_sentiment_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "overall_sentiment_label": self.aggregated_data.get(
    ...
    async def analyze_sector_sentiment(
        ...
        for symbol, data in self.aggregated_data.get("symbols", {}).items():
            if symbol in sector_mapping:
                ...
                if score is not None and sector in sector_sentiment:
                    ...
                    if sector_sentiment[sector][
                        "strongest_symbol"
                    ] is None or score > self.aggregated_data["symbols"][
                        sector_sentiment[sector]["strongest_symbol"]
                    ].get(
                        "sentiment_score", 0
                    ):
                    ...
                    if sector_sentiment[sector][
                        ...
                        sector_sentiment[sector]["weakest_symbol"]
                    ].get(
                        "sentiment_score", 0
                    ):
