...
class AdaptiveParameterOptimizer:
    ...
    def __init__(
        ...
        self.stability_factor = 0.03  # Further reduced from 0.05 for even more gradual adaptation
        ...
        self.adaptation_frequency = datetime.timedelta(hours=36)  # Increased from 24 to 36 hours for more stability
            ...
            if 'default_parameters' in config:
                for regime_str, params in config['default_parameters'].items():
                    try:
                        ...
                        logger.warning(f"Unknown volatility regime in config: {regime_str}")
    ...
    def _load_performance_history(self) -> None:
        ...
        if os.path.exists(history_path):
            try:
                ...
                for regime_str, entries in history_data.get('performance_history', {}).items():
                    try:
                        regime = VolatilityRegime[regime_str.upper()]
                        self.performance_history[regime] = entries
                    except KeyError:
                        logger.warning(f"Unknown volatility regime in history: {regime_str}")
                        
                self.parameter_history = history_data.get('parameter_history', [])
                
                logger.info(f"Loaded performance history with {sum(len(h) for h in self.performance_history.values())} entries")
    ...
    def _save_performance_history(self) -> None:
        ...
        try:
            ...
            performance_history_serializable = {
                regime.name: entries for regime, entries in self.performance_history.items()
            }
    ...
    def detect_market_condition(
        ...
        volatility_regime, volatility_value = self.volatility_detector.detect_regime(data, vix_data)
        ...
        if 'close' in data.columns and len(data) >= 50:
            ...
            if len(data) >= 30:
                recent_volatility = data['close'].pct_change().iloc[-10:].std() * np.sqrt(252) * 100
                older_volatility = data['close'].pct_change().iloc[-30:-10].std() * np.sqrt(252) * 100
                
                regime_change = abs(recent_volatility - older_volatility) > 10  # 10% difference threshold
            else:
                regime_change = False
        ...
        logger.info(f"Detected market condition: Volatility={volatility_regime.name}, "
    ...
    def adapt_lookback_period(
        ...
        if market_condition == MarketCondition.REGIME_CHANGE:
            ...
            if volatility_regime in [VolatilityRegime.HIGH, VolatilityRegime.EXTREME]:
                ...
                adjustment = -5
        elif market_condition == MarketCondition.BULL_TRENDING or market_condition == MarketCondition.BEAR_TRENDING:
            # Longer lookback for trending markets, but only slightly for high volatility
            if volatility_regime in [VolatilityRegime.HIGH, VolatilityRegime.EXTREME]:
                adjustment = 1
            else:
                adjustment = 5
        elif market_condition == MarketCondition.BULL_VOLATILE or market_condition == MarketCondition.BEAR_VOLATILE:
    ...
    def adapt_confidence_threshold(
        ...
        """
        # Start with the default for this volatility regime
        base_threshold = self.optimized_parameters[volatility_regime]["confidence_threshold"]
    ...
    def adapt_prediction_horizon(
        ...
        """
        # Start with the default for this volatility regime
        base_horizon = self.optimized_parameters[volatility_regime]["prediction_horizon"]
    ...
    def adapt_model_weights(
        ...
        base_weights = self.optimized_parameters[volatility_regime]["model_weights"].copy()
        ...
        if volatility_regime == VolatilityRegime.HIGH:
            ...
            base_weights["random_forest"] = max(base_weights.get("random_forest", 3.0), 4.0)
    ...
    def update_performance_metrics(
        ...
        if len(self.performance_history[volatility_regime]) > self.history_window:
            self.performance_history[volatility_regime] = self.performance_history[volatility_regime][-self.history_window:]
        ...
        logger.info(f"Updated performance metrics for {volatility_regime.name} regime: "
    ...
    def optimize_parameters(
        ...
        if not force_update and self.last_adaptation_time is not None:
            ...
            if time_since_adaptation < self.adaptation_frequency:
                logger.info(f"Skipping parameter optimization, last adaptation was {time_since_adaptation} ago")
                return
        
        # Check if we have enough performance history
        if len(self.performance_history[volatility_regime]) < 5:
            logger.info(f"Not enough performance history for {volatility_regime.name} regime to optimize parameters")
            return
        
        logger.info(f"Optimizing parameters for {volatility_regime.name} volatility regime")
        ...
        recent_entries = self.performance_history[volatility_regime][-min(20, len(self.performance_history[volatility_regime])):]
        ...
        for entry in recent_entries:
            ...
            parameter_performance[param_key]["sharpe_ratios"].append(entry["sharpe_ratio"])
        ...
        for param_key, perf in parameter_performance.items():
            ...
            if volatility_regime in [VolatilityRegime.HIGH, VolatilityRegime.EXTREME]:
                performance_score = (0.6 * avg_win_rate) + (0.3 * avg_sharpe) + (0.1 * (avg_return / 5.0))
            else:
                performance_score = (0.4 * avg_win_rate) + (0.4 * avg_sharpe) + (0.2 * (avg_return / 5.0))
        ...
        if best_param_key is not None:
            ...
            self.optimized_parameters[volatility_regime]["lookback_periods"] = int(
                (1 - self.adaptation_speed * self.stability_factor) * current_params["lookback_periods"] +
                self.adaptation_speed * self.stability_factor * lookback
            )
            
            self.optimized_parameters[volatility_regime]["prediction_horizon"] = int(
                (1 - self.adaptation_speed * self.stability_factor) * current_params["prediction_horizon"] +
                self.adaptation_speed * self.stability_factor * horizon
            )
            
            self.optimized_parameters[volatility_regime]["confidence_threshold"] = (
                (1 - self.adaptation_speed * self.stability_factor) * current_params["confidence_threshold"] +
                self.adaptation_speed * self.stability_factor * threshold
            )
            
            logger.info(f"Updated parameters for {volatility_regime.name} regime: "
            ...
            logger.info(f"No optimal parameter set found for {volatility_regime.name} regime, keeping current parameters")
    ...
    def get_optimized_parameters(
        ...
        volatility_regime, market_condition, volatility_value = self.detect_market_condition(data, vix_data)
        ...
        lookback_periods = self.adapt_lookback_period(volatility_regime, market_condition)
        prediction_horizon = self.adapt_prediction_horizon(volatility_regime, market_condition)
        confidence_threshold = self.adapt_confidence_threshold(volatility_regime, market_condition)
        model_weights = self.adapt_model_weights(volatility_regime, market_condition)
        position_sizing = self.get_recommended_position_sizing(volatility_regime, market_condition, circuit_breaker_status)
        ...
        logger.info(f"Optimized parameters for {volatility_regime.name}/{market_condition.value}: "
            ...
            for param_name, param_range in parameter_ranges.items():
                ...
                for direction in [-1, 1]:
                    ...
                    if min_value is not None:
                        test_parameters[param_name] = max(min_value, test_parameters[param_name])
                    if max_value is not None:
                        test_parameters[param_name] = min(max_value, test_parameters[param_name])
                    ...
                    performance = performance_evaluator(historical_data, test_parameters)
                    ...
                    if performance > best_performance:
                        ...
                        improved = True
                        logger.info(f"Iteration {iteration+1}: Improved performance to {best_performance} "
