...
class MarketRegimeDetector:
    ...
    def extract_features(self, prices: np.ndarray) -> pd.DataFrame:
        ...
        features["trend"] = price_series.diff(self.window_size) / price_series.shift(
            self.window_size
        )
        ...
        features["volatility_change"] = (
            features["volatility"].pct_change(self.volatility_window).fillna(0)
        )
        ...
        features["mean_dev"] = (
            price_series - price_series.rolling(self.window_size).mean()
        ) / price_series.rolling(self.window_size).std()
    ...
    def _determine_regime_type(
        ...
        if abs(mean_dev) > 1.5 and (
            (mean_dev < 0 and rsi > 50) or (mean_dev > 0 and rsi < 50)
        ):
            return MarketRegime.MEAN_REVERTING

        # Check for ranging markets
        if (
            abs(trend) < self.trend_threshold * 0.5
        ...
        if (
            kurtosis_val > self.kurtosis_threshold
            or volatility > self.volatility_threshold
        ):
