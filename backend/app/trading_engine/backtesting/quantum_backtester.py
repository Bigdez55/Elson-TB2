...
class QuantumModelBacktester:
    ...
    async def prepare_data(
        ...
        data["future_return"] = (
            data[target_column].pct_change(target_shift).shift(-target_shift)
        )
    ...
    def compare_with_benchmarks(
        ...
        """
        models_to_compare = {"quantum": quantum_model}
        ...
        for name, model in models_to_compare.items():
            ...
            if (
                name != "quantum"
                or not hasattr(quantum_model, "model")
                or quantum_model.model is None
            ):
                model.fit(X_train, y_train)
    ...
    def plot_results(
        ...
        ax1.plot(
            dates, buy_hold_values[1:], label="Buy & Hold", linewidth=1.5, alpha=0.7
        )
        ax1.plot(
            dates,
            perfect_values[1:],
        ...
        # Plot positions
        ax2.fill_between(
            dates, positions[1:], 0, step="post", alpha=0.7, label="Position"
        )
    ...
    def save_results(
        self, results: Dict[str, Any], symbol: str, model_name: str
    ) -> str:
        """
        ...
        save_data = {
            ...
            if not isinstance(v, (plt.Figure, np.ndarray, pd.DatetimeIndex))
        }
    ...
    async def run_full_backtest(
        ...
        train_metrics = {
            ...
            "precision": precision_score(
                data_dict["y_train"], y_pred_train, zero_division=0
            ),
        ...
        test_metrics = {
            ...
            "precision": precision_score(
                data_dict["y_test"], y_pred_test, zero_division=0
            ),
        ...
        test_returns = (
            data_dict["data"]["future_return"].iloc[-len(data_dict["y_test"]) :].values
        )

        financial_impact = self.test_prediction_impact(
            predictions=y_pred_test, actual_returns=test_returns
        )
