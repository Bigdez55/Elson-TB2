...
class HybridModelBacktester:
    ...
    def __init__(
        ...
        self.parameter_optimizer = (
            AdaptiveParameterOptimizer() if enable_adaptive_parameters else None
        )
    ...
    def _generate_synthetic_data(
        ...
        if len(returns) < periods:
            ...
            additional = np.random.normal(
                0, volatility_map[VolatilityRegime.NORMAL], periods - len(returns)
            )
    ...
    def _simulate_simple_prediction(
        self, data: pd.DataFrame, lookback: int = 20, confidence_multiplier: float = 1.0
        ...
        for i in range(lookback, len(data)):
            ...
            if (
                ...
                and data["ma_fast"].iloc[i] > data["ma_slow"].iloc[i]
                and data["rsi"].iloc[i] < 70 * confidence_multiplier
            ):
                decisions.append(TradingDecision.BUY)
            elif (
                data["ma_fast"].iloc[i - 1] > data["ma_slow"].iloc[i - 1]
                and data["ma_fast"].iloc[i] < data["ma_slow"].iloc[i]
                and data["rsi"].iloc[i] > 30 / confidence_multiplier
            ):
    ...
    def _evaluate_performance(
        ...
        for i, trade in enumerate(trades):
            ...
            if trade["decision"] == "BUY":
                profit = (
                    next_trade["executed_price"] - trade["executed_price"]
                ) / trade["executed_price"]
                profit -= (trade["fee"] + next_trade["fee"]) / trade["executed_price"]
            else:  # SELL
                profit = (
                    trade["executed_price"] - next_trade["executed_price"]
                ) / trade["executed_price"]
                profit -= (trade["fee"] + next_trade["fee"]) / trade["executed_price"]
            ...
            regime, volatility_value = volatility_data.get(
                trade_index, (VolatilityRegime.NORMAL, 20.0)
            )
        ...
        for regime, regime_trades in trades_by_regime.items():
            ...
            avg_return = sum(t["adjusted_profit_pct"] for t in regime_trades) / len(
                regime_trades
            )
            ...
            avg_position_size = sum(t["position_size"] for t in regime_trades) / len(
                regime_trades
            )
        ...
        if not all_trades:
            ...
            overall_return = sum(t["adjusted_profit_pct"] for t in all_trades) / len(
                all_trades
            )
            ...
            position_size_adjusted_win_rate = (
                position_size_adjusted_wins / total_position_size
        ...
        if regime_performance[VolatilityRegime.EXTREME].sample_count > 0:
            extreme_vol_target = (
                regime_performance[VolatilityRegime.EXTREME].win_rate >= 0.60
            )
    ...
    def run_backtest(
        ...
        if data is None:
            try:
                ...
                logger.warning(
                    f"Could not load market data: {str(e)}. Generating synthetic data."
                )
        ...
        for i in range(len(data)):
            ...
            volatility_regime, volatility_value = (
                self.volatility_detector.detect_regime(data_window)
            )
            ...
            if self.enable_circuit_breaker:
                ...
                tripped, cb_status, base_position_size = (
            ...
            if self.enable_adaptive_parameters:
                ...
                optimized_params = self.parameter_optimizer.get_optimized_parameters(
                    ...
                    circuit_breaker_status=(
                        cb_status if self.enable_circuit_breaker else None
                    ),
                )
                ...
                self.parameter_adaptations.append(
                    {
                        "timestamp": data.index[i].isoformat(),
                        "volatility_regime": optimized_params["regime_info"][
                            "volatility_regime"
                        ],
                        "market_condition": optimized_params["regime_info"].get(
                            "market_condition", "UNKNOWN"
                        ),
                        "volatility_value": optimized_params["regime_info"][
                            "volatility_value"
                        ],
                        "lookback_periods": optimized_params["lookback_periods"],
                        "prediction_horizon": optimized_params["prediction_horizon"],
                        "confidence_threshold": optimized_params[
                ...
                confidence_multiplier = (
                    optimized_params["confidence_threshold"] / 0.6
                )  # Normalize
            ...
            if self.enable_circuit_breaker:
                ...
                allowed, _ = self.circuit_breaker.check(
                    CircuitBreakerType.VOLATILITY, "BACKTEST"
                )
                can_trade = allowed
        ...
        result = self._evaluate_performance(
            trades=executed_trades, data=data, volatility_data=volatility_data
        )
    ...
    def _generate_performance_plots(
        ...
        if self.parameter_adaptations:
            ...
            # Prepare data
            timestamps = [
                pd.to_datetime(p["timestamp"]) for p in self.parameter_adaptations
            ]
            lookback_periods = [
                p["lookback_periods"] for p in self.parameter_adaptations
            ]
            confidence_thresholds = [
                p["confidence_threshold"] for p in self.parameter_adaptations
            ]
            ...
            plt.subplot(3, 1, 2)
            plt.plot(
                timestamps, confidence_thresholds, "g-", label="Confidence Threshold"
            )
        ...
        if self.circuit_breaker_events:
            ...
            cb_timestamps = [
                pd.to_datetime(e["timestamp"]) for e in self.circuit_breaker_events
            ]
            cb_statuses = [
                e["circuit_breaker_status"] for e in self.circuit_breaker_events
            ]
        ...
        plt.axvline(
            x=10, color="red", linestyle="--", label="Robustness Target (≤10pp)"
        )
    ...
    def _save_results(self, result: BacktestResult, test_name: str) -> None:
        ...
        if self.parameter_adaptations:
            param_file = os.path.join(
                self.output_path, f"{test_name}_parameter_adaptations.json"
            )
        ...
        if self.circuit_breaker_events:
            cb_file = os.path.join(
                self.output_path, f"{test_name}_circuit_breaker_events.json"
            )
...
def run_hybrid_model_backtest(
    ...
    # Run backtest
    result = backtester.run_backtest(test_name=test_name, generate_plots=generate_plots)
