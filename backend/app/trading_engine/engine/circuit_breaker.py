...
class CircuitBreaker:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if config_path and os.path.exists(config_path):
            ...
            # Default configuration optimized for personal trading
            self.config = {
                "volatility": {
                    "market_wide": {
                        "extreme": 0.35,  # 35% extreme volatility threshold
                        "high": 0.20,  # 20% high volatility threshold
                        "normal": 0.15,  # 15% normal volatility threshold
                        "position_sizing": {
                            "extreme": 0.03,  # 3% of normal position size
                            "high": 0.10,  # 10% of normal position size
                            "normal": 0.90,  # 90% of normal position size
                    ...
                    "cool_down_minutes": {
                        "extreme": 90,  # 90 min cool-down after extreme volatility
                        "high": 45,  # 45 min cool-down after high volatility
                        "normal": 20,  # 20 min cool-down after normal volatility
                        "low": 0,  # No cool-down for low volatility
                    },
                    "hysteresis": {
                        "samples": 20,  # Number of samples to track for hysteresis
                        "threshold": 0.85,  # 85% of samples must be in new regime
                    },
                },
                "daily_loss": {
    ...
    def process_volatility(
        ...
        with self.lock:
            ...
            # Note: Asset class modifiers could be used to adjust thresholds
            # but are not currently implemented in this method
    ...
    def trip(
        ...
        with self.lock:
            ...
            if reset_after > 0:
                auto_reset = (
                    datetime.now() + timedelta(minutes=reset_after)
                ).isoformat()
    ...
    def reset(
        self, breaker_type: CircuitBreakerType, scope: Optional[str] = None
    ) -> bool:
        """
        ...
        with self.lock:
            ...
            if key in self.circuit_breakers:
                ...
                if current_status == CircuitBreakerStatus.OPEN.value:
                    self.circuit_breakers[key][
                        "status"
                    ] = CircuitBreakerStatus.RESTRICTED.value
                    self.circuit_breakers[key][
                        "reset_attempted_at"
                    ...
                    self.circuit_breakers[key][
                        "status"
                    ] = CircuitBreakerStatus.CAUTIOUS.value
                    self.circuit_breakers[key][
                        "reset_attempted_at"
                    ] = datetime.utcnow().isoformat()
                    self._save_status()
    ...
    def _is_more_restrictive(
        self, status1: CircuitBreakerStatus, status2: CircuitBreakerStatus
    ) -> bool:
    ...
    def _check_single_breaker(self, key: str) -> Tuple[bool, CircuitBreakerStatus]:
        ...
        if key in self.circuit_breakers:
            ...
            if status == CircuitBreakerStatus.OPEN:
                ...
                CircuitBreakerStatus.RESTRICTED,
                CircuitBreakerStatus.CAUTIOUS,
            ]:
    ...
    def _process_auto_resets(self) -> None:
        ...
        for key, breaker in self.circuit_breakers.items():
            if breaker["status"] != CircuitBreakerStatus.CLOSED.value and breaker.get(
                "auto_reset_at"
            ):
        ...
        for key in keys_to_update:
            breaker_data = self.circuit_breakers[key]
            breaker_type = breaker_data["type"]
            current_status = breaker_data["status"]
            scope = breaker_data.get("scope", "global")
            ...
            if key in self.circuit_breakers:
                scope = self.circuit_breakers[key].get("scope")
                logger.info(
                ...
                if breaker_type == CircuitBreakerType.VOLATILITY.value:
                    ...
                    minutes = self.config["volatility"]["cool_down_minutes"].get(
                        level, 30
                    )
                    self.circuit_breakers[key]["auto_reset_at"] = (
    ...
    def get_position_sizing(
        ...
        Enhanced to implement graduated position sizing for different
        volatility regimes and circuit breaker states.
        ...
        allowed, status = (
            self.check(breaker_type, scope)
            if breaker_type
            else (True, CircuitBreakerStatus.CLOSED)
        )
        ...
        if (
            scope
            and scope in self.volatility_history
            and self.volatility_history[scope]
        ):
        ...
        if status == CircuitBreakerStatus.OPEN:
            ...
            if current_regime == VolatilityLevel.EXTREME:
                return 0.03  # 3% for extreme volatility
            elif current_regime == VolatilityLevel.HIGH:
                return 0.10  # 10% for high volatility
            elif current_regime == VolatilityLevel.NORMAL:
                return 0.40  # 40% for normal volatility
            else:
                return 0.60  # 60% for low volatility
        elif status == CircuitBreakerStatus.CAUTIOUS:
            if current_regime == VolatilityLevel.EXTREME:
                return 0.05  # 5% for extreme volatility
            elif current_regime == VolatilityLevel.HIGH:
                return 0.15  # 15% for high volatility
            elif current_regime == VolatilityLevel.NORMAL:
                return 0.65  # 65% for normal volatility
            else:
                return 0.85  # 85% for low volatility
        else:  # CLOSED
            if current_regime == VolatilityLevel.EXTREME:
                return 0.03  # 3% even when closed during extreme volatility
            elif current_regime == VolatilityLevel.HIGH:
                return 0.10  # 10% during high volatility
            elif current_regime == VolatilityLevel.NORMAL:
                return 0.90  # 90% during normal volatility
            else:
