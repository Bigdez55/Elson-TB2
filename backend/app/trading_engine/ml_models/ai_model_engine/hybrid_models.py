        ...
        for name, model in self.models.items():
            ...
            try:
                ...
                if hasattr(model, "validation_split") and "validation_split" in kwargs:
                    model.fit(
                        X, y, validation_split=kwargs.get("validation_split", 0.2)
                    )
                else:
                    model.fit(X, y)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.voting == "hard":
            ...
            stacked_predictions = np.column_stack(
                [predictions[name] for name in self.fitted_models]
            )
            majority_votes = np.apply_along_axis(
                lambda x: np.argmax(
                    np.bincount(
                        x,
                        weights=[
                            self.weights.get(name, 1.0) for name in self.fitted_models
                        ],
                    )
                ),
                axis=1,
        ...
        for name, model in self.fitted_models.items():
            try:
                if hasattr(model, "score") and callable(model.score):
                    if isinstance(
                        model, (QuantumKernelClassifier, QuantumVariationalClassifier)
                    ):
...
class HybridMachineLearningModel:
    ...
    def _create_ensemble_model(self) -> EnsembleVotingClassifier:
        ...
        ensemble = EnsembleVotingClassifier(
            voting=voting, weights=weights, threshold=threshold
        )
        ...
        if self.ensemble_params.get("use_random_forest", True):
            ...
            ensemble.add_model(
                "random_forest", random_forest, weights.get("random_forest", 1.0)
            )
        ...
        if self.ensemble_params.get("use_gradient_boosting", True):
            ...
            ensemble.add_model(
                "gradient_boosting",
                gradient_boosting,
                weights.get("gradient_boosting", 1.0),
            )
        ...
        if self.ensemble_params.get("use_neural_network", True):
            ...
            ensemble.add_model(
                "neural_network", neural_network, weights.get("neural_network", 1.0)
            )
        ...
        if self.ensemble_params.get("use_quantum_kernel", True):
            ...
            ensemble.add_model(
                "quantum_kernel", quantum_kernel, weights.get("quantum_kernel", 1.2)
            )
        ...
        if self.ensemble_params.get("use_quantum_variational", True):
            ...
            ensemble.add_model(
                "quantum_variational",
                quantum_variational,
                weights.get("quantum_variational", 1.2),
            )
    ...
    def _engineer_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if len(data) < self.lookback_periods:
                logger.warning(
                    f"Input data has fewer rows ({len(data)}) than lookback periods ({self.lookback_periods}). "
            ...
            if data.isna().any().any():
                logger.warning(
                    "Input data contains NaN values. These will be forward-filled."
                )
                data = data.ffill()
            ...
            for window in [5, 10, 20, 50]:
                ...
                # Relative to price
                df[f"close_to_sma_{window}"] = df["close"] / df[
                    f"sma_{window}"
                ].replace(0, np.nan)
                df[f"close_to_ema_{window}"] = df["close"] / df[
                    f"ema_{window}"
                ].replace(0, np.nan)
            ...
            df["volatility_ratio_5_20"] = df["volatility_5"] / df[
                "volatility_20"
            ].replace(0, np.nan)
            df["volatility_ratio_10_50"] = df["volatility_10"] / df[
                "volatility_50"
            ].replace(0, np.nan)
            ...
            df["normal_vol_regime"] = (
                (df["annualized_volatility"] >= 0.15)
                & (df["annualized_volatility"] < 0.25)
            ).astype(int)
            df["high_vol_regime"] = (
                (df["annualized_volatility"] >= 0.25)
                & (df["annualized_volatility"] < 0.40)
            ).astype(int)
            df["extreme_vol_regime"] = (df["annualized_volatility"] >= 0.40).astype(int)

            # Volatility trend
            df["vol_trend_10d"] = (
                df["volatility_10"] - df["volatility_10"].shift(10)
            ) / df["volatility_10"].shift(10).replace(0, np.nan)
            ...
            # Price patterns
            df["body_size"] = abs(df["close"] - df["open"]) / df["open"].replace(
                0, np.nan
            )
            ...
            # Trend features
            df["trend_5_1"] = (df["close"] - df["close"].shift(5)) / df["close"].shift(
                5
            ).replace(0, np.nan)
            df["trend_10_5"] = (df["close"] - df["close"].shift(10)) / df[
                "close"
            ].shift(10).replace(0, np.nan)
            df["trend_20_10"] = (df["close"] - df["close"].shift(20)) / df[
                "close"
            ].shift(20).replace(0, np.nan)

            # Calculate market regime features
            # Trend strength based on SMA relationships
            df["bull_market"] = (
                (df["close"] > df["sma_50"]) & (df["sma_20"] > df["sma_50"])
            ).astype(int)
            df["bear_market"] = (
                (df["close"] < df["sma_50"]) & (df["sma_20"] < df["sma_50"])
            ).astype(int)
            ...
            try:
                ...
                df["bb_relative"] = (df["close"] - df["bb_lower"]) / bb_range.replace(
                    0, np.nan
                )
            ...
            if df.isna().any().any():
                logger.warning(
                    "Generated features contain NaN values. These will be forward-filled then backfilled."
                )
                df = df.ffill().bfill()
                ...
                if df.isna().any().any():
                    logger.warning(
                        "NaN values persist after forward/back filling. Replacing with zeros."
                    )
            ...
            if "df" in locals() and isinstance(df, pd.DataFrame) and len(df) > 0:
                ...
                df["sma_20"] = (
                    df["close"]
                    .rolling(window=min(20, len(df)))
                    .mean()
                    .fillna(df["close"])
                )
                df["volatility_20"] = (
                    df["returns"].rolling(window=min(20, len(df))).std().fillna(0)
                )
                self.feature_names = ["returns", "sma_20", "volatility_20"]
                return df[
                    ["open", "high", "low", "close", "volume"] + self.feature_names
                ]
            else:
    ...
    def _prepare_data(self, data: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            if len(df) < max(self.prediction_horizon * 2, 30):
                logger.warning(
                    f"Limited data ({len(df)} rows) available after feature engineering. "
            ...
            try:
                ...
                missing_features = [
                    f for f in self.feature_names if f not in df.columns
                ]
                ...
                logger.warning(
                    f"Proceeding with limited feature set: {available_features}"
                )
            ...
            try:
                ...
                if future_returns.isna().any():
                    logger.warning(
                        f"Data contains {future_returns.isna().sum()} NaN values in target calculation. "
                ...
                if "annualized_volatility" in df.columns:
                    ...
                    if high_vol_mask.any():
                        ...
                        logger.info(
                            f"Detected {high_vol_mask.sum()} high volatility data points"
                        )
                        ...
                        if "low_vol_regime" in df.columns:
                            self.volatility_regime_stats = {
                                ...
                                "normal": (
                                    df["normal_vol_regime"].mean()
                                    if "normal_vol_regime" in df.columns
                                    else 0
                                ),
                                "high": (
                                    df["high_vol_regime"].mean()
                                    if "high_vol_regime" in df.columns
                                    else 0
                                ),
                                "extreme": (
                                    df["extreme_vol_regime"].mean()
                                    if "extreme_vol_regime" in df.columns
                                    else 0
                                ),
                            }
                ...
                if len(df) > self.prediction_horizon:
                    ...
                    raise ValueError(
                        f"Insufficient data for target calculation: {str(e)}"
                    )
            ...
            if not valid_indices.all():
                logger.warning(
                    f"Removing {(~valid_indices).sum()} rows with NaN targets"
                )
                X = X[valid_indices]
                y = y[valid_indices]
            ...
            if len(X) != len(y):
                raise ValueError(
                    f"Feature and target length mismatch: X={len(X)}, y={len(y)}"
                )

            # Check for NaN/inf in features
            if np.isnan(X).any() or np.isinf(X).any():
                logger.warning(
                    "NaN or Inf values detected in features. Replacing with zeros."
                )
    ...
    def fit(self, data: pd.DataFrame) -> "HybridMachineLearningModel":
        ...
        try:
            logger.info(
                f"Fitting hybrid model with lookback={self.lookback_periods}, horizon={self.prediction_horizon}"
            )
            ...
            if len(X) < 50:
                logger.warning(
                    f"Training with very limited data ({len(X)} samples). Model may not perform well."
                )

            # Track class balance for potential adjustment
            class_balance = np.mean(y)
            logger.info(
                f"Class balance: {class_balance:.2f} (positive class proportion)"
            )
            ...
            if (
                hasattr(self, "high_volatility_present")
                and self.high_volatility_present
            ):
                ...
                if "gradient_boosting" in self.model.weights:
                    self.model.weights[
                        "gradient_boosting"
                    ] *= 1.2  # GBM often handles noise well

                if "random_forest" in self.model.weights:
                    self.model.weights[
                        "random_forest"
                    ] *= 1.1  # RF is stable with noise

                if "neural_network" in self.model.weights:
                    self.model.weights[
                        "neural_network"
                    ] *= 0.8  # NN can be more sensitive to noise
                ...
                if self.model.threshold < 0.6:
                    ...
                    logger.info(
                        f"Adjusted decision threshold from {original_threshold} to {self.model.threshold} for high volatility"
                    )
            ...
            try:
                ...
                logger.info(
                    f"Validation metrics: accuracy={val_metrics.get('accuracy', 'N/A'):.4f}, "
                ...
                if win_rate is not None:
                    if win_rate >= 0.7:
                        logger.info(
                            f"Model achieved win rate of {win_rate:.2%}, meeting 70% target"
                        )
                        self.target_achieved = True
                    else:
                        logger.warning(
                            f"Model win rate of {win_rate:.2%} does not meet 70% target"
                        )
            ...
            try:
                for name, model in self.model.fitted_models.items():
                    if name == "random_forest" and hasattr(
                        ...
                        and hasattr(model.encoder, "analyze_feature_importance")
                    ):
                        self.feature_importance[name] = (
                            model.encoder.analyze_feature_importance()
                        )
    ...
    def predict(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if "annualized_volatility" in df.columns:
                ...
                logger.info(
                    f"Current market volatility: {current_volatility:.2%} ({volatility_regime} regime)"
                )

                # Apply circuit breaker for extreme volatility (known limitation)
                if volatility_regime == "extreme" and not self.ensemble_params.get(
                    "bypass_circuit_breaker", False
                ):
                    logger.warning(
                        f"CIRCUIT BREAKER: Extreme volatility detected ({current_volatility:.2%}). "
            ...
            try:
                ...
                logger.warning(
                    f"Proceeding with limited feature set: {len(available_features)} features"
                )
                X = df[available_features].values

                # Feature mismatch warning
                logger.warning(
                    "Feature mismatch between training and prediction data. Results may be unreliable."
                )
            ...
            confidence_threshold = self.ensemble_params.get(
            ...
            if volatility_regime == "high":
                ...
                logger.info(
                    f"Adjusted confidence threshold to {confidence_threshold} for high volatility"
                )
            ...
            if np.any(low_confidence_mask):
                logger.warning(
                    f"{np.sum(low_confidence_mask)} out of {len(confidence)} predictions have "
            ...
            for i in range(len(y_pred)):
                ...
                confidence_level = (
                    "high" if conf >= 0.75 else "medium" if conf >= 0.6 else "low"
                )
    ...
    def backtest(
        self, data: pd.DataFrame, initial_capital: float = 10000.0
    ) -> Dict[str, Any]:
        """
        ...
        backtest_df["strategy_return"] = (
            backtest_df["position"] * backtest_df["future_return"]
        )
        ...
        backtest_df["cumulative_market_return"] = (
            1 + backtest_df["future_return"]
        ).cumprod() - 1
        ...
        backtest_df["market_value"] = initial_capital * (
            1 + backtest_df["cumulative_market_return"]
        )
        backtest_df["strategy_value"] = initial_capital * (
            1 + backtest_df["cumulative_strategy_return"]
        )
        ...
        sharpe_ratio = (
            (annualized_strategy_return - 0.02) / strategy_volatility
        ...
        backtest_df["drawdown"] = (
            backtest_df["strategy_value"] - backtest_df["strategy_peak"]
        ) / backtest_df["strategy_peak"]
        ...
        for threshold in [0.5, 0.6, 0.7, 0.8, 0.9]:
            ...
            if len(high_conf_trades) > 0:
                high_conf_win_rate = high_conf_trades[
                    high_conf_trades["strategy_return"] > 0
                ].shape[0] / len(high_conf_trades)
