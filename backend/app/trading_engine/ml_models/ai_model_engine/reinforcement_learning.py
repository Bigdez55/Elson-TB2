...
class TradingEnvironment(gym.Env):
    ...
    def reset(self):
        ...
        for i in range(self.window_size):
            observation = self._get_observation(
                self._starting_index - self.window_size + i
            )
    ...
    def step(self, action):
        ...
        current_data = self._get_market_data_slice(
            self.current_step + self._starting_index
        )
        ...
        self._done = (
            ...
            or total_value <= 0.5 * self.initial_balance
        )  # Stop if 50% loss
    ...
    def _execute_strategy_with_data(
    ...
    def _apply_signals(self, signals: Dict) -> Tuple[float, float]:
        ...
        if action == "buy":
            ...
            if self.current_position > 0:
                ...
                trade_pnl = position_to_sell * (
                    price - (self.position_value / self.current_position)
                )
    ...
    def _get_observation(self, index: int) -> np.ndarray:
        ...
        all_features = np.concatenate(
            [market_features, regime_features, strategy_metrics]
        )
    ...
    def _get_state(self) -> np.ndarray:
        ...
        if len(self._observation_history) < self.window_size:
            # Pad with zeros
            padding = np.zeros(
                (self.window_size - len(self._observation_history), state.shape[1]),
                dtype=np.float32,
            )
...
class StrategySelector:
    ...
    async def train(self, market_data: pd.DataFrame):
        ...
        for episode in range(self.training_episodes):
            ...
            # Log progress
            logger.info(
                f"Episode {episode+1}/{self.training_episodes} - "
            ...
            if val_return > best_return:
                ...
                # Save best model
                self.save_model(
                    f"{self.model_path}_best"
                    if self.model_path
                    else "models/strategy_selector_best"
                )
                early_stop_counter = 0
            else:
        ...
        # Save final model
        self.save_model(
            self.model_path if self.model_path else "models/strategy_selector"
        )
    ...
    async def select_strategy(self, market_data: pd.DataFrame) -> Tuple[int, Dict]:
        ...
        if self.agent is None:
            ...
            logger.warning(
                "No trained agent available, using baseline strategy selector"
            )
        ...
        if len(self.observation_window) < self.window_size:
            logger.warning(
                f"Observation window not full ({len(self.observation_window)}/{self.window_size}), using baseline"
            )
            return self._baseline_strategy_selection(market_data)
        ...
        selection_info = {
            ...
            "strategy_name": (
                self.strategies[strategy_idx].name
                if strategy_idx < len(self.strategies)
                else "Hold"
            ),
        ...
        logger.info(
            f"Selected strategy: {selection_info['strategy_name']} with confidence {selection_info['confidence']:.4f}"
        )

        return strategy_idx, selection_info

    def _baseline_strategy_selection(
        ...
        selection_info = {
            ...
            "strategy_name": (
                self.strategies[strategy_idx].name
                if hasattr(self.strategies[strategy_idx], "name")
                else f"Strategy {strategy_idx}"
            ),
