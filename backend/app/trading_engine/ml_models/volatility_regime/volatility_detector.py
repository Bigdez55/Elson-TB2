...
class VolatilityDetector:
    ...
    def calculate_realized_volatility(self, returns):
        ...
        if len(returns) < self.lookback_periods:
            logger.warning(
                f"Insufficient data for volatility calculation. Using available {len(returns)} periods."
            )
    ...
    def detect_regime(self, data, vix_data=None):
        ...
        if vix_data is not None and "close" in vix_data.columns:
            try:
                ...
                blended_volatility = (
                    1 - self.vix_weight
                ) * realized_volatility + self.vix_weight * vix_value
            except Exception as e:
                logger.warning(
                    f"Error incorporating VIX data: {str(e)}. Using realized volatility only."
                )
