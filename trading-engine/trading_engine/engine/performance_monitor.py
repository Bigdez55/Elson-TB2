...
class PerformanceMonitor:
    ...
    async def get_performance_metrics(self, user_id: int, timeframe: str = 'daily') -> Dict:
    ...
    def _update_portfolio_metrics(self, portfolio: Portfolio, trade: Trade, profit_loss: Decimal):
        ...
        if trade.side == 'BUY':
            ...
            old_value = Decimal(str(position['quantity'])) * Decimal(str(position['average_price']))
            new_value = trade.quantity * trade.price
            new_quantity = Decimal(str(position['quantity'])) + trade.quantity
            
            if new_quantity > 0:
                position['average_price'] = float((old_value + new_value) / new_quantity)
            position['quantity'] = float(new_quantity)
        else:
            # Update realized P/L for sells
            position['realized_pl'] = float(Decimal(str(position['realized_pl'])) + profit_loss)
            position['quantity'] = float(Decimal(str(position['quantity'])) - trade.quantity)
    ...
    async def _calculate_portfolio_metrics(self, portfolio: Portfolio) -> Dict:
        ...
        if len(daily_returns) > 1:
            ...
            if returns_std > 0:
                metrics['sharpe_ratio'] = (np.mean(daily_returns) - settings.RISK_FREE_RATE) / returns_std
    ...
    async def _calculate_daily_metrics(self, portfolio: Portfolio) -> Dict:
        ...
        return await self._calculate_metrics_for_timeframe(portfolio, start_date, end_date)
