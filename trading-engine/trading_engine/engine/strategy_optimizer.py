   ...
   def _run_backtest(self, strategy: MovingAverageStrategy, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            for i in range(len(signals) - 1):
                ...
                if signal == 1 and position <= 0:  # Buy signal
                    position = portfolio_value * 0.95 / current_price  # Use 95% of portfolio
                    trade = {
    ...
    def _calculate_backtest_metrics(
        ...
        try:
            ...
            sharpe_ratio = np.mean(excess_returns) / daily_std * np.sqrt(252) if daily_std > 0 else -np.inf
            ...
            return {
                ...
                'avg_return_per_trade': float(total_return / total_trades) if total_trades > 0 else 0,
                'metrics_version': '1.0'
            }
    ...
    def _calculate_profit_factor(self, trades: List[Dict]) -> float:
        ...
        profits = sum(t['portfolio_value'] - t.get('previous_value', 0) 
                     for t in trades if t['type'] == 'SELL' 
                     and t['portfolio_value'] > t.get('previous_value', 0))
        losses = abs(sum(t['portfolio_value'] - t.get('previous_value', 0)
                        for t in trades if t['type'] == 'SELL'
                        and t['portfolio_value'] < t.get('previous_value', 0)))
