...
from .change_point_detector import detect_regime_changes, define_market_regimes
...
class TimeframeEstimator:
    ...
    def __init__(
        self,
        timeframes: List[str] = ['1d', '3d', '1w', '2w', '1m']
    ):
        """
    ...
    def extract_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        regime_df = define_market_regimes(df, change_points, column='close', n_regimes=4)
    ...
    def prepare_features(self, df: pd.DataFrame) -> np.ndarray:
        ...
        features = [
            ...
            'volume_ma_ratio', 'trend_5_20', 'trend_5_50', 'trend_20_50',
            'daily_range_ma5'
        ]
    ...
    def fit(
        ...
        if extract_features:
            if not isinstance(X_data, pd.DataFrame):
                raise ValueError("X_data must be a DataFrame when extract_features=True")
...
class EventBasedTimeframeEstimator:
    ...
    def recommend_timeframe(
        ...
        if method == 'weighted_avg':
            ...
            timeframe_key = self.default_event_impact.get(event_type, {}).get(impact, 'medium')
...
def detect_market_events(
    ...
    trend_changes = []
    ma_cross = (price_data['ma_20'] > price_data['ma_50']) != (price_data['ma_20'].shift(1) > price_data['ma_50'].shift(1))
    ...
    for day in trend_change_days:
        ...
        if idx + 5 < len(price_data):
            change = abs(price_data['close'].iloc[idx+5] / price_data['close'].iloc[idx] - 1)
            ...
            trend_changes.append({
                ...
                'impact': impact,
                'details': {
                    'direction': 'up' if price_data['ma_20'].iloc[idx] > price_data['ma_50'].iloc[idx] else 'down',
                    'change': change
                }
            })
    ...
    if news_data is not None and 'sentiment' in news_data.columns and 'date' in news_data.columns:
        news_events = []
        ...
        for date, group in news_by_date:
            ...
            if 'score' in group.columns:
                ...
                if abs(avg_sentiment) > sentiment_threshold:
                    impact = 'high' if abs(avg_sentiment) > sentiment_threshold * 1.3 else 'medium'
