...
class WindowBasedDetector(ChangePointDetector):
    ...
    def detect(
        ...
        if len(data) < 2 * self.window_size:
            logger.warning(f"Data length ({len(data)}) is less than 2 * window_size ({2 * self.window_size})")
            return []
        ...
        for t in range(1, n + 1):
            ...
            for r in range(t):
                if run_length[r, t-1] > 0:
                    ...
                    # Update run length
                    growth_probs[r+1] = run_length[r, t-1] * (1 - self.hazard_rate) * likelihood
            
            # Change point probability
            cp_prob = np.sum(run_length[:t, t-1]) * self.hazard_rate * norm.pdf(data[t-1], 0, np.sqrt(prior_var))
    ...
    **kwargs
    ...
    if plot:
        fig = detector.plot_change_points(
            values, 
            change_points,
            title=f"Change Point Detection ({detector_type})"
        )
        result['figure'] = fig
...
def define_market_regimes(
    ...
    for i in range(int(np.max(segment)) + 1):
        ...
        if len(segment_data) > 0:
            ...
            trend = (segment_data[column].iloc[-1] / segment_data[column].iloc[0] - 1) if len(segment_data) > 1 else 0
    ...
    for regime in range(n_regimes):
        ...
        if len(regime_data) > 0:
            ...
            trend = (regime_data[column].iloc[-1] / regime_data[column].iloc[0] - 1) if len(regime_data) > 1 else 0
    ...
    result_df['regime_type'] = result_df['regime'].map(lambda x: regime_stats.get(x, {}).get('type', 'unknown'))
