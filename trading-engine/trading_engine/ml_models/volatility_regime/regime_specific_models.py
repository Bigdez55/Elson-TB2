...
from ..ai_model_engine.quantum_models import QuantumKernelClassifier, QuantumVariationalClassifier
from ..ai_model_engine.hybrid_models import EnsembleVotingClassifier, HybridMachineLearningModel
...
class RegimeSpecificModelSelector:
    ...
    def initialize_models(self, base_config: Dict[str, Any] = None):
        ...
        logger.info(f"Initialized regime-specific models for {len(self.models)} volatility regimes")
    ...
    def train_regime_specific_models(self, historical_data: pd.DataFrame):
        ...
        if 'volatility_regime' not in historical_data.columns:
            logger.error("Cannot train regime-specific models: 'volatility_regime' column missing")
            return
        ...
        for regime_str, regime_enum in regime_mapping.items():
            regime_df = historical_data[historical_data['volatility_regime'] == regime_str]
            ...
            logger.info(f"Extracted {len(regime_df)} samples for {regime_str} volatility regime")
            
        # Train specialized models for each regime
        for regime, data in regime_data.items():
            if len(data) < 100:  # Minimum data requirement
                logger.warning(f"Insufficient data for {regime.name} regime: {len(data)} samples")
                continue
            ...
            try:
                ...
                logger.info(f"Successfully trained specialized model for {regime.name} volatility regime")
                
            except Exception as e:
                logger.error(f"Error training model for {regime.name} regime: {str(e)}")
    
    def _train_specialized_model(self, 
                               regime: VolatilityRegime, 
                               data: pd.DataFrame,
                               base_model: HybridMachineLearningModel) -> HybridMachineLearningModel:
        """
        ...
        if 'return' in data.columns and 'label' in data.columns:
            ...
            features = data.drop(['return', 'label', 'volatility_regime'], axis=1, errors='ignore')
            target = data['label']
            
            # Train the specialized model on this regime's data
            specialized_model.train(features, target)
            
            logger.info(f"Trained {regime.name} model on {len(data)} samples with "
                      f"{len(features.columns)} features")
        else:
            logger.warning(f"Cannot train model for {regime.name}: missing required columns")
    ...
    def update_regime_performance(self, 
        ...
        if len(new_predictions) != len(actual_outcomes):
            logger.warning(f"Cannot update performance: prediction and outcome counts don't match")
            return
        ...
        if current_samples + total > 0:
            new_win_rate = ((current_win_rate * current_samples) + (correct / total * total)) / (current_samples + total)
        else:
            new_win_rate = 0.0
        ...
        logger.info(f"Updated {regime.name} performance: Win rate {new_win_rate:.2%} ({current_samples + total} samples)")
    
    def _get_regime_config(self, regime: VolatilityRegime, base_config: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _create_regime_model(self, regime: VolatilityRegime, config: Dict[str, Any]) -> HybridMachineLearningModel:
        """
        ...
        logger.info(f"Created model for {regime.name} volatility regime with "
                   f"lookback={config.get('lookback_periods')}, "
                   f"horizon={config.get('prediction_horizon')}")
        
        return model
    
    def get_model_for_regime(self, regime: VolatilityRegime) -> HybridMachineLearningModel:
        """
        ...
        if self.enable_smooth_transitions:
            ...
            regime_changed = (self.current_regime != regime and self.current_regime is not None)
            ...
            if regime_changed and self.transition_state['current_stable_count'] >= self.transition_state['stabilize_steps']:
                ...
                logger.info(f"Detected potential change to {regime.name} regime, but waiting for stabilization "
        ...
        if temp_regime in self.trained_models:
            logger.info(f"Using trained specialized model for {temp_regime.name} volatility regime")
        ...
        if temp_regime in self.models:
            logger.info(f"Using configuration-based model for {temp_regime.name} volatility regime (no trained model available)")
            return self.models[temp_regime]
        
        # Fall back to default model if not found
        logger.warning(f"No model available for {temp_regime.name} regime, using default model")
        return self.default_model
        
    def _handle_regime_transition(self, 
                                 from_regime: VolatilityRegime, 
                                 to_regime: VolatilityRegime) -> HybridMachineLearningModel:
        """
        ...
        if not self.transition_state['in_transition']:
            ...
            logger.info(f"Starting transition from {from_regime.name} to {to_regime.name} regime")
        ...
        # Continue existing transition
            ...
            if to_regime != self.transition_state['to_regime']:
                ...
                previous_to_model = self.trained_models.get(previous_to_regime, self.models.get(previous_to_regime))
            ...
            self.transition_state['transition_progress'] += self.transition_state['transition_speed']
            ...
            if round(blend_factor * 10) % 2 == 0:  # Log every 20% progress to avoid too much logging
                logger.info(f"Regime transition at {blend_factor*100:.1f}% from {from_regime.name} to {to_regime.name}")
            ...
            current_from_model = self.trained_models.get(current_from_regime, self.models.get(current_from_regime))
    ...
    def get_current_regime_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "position_sizing": current_model.ensemble_params.get("position_sizing_factor", 1.0)
        }
    ...
    def create_blended_model(self, 
                           model1: Any, 
                           model2: Any,
                           blend_ratio: float = 0.5,
                           blend_factor: float = None) -> Any:
        """
        ...
        if hasattr(model1, 'lookback_periods') and hasattr(model2, 'lookback_periods'):
            blended_model.lookback_periods = int(model1.lookback_periods * (1-blend_ratio) + 
                                              model2.lookback_periods * blend_ratio)
        
        if hasattr(model1, 'prediction_horizon') and hasattr(model2, 'prediction_horizon'):
            blended_model.prediction_horizon = int(model1.prediction_horizon * (1-blend_ratio) + 
        ...
        if hasattr(model1, 'ensemble_params') and hasattr(model2, 'ensemble_params'):
            ...
            blended_threshold = threshold1 * (1-blend_ratio) + threshold2 * blend_ratio
            ...
            for model_type in all_models:
                ...
                blended_weights[model_type] = weight1 * (1-blend_ratio) + weight2 * blend_ratio
        ...
        logger.info(
            f"Created enhanced blended model with ratio {blend_ratio:.2f}"
        )
