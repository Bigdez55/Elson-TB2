...
class ClassificationEnsemble(ModelCombiner):
    ...
    def __init__(
        ...
        name: str = "classification_ensemble"
    ):
        """
    ...
    def combine(
        ...
        if self.method == "weighted_voting":
            ...
            if len(probabilities) != len(self.models):
                raise ValueError(f"Number of probability arrays ({len(probabilities)}) "
                                f"doesn't match number of models ({len(self.models)})")
...
class RegressionEnsemble(ModelCombiner):
    ...
    def combine(
        ...
        if self.method == "weighted_average":
            ...
            # Handle confidence intervals if provided
            if confidence_intervals is not None:
                lower = np.median(np.array([low for low, _ in confidence_intervals]), axis=0)
                upper = np.median(np.array([high for _, high in confidence_intervals]), axis=0)
            ...
            if confidence_intervals is not None:
                logger.warning("Confidence intervals are not supported with stacking method")
...
class DynamicWeightEnsemble(ModelCombiner):
    ...
    def update_weights(
        ...
        if performance_metric is None:
            if self.is_classification:
                ...
                performance_metric = lambda y_true, y_pred: -np.mean((y_true - y_pred)**2)
    ...
    def combine(
        ...
        weights = self.dynamic_weights if use_dynamic_weights and len(self.dynamic_weights) == len(self.models) else self.weights
...
class RangePredictionEnsemble(ModelCombiner):
    ...
    def __init__(
        self, 
        method: str = "weighted_average",
        name: str = "range_prediction_ensemble"
    ):
        """
    ...
    def combine(
        ...
        has_intervals = len(lower_preds) == len(self.models) and len(upper_preds) == len(self.models)
        ...
        if self.method == "weighted_average":
            ...
            mean_stacked = np.concatenate([np.repeat(pred.reshape(-1, 1), count, axis=1) 
                                        for pred, count in zip(mean_preds, rep_counts)], axis=1)
            ...
            if has_intervals:
                lower_stacked = np.concatenate([np.repeat(pred.reshape(-1, 1), count, axis=1) 
                                            for pred, count in zip(lower_preds, rep_counts)], axis=1)
                
                upper_stacked = np.concatenate([np.repeat(pred.reshape(-1, 1), count, axis=1) 
                                            for pred, count in zip(upper_preds, rep_counts)], axis=1)
