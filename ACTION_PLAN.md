...
|--------|---------|-------|
| `scripts/deploy-vllm-dora.sh` | Deploy vLLM with DoRA/LoRA adapters | l4, spot, 2xt4, quantized |
...
```bash
# L4 with DoRA adapter (recommended)
./scripts/deploy-vllm-dora.sh l4 dora

# Spot VM with QDoRA (cost-effective)
./scripts/deploy-vllm-dora.sh spot qdora

# 2x T4 with base model
