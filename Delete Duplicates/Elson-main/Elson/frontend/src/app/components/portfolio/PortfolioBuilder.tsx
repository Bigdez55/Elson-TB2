...
const PortfolioBuilder: React.FC = () => {
  ...
  const calculateEtfAllocations = () => {
    ...
    Object.keys(allocation).forEach(assetClass => {
      ...
      if (assetAmount > 0 && SUGGESTED_ETFS[assetClass]) {
        // Use first ETF for each asset class as default
        const etf = SUGGESTED_ETFS[assetClass][0];
              ...
              {activeAssetClass && (
                ...
                >
                  <Typography variant="subtitle2" color="primary" gutterBottom>
                    {ASSET_CLASS_EDUCATION[activeAssetClass].title}
                  </Typography>
                  <Typography variant="body2" paragraph>
                    {ASSET_CLASS_EDUCATION[activeAssetClass].description}
                  </Typography>
                  <Grid container spacing={2}>
                    <Grid item xs={6}>
                      ...
                      <Typography variant="body2">
                        {ASSET_CLASS_EDUCATION[activeAssetClass].riskLevel}
                      </Typography>
                    </Grid>
                    <Grid item xs={6}>
                      <Typography variant="caption" color="text.secondary">
                      ...
                      <Typography variant="body2">
                        {ASSET_CLASS_EDUCATION[activeAssetClass].timeHorizon}
                      </Typography>
                    </Grid>
                  </Grid>
                  <Box sx={{ mt: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.5 }}>
                      {ASSET_CLASS_EDUCATION[activeAssetClass].examples.map((example, index) => (
                        ...
                        <Grid item xs={12} key={asset.id}>
                          <Paper variant="outlined" sx={{ p: 1.5 }}>
                            ...
                            <List dense disablePadding>
                              {SUGGESTED_ETFS[asset.id].map((etf, index) => (
                                <ListItem 
                                  key={etf.symbol}
                                  selected={index === 0}
                                  sx={{ 
