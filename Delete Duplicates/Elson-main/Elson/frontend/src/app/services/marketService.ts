...
const marketService = {
   ...
   */
  async getQuote(symbol: string, forceRefresh: boolean = false): Promise<StockQuote> {
    try {
   ...
   */
  async getBatchQuotes(symbols: string[], forceRefresh: boolean = false): Promise<Record<string, StockQuote>> {
    try {
   ...
   */
  async getMarketHours(market: string = 'US'): Promise<{ is_open: boolean; next_open?: string; next_close?: string }> {
    try {
