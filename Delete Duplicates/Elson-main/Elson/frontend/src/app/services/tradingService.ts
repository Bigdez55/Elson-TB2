...
export const tradingService = {
  ...
  async getOrders(params?: {
    ...
    limit?: number;
    offset?: number;
  }, forceRefresh: boolean = false): Promise<{ orders: Order[]; total: number }> {
    try {
  ...
  async getTrades(params?: {
    ...
    limit?: number;
    offset?: number;
  }, forceRefresh: boolean = false): Promise<{ trades: Trade[]; total: number }> {
    try {
      // Create parameters object for cache key generation
      const cacheParams: Record<string, any> = {};
      if (params) {
  ...
  // Position operations
  async getPositions(forceRefresh: boolean = false): Promise<Position[]> {
    try {
  ...
  async getPortfolio(forceRefresh: boolean = false): Promise<{
    balance: number;
    positions: Position[];
    totalValue: number;
    dailyPnL: number;
  ...
  // Performance metrics
  async getPerformanceMetrics(timeframe: string = '24h', forceRefresh: boolean = false): Promise<{
    winRate: number;
    profitFactor: number;
    totalTrades: number;
    averageWin: number;
  ...
  // Risk management
  async getRiskMetrics(symbol: string, forceRefresh: boolean = false): Promise<{
    maxDrawdown: number;
    sharpeRatio: number;
    volatility: number;
  ...
  async getStrategyPerformance(strategyId: string, forceRefresh: boolean = false): Promise<{
    returns: number;
    trades: number;
    winRate: number;
    profitFactor: number;
  ...
  async getRecurringInvestments(forceRefresh: boolean = false): Promise<RecurringInvestment[]> {
    try {
  ...
  async getRecurringInvestmentById(id: string | number, forceRefresh: boolean = false): Promise<RecurringInvestment> {
    try {
  ...
  async getRoundupSummary(forceRefresh: boolean = false): Promise<RoundupSummary> {
    try {
