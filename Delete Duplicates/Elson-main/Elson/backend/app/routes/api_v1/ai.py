...
def get_ai_portfolio_manager(
    ...
    return AIPortfolioManager(db, market_integration.market_data, neural_network, redis_client)
...
async def get_recommendations(
    ...
    recommendations = await advisor_service.get_recommendations_for_user(current_user.id, limit=limit)
...
async def execute_recommendation(
    ...
    trade, error = await advisor_service.create_trade_from_recommendation(current_user.id, recommendation)
    ...
    if current_user.role == UserRole.MINOR and trade.status == TradeStatus.PENDING_APPROVAL:
        notification_service = NotificationService(db)
...
async def schedule_recurring_optimization(
    ...
    if not success:
        raise HTTPException(
            status_code=400,
            detail="Failed to schedule recurring optimization"
        )
