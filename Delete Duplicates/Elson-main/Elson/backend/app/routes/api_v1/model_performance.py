...
PERFORMANCE_DATA_PATH = os.path.join(os.path.dirname(__file__), "../../../../performance_history.json")
...
async def get_current_volatility(
    ...
    lookback_days: int = Query(30, description="Days to look back for volatility calculation"),
...
async def get_performance_by_regime(
    ...
    try:
        ...
        mock_performance = {
            ...
            "by_regime": {
                regime: {
                    "win_rate": round(0.72 + (i * 0.02), 2) if i < 2 else round(0.72 - (i * 0.02), 2),
                    "avg_return": round(1.5 + (i * 0.2), 2) if i < 2 else round(1.5 - (i * 0.05), 2),
                    "sharpe_ratio": round(1.8 + (i * 0.1), 2) if i < 2 else round(1.8 - (i * 0.1), 2),
...
async def get_parameter_adaptations(
    ...
    try:
            ...
            "time_series": [
                {
                    ...
                    "volatility_regime": ["LOW", "NORMAL", "HIGH", "EXTREME"][(i // 7) % 4],
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching parameter data: {str(e)}")
