...
class StripeService:
    ...
    @staticmethod
    def create_customer(user_id: int, email: str, name: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def update_subscription(
        ...
        try:
            ...
            if price_id:
                update_params["items"] = [{"id": stripe.Subscription.retrieve(subscription_id)["items"]["data"][0]["id"], "price": price_id}]
    ...
    @staticmethod
    def get_payment_methods(customer_id: str, type: str = "card") -> List[Dict[str, Any]]:
        """
    ...
    def process_invoice_paid(
        ...
        try:
            ...
            if invoice.get("lines", {}).get("data") and len(invoice["lines"]["data"]) > 0:
    ...
    def process_payment_failed(
        ...
        try:
            ...
            if retry_count >= max_retries:
                logger.warning(f"Payment failed after {retry_count} retries for subscription {db_subscription.id}")
                db_subscription.is_active = False
                
                # We don't set auto_renew to False to allow reactivation if customer updates payment method
                
                # Log the next steps
                logger.info(f"Subscription {db_subscription.id} marked inactive due to payment failures")
            else:
                logger.info(f"Payment failed (attempt {retry_count+1}/{max_retries+1}) for subscription {db_subscription.id}")
                
            db.commit()
            
            # Return success status with attempt information
            return True, f"Failed payment recorded (attempt {retry_count+1}/{max_retries+1})"
            ...
            for subscription in subscriptions:
                ...
                if card_details:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to encrypt updated payment details: {str(enc_err)}")
            
            db.commit()
            return True, f"Updated payment method for {len(subscriptions)} subscriptions"
        ...
        try:
            ...
            for subscription in db_subscriptions:
                if bank_account["status"] == "verified":
                    ...
                    logger.info(f"Bank account verified for subscription {subscription.id}")
                elif bank_account["status"] == "verification_failed":
                    # Mark subscription as inactive if verification failed
                    subscription.is_active = False
                    logger.warning(f"Bank account verification failed for subscription {subscription.id}")
                ...
                try:
                    ...
                    logger.error(f"Failed to encrypt bank account details: {str(enc_err)}")
            
            db.commit()
            return True, f"Processed bank account verification for {len(db_subscriptions)} subscriptions"
    ...
    def process_paypal_webhook(
        ...
        try:
            ...
            db_subscription = db.query(Subscription).filter(Subscription.id == int(subscription_id)).first()
            ...
            try:
                ...
                logger.error(f"Failed to encrypt PayPal payment details: {str(enc_err)}")
            ...
            if session.get("subscription"):
                ...
                try:
                    ...
                    logger.error(f"Failed to retrieve Stripe subscription details: {str(stripe_err)}")
            ...
            db.commit()
            logger.info(f"PayPal subscription successfully processed for user {db_subscription.user_id}, " +
