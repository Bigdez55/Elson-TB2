...
class SchwabBroker(BaseBroker):
    ...
    def __init__(self, db=None, api_key: Optional[str] = None, 
        ...
        self.api_secret = api_secret or get_secret("SCHWAB_SECRET")
        ...
        self.config = schwab_config.get_sandbox_config() if sandbox else schwab_config.get_production_config()
    ...
    def _refresh_auth_token(self) -> None:
        ...
        try:
            ...
            error_message = f"Unexpected error during authentication token refresh: {str(e)}"
    ...
    def _handle_error_response(self, response: requests.Response) -> None:
        ...
        try:
            ...
            error_message = error_data.get("message", error_data.get("errorMessage", "Unknown error"))
            error_code = error_data.get("error_code", error_data.get("errorCode", str(response.status_code)))
            ...
            logger.error(f"Schwab API Error (non-JSON): {response.status_code} - {error_message[:200]}")
            metrics.increment("broker.schwab.error", tags={"error_code": error_code})
            
            raise BrokerError(
                message=f"Schwab API Error: {error_message}",
                error_code=error_code
            )
    ...
    def _api_request(self, method: str, endpoint: str, 
        ...
        try:
            ...
            if self.sandbox and settings.LOG_LEVEL == "DEBUG":
                logger.debug(f"Response data [{request_id}]: {json.dumps(response_data, indent=2)}")
            
            metrics.increment("broker.schwab.request.success", tags={"method": method, "endpoint": endpoint})
            return response_data
            
        except requests.exceptions.Timeout:
            self.error_count += 1
            error_message = f"API request timed out after {request_timeout}s: {method} {url}"
            logger.error(error_message)
            metrics.increment("broker.schwab.request.timeout", tags={"method": method, "endpoint": endpoint})
            ...
            logger.error(error_message)
            metrics.increment("broker.schwab.request.connection_error", tags={"method": method, "endpoint": endpoint})
            ...
            logger.error(error_message)
            metrics.increment("broker.schwab.request.error", tags={"method": method, "endpoint": endpoint})
            ...
            metrics.increment("broker.schwab.request.json_error", tags={"method": method, "endpoint": endpoint})
            ...
            logger.error(error_message, exc_info=True)
            metrics.increment("broker.schwab.request.unexpected_error", tags={"method": method, "endpoint": endpoint})
    ...
    def execute_trade(self, trade: Trade) -> Dict[str, Any]:
        ...
        logger.info(f"Executing trade: {trade.symbol} {trade.side} {trade.quantity or trade.investment_amount}")
        ...
        try:
            ...
            response = self._api_request(
                method="POST",
                endpoint=schwab_config.ORDER_ENDPOINT,
                data=order_data
            )
            ...
            logger.info(f"Successfully placed order {order_id} for {trade.symbol}, status: {status}")
            metrics.increment("broker.schwab.trade.executed", tags={"symbol": trade.symbol, "side": trade.side.value})
            ...
            logger.error(f"Failed to execute trade for {trade.symbol}: {e.message} (code: {e.error_code})")
            metrics.increment("broker.schwab.trade.failed", tags={"symbol": trade.symbol, "side": trade.side.value})
    ...
    def get_account_info(self, account_id: str) -> Dict[str, Any]:
        ...
        try:
            response = self._api_request(
                method="GET",
                endpoint=f"{schwab_config.ACCOUNT_ENDPOINT}/{account_id}"
            )
            ...
            logger.error(f"Failed to retrieve account information for {account_id}: {e.message}")
    ...
    def get_positions(self, account_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve positions for account {account_id}: {e.message}")
    ...
    def get_trade_status(self, broker_order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve status for order {broker_order_id}: {e.message}")
    ...
    def cancel_trade(self, broker_order_id: str) -> bool:
        ...
        try:
            ...
            if is_canceled:
                ...
                logger.warning(f"Failed to cancel order {broker_order_id}, response: {response}")
            ...
            if "ALREADY_FILLED" in e.error_code or "ALREADY_EXECUTED" in e.error_code:
                logger.info(f"Cannot cancel order {broker_order_id} as it is already executed")
    ...
    def get_order_history(self, account_id: str, start_date: Optional[datetime] = None, 
        ...
        try:
            ...
            logger.error(f"Failed to retrieve order history for account {account_id}: {e.message}")
    ...
    def get_trade_execution(self, broker_order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Retrieved {len(executions)} executions for order {broker_order_id}")
            ...
            logger.error(f"Failed to retrieve execution details for order {broker_order_id}: {e.message}")
    ...
    def get_market_hours(self, market: str = "EQUITY") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Market {market} is {'open' if result['is_open'] else 'closed'}")
    ...
    def get_quote(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Retrieved quote for {symbol}: last price {result['last_price']}")
    ...
    def place_bracket_order(self, trade: Trade, take_profit_price: float, 
                           stop_loss_price: float) -> Dict[str, Any]:
        """Place a bracket order (entry + take profit + stop loss)."""
        logger.info(f"Placing bracket order for {trade.symbol}: "
        ...
        try:
            ...
            logger.info(f"Successfully placed bracket order {order_id} for {trade.symbol}")
            ...
            logger.error(f"Failed to place bracket order for {trade.symbol}: {e.message}")
            metrics.increment("broker.schwab.bracket_order.failed")
            raise
    
    def place_trailing_stop(self, trade: Trade, trail_amount: float, 
                           trail_type: str = "percent") -> Dict[str, Any]:
        """Place a trailing stop order."""
        logger.info(f"Placing trailing stop order for {trade.symbol}: "
        ...
        try:
            ...
            logger.info(f"Successfully placed trailing stop order {order_id} for {trade.symbol}")
            ...
            logger.error(f"Failed to place trailing stop order for {trade.symbol}: {e.message}")
    ...
    def _prepare_order_data(self, trade: Trade) -> Dict[str, Any]:
        ...
        if trade.order_type in [OrderType.LIMIT, OrderType.STOP_LIMIT] and trade.limit_price:
            order_data["limit_price"] = str(trade.limit_price)
        
        # Add stop price for stop orders
        if trade.order_type in [OrderType.STOP, OrderType.STOP_LIMIT] and trade.stop_price:
