...
from app.models.trade import Trade, TradeStatus, OrderType, OrderSide, InvestmentType
...
class PaperBroker(BaseBroker):
    ...
    def __init__(self, db: Session):
        ...
        self.paper_trading = PaperTradingService(self.market_simulation)
    ...
    def execute_trade(self, trade: Trade) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"Executing paper trade: {trade.id} {trade.symbol} {trade.trade_type}")
            
            # Execute the trade with the paper trading service
            result = self.paper_trading.execute_paper_trade(trade)
            ...
            self._update_positions(
                account_id=str(trade.portfolio_id),  # Use portfolio_id as account_id in paper trading
                symbol=trade.symbol,
    ...
    def get_account_info(self, account_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            portfolio = self.db.query(Portfolio).filter(Portfolio.id == int(account_id)).first()
    ...
    def get_positions(self, account_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for position in account_positions:
                current_price = self.market_simulation.get_current_price(position["symbol"])
    ...
    def get_trade_status(self, broker_order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "executed_at": order["executed_at"].isoformat() if isinstance(order["executed_at"], datetime) else order["executed_at"],
    ...
    def _update_positions(self, account_id: str, symbol: str, quantity: Decimal, 
                         price: Decimal, trade_type: str) -> None:
    ...
    def _calculate_next_market_open(self, now: datetime, market_open: datetime, 
                                  is_weekday: bool) -> str:
