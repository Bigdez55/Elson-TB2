...
class MockResponseGenerator:
    ...
    def generate_order_response(self, order_data: Dict[str, Any], 
    ...
    def generate_order_status(self, order_id: str, 
    ...
    def generate_order_history(self, account_id: str, 
        ...
        if start_date:
            orders = [order for order in orders 
                     if datetime.fromisoformat(order["created_at"].split("T")[0]) >= start_date]
    ...
    def generate_market_hours(self, market: str = "EQUITY") -> Dict[str, Any]:
        ...
        is_open = (
            now.weekday() < 5 and  # Monday through Friday
            now.time() >= next_open.time() and
            now.time() < next_close.time()
        )
            ...
            response.update({
                "EQUITY": {
                    ...
                    "extended": {
                        ...
                        "closes_at": next_close.replace(hour=20, minute=0).isoformat()
                    }
                }
            })
...
def generate_mock_response(broker_type: str, response_type: str, 
    ...
    if response_type == "account_info":
        ...
        return generator.generate_market_hours(
            market=kwargs.get("market", "EQUITY")
        )
    else:
...
def save_mock_responses(broker_type: str, output_dir: str) -> None:
    ...
    responses = {
        "account_info.json": generator.generate_account_info(account_id="test-account-123"),
        ...
        "order_market_buy.json": generator.generate_order_response(
            order_data={"symbol": "AAPL", "side": "buy", "type": "market", "quantity": "10"},
            account_id="test-account-123"
        ),
        "order_limit_buy.json": generator.generate_order_response(
            order_data={"symbol": "MSFT", "side": "buy", "type": "limit", 
                       "quantity": "5", "limit_price": "300.00"},
            account_id="test-account-123"
        ),
