...
class AlpacaBroker(BaseBroker):
    ...
    def __init__(self, db=None, api_key: Optional[str] = None, 
                api_secret: Optional[str] = None, use_paper: bool = False):
    ...
    def get_trade_status(self, broker_order_id: str) -> Dict[str, Any]:
        ...
        response = self._api_request(
            method="GET",
            endpoint=f"{ORDERS_ENDPOINT}/{broker_order_id}"
        )
        ...
        return {
            ...
            "filled_price": float(response.get("filled_avg_price", 0)) if response.get("filled_avg_price") else None,
    ...
    def get_order_history(self, account_id: str, start_date: Optional[datetime] = None, 
        ...
        for order in response:
            orders.append({
                ...
                "filled_price": float(order.get("filled_avg_price", 0)) if order.get("filled_avg_price") else None,
    ...
    def get_trade_execution(self, broker_order_id: str) -> Dict[str, Any]:
        ...
        if response.get("status") == "filled":
            ...
            return {
                ...
                "status": self._map_broker_status(response.get("status", "unknown")),
                "filled_quantity": float(response.get("filled_qty", 0)),
                "average_price": float(response.get("filled_avg_price", 0)) if response.get("filled_avg_price") else None,
                "executions": []
            }
    ...
    def get_quote(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_price": float(quote.get("ap", 0)),  # Use ask as last price approximation
    ...
    def place_bracket_order(self, trade: Trade, take_profit_price: float, 
        ...
        order_data["stop_loss"] = {
            ...
            "limit_price": str(stop_loss_price * 0.99) if trade.side == OrderSide.BUY else str(stop_loss_price * 1.01)
        }
        ...
        return {
            ...
            "status": self._map_broker_status(response.get("status", "new")),
            "submitted_at": response.get("created_at"),
            "take_profit_id": response.get("legs", [{}])[0].get("id") if response.get("legs") else None,
            "stop_loss_id": response.get("legs", [{}])[1].get("id") if response.get("legs") and len(response.get("legs")) > 1 else None,
            "broker_response": response
        }
