...
class MarketSimulationService:
    ...
    def get_current_price(self, symbol: str) -> float:
        ...
        if symbol not in self.simulated_prices or \
           (current_time - self.last_update_time.get(symbol, 0)) > self.update_interval_seconds:
    ...
    def get_price_history(self, symbol: str, days: int = 30, interval: str = "1d") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, price in enumerate(prices):
                ...
                if i > 0:
                    ...
                    open_price = float(price) * (1 - random.uniform(-daily_volatility/2, daily_volatility/2))
    ...
    def get_market_depth(self, symbol: str, levels: int = 5) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _update_simulated_price(self, symbol: str) -> None:
        ...
        try:
            ...
            if symbol not in self.simulated_prices:
                if self.market_data:
                    try:
                        seed_price = Decimal(str(self.market_data.get_current_price(symbol)))
    ...
    def _initialize_simulated_price(self, symbol: str, seed_price: Optional[Decimal] = None) -> None:
        ...
        symbol_volatility = self.default_volatility * (1 + Decimal(str(random.uniform(-0.5, 1.0))))
    ...
    def _update_with_random_walk(self, symbol: str) -> None:
        ...
        seconds_since_update = time.time() - self.last_update_time.get(symbol, time.time())
        ...
        if random.random() < gap_probability:
            # Create a price gap
            gap_size = volatility * Decimal(str(random.uniform(-2, 2))) * self.max_gap_pct
            mid_price = mid_price * (Decimal('1') + gap_size)
        else:
            # Normal random walk
            random_change = volatility * Decimal(str(random.uniform(-1, 1))) * time_factor
