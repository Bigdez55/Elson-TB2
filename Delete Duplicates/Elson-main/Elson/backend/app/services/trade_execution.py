...
class TradeExecutionService:
    ...
    def __init__(
        ...
        # Configuration
        self.max_order_chunk_size = Decimal(str(settings.MAX_ORDER_CHUNK_SIZE or "10000"))
        self.min_time_between_chunks = int(settings.MIN_TIME_BETWEEN_CHUNKS or 5)  # seconds
        self.max_partial_fill_attempts = int(settings.MAX_PARTIAL_FILL_ATTEMPTS or 5)
        self.execution_quality_threshold = Decimal(str(settings.EXECUTION_QUALITY_THRESHOLD or "0.001"))  # 0.1%
    ...
    async def execute_order(
        ...
        if trade.status != TradeStatus.PENDING:
            logger.warning(f"Cannot execute trade {trade_id} with status {trade.status}")
            return {"success": False, "error": f"Trade is not in PENDING state: {trade.status}"}
        ...
        try:
            ...
            elif trade.order_type == OrderType.STOP or trade.order_type == OrderType.STOP_LIMIT:
            ...
            record_metric("trade_execution_time", execution_time, 
                         {"symbol": trade.symbol, "order_type": trade.order_type.value})
            
            if "slippage" in metrics:
                record_metric("trade_execution_slippage", float(metrics["slippage"]), 
                             {"symbol": trade.symbol, "order_type": trade.order_type.value})
    ...
    async def create_trailing_stop_order(
        ...
        try:
            market_price = Decimal(str(self.market_data.get_current_price(trade.symbol)))
        except Exception as e:
        ...
        trailing_stop = Trade(
            ...
            trade_type="sell" if trade.trade_type == "buy" else "buy",  # Opposite of original
    ...
    async def chunk_and_execute_large_order(
        ...
        try:
            ...
            if strategy == ExecutionStrategy.TWAP:
                ...
                chunk_sizes = await self._calculate_vwap_chunks(trade.symbol, original_quantity)
            ...
            logger.info(f"Executing {trade.id} in {len(chunk_sizes)} chunks using {strategy.value} strategy")
            
            # Execute each chunk
            for i, chunk_size in enumerate(chunk_sizes):
                logger.info(f"Executing chunk {i+1}/{len(chunk_sizes)} of size {chunk_size} for trade {trade.id}")
                ...
                if broker_service:
                    ...
                    # Simulate execution
                    current_price = Decimal(str(self.market_data.get_current_price(trade.symbol)))
                    chunk_result = {
                        "success": True,
                ...
                if chunk_result["success"]:
                    ...
                    chunk_trade.filled_quantity = Decimal(str(chunk_result["filled_quantity"]))
                    chunk_trade.average_price = Decimal(str(chunk_result["average_price"]))
                    ...
                    total_value += chunk_trade.filled_quantity * chunk_trade.average_price
                    ...
                    chunk_trade.broker_status = f"Failed: {chunk_result.get('error', 'Unknown error')}"
                    logger.error(f"Chunk {i+1} execution failed: {chunk_result.get('error')}")
            ...
            average_price = total_value / executed_quantity if executed_quantity > 0 else Decimal("0")
            ...
            logger.error(f"Error chunking and executing trade {trade.id}: {e}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def calculate_execution_metrics(
        ...
        if execution_result.get("success") and trade.average_price:
            ...
            if "market_price_at_execution" in execution_result:
                market_price = Decimal(str(execution_result["market_price_at_execution"]))
                actual_price = Decimal(str(trade.average_price))
            ...
            if "original_quantity" in execution_result and "executed_quantity" in execution_result:
            ...
            if "chunks" in execution_result and execution_result["chunks"]:
                ...
                if len(chunk_prices) > 1:
                    price_dispersion = statistics.stdev([float(p) for p in chunk_prices])
    ...
    def _calculate_average_price(
        ...
        total_value = (previous_quantity * previous_avg_price) + (new_quantity * new_price)
        
        return total_value / total_quantity if total_quantity > 0 else Decimal("0")
    
    async def _execute_regular_order(self, trade: Trade, broker_service=None) -> Dict[str, Any]:
        ...
        try:
            ...
            if broker_service:
                ...
                execution_price = trade.price if trade.order_type == OrderType.LIMIT else market_price
    ...
    async def _execute_stop_order(self, trade: Trade, broker_service=None) -> Dict[str, Any]:
    ...
    def _update_trade_with_execution_results(self, trade: Trade, result: Dict[str, Any]) -> None:
        ...
        trade.status = TradeStatus.FILLED
        trade.filled_quantity = Decimal(str(result.get("filled_quantity", trade.quantity)))
    ...
    def _calculate_even_chunks(self, quantity: Decimal) -> List[Decimal]:
        ...
        num_chunks = (quantity / self.max_order_chunk_size).to_integral_exact(rounding='ROUND_UP')
    ...
    async def _calculate_vwap_chunks(self, symbol: str, quantity: Decimal) -> List[Decimal]:
        ...
        try:
            ...
            if opening_chunk > self.max_order_chunk_size:
                num_opening_chunks = (opening_chunk / self.max_order_chunk_size).to_integral_exact(rounding='ROUND_UP')
                opening_chunks = [opening_chunk / num_opening_chunks for _ in range(int(num_opening_chunks))]
                chunks.extend(opening_chunks)
            else:
                chunks.append(opening_chunk)
                
            if midday_chunk > self.max_order_chunk_size:
                num_midday_chunks = (midday_chunk / self.max_order_chunk_size).to_integral_exact(rounding='ROUND_UP')
                midday_chunks = [midday_chunk / num_midday_chunks for _ in range(int(num_midday_chunks))]
                chunks.extend(midday_chunks)
            else:
                chunks.append(midday_chunk)
                
            if closing_chunk > self.max_order_chunk_size:
                num_closing_chunks = (closing_chunk / self.max_order_chunk_size).to_integral_exact(rounding='ROUND_UP')
                closing_chunks = [closing_chunk / num_closing_chunks for _ in range(int(num_closing_chunks))]
            ...
            logger.error(f"Error calculating VWAP chunks: {e}, falling back to even chunks")
            return self._calculate_even_chunks(quantity)
