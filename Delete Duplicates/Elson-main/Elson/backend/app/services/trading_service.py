...
from app.models.trade import (
    Trade, TradeStatus, OrderType, OrderSide, 
    TradeSource, InvestmentType
)
from app.models.portfolio import Portfolio, Position
from app.models.user import User, UserRole
from app.models.account import Account, AccountType
from app.services.broker.factory import get_broker, get_resilient_broker, BrokerType
...
class TradingService:
    ...
    def __init__(
        ...
        if settings.PRODUCTION_ENV:
            metrics.gauge("trading_service.using_resilient_broker", 1 if use_resilient_broker else 0)
        
        # Configure broker - use resilient broker in production for failover
        if use_resilient_broker and settings.PRODUCTION_ENV:
            ...
            logger.info(f"Trading service using single broker of type: {broker_type or settings.DEFAULT_BROKER}")
    ...
    async def create_trade(
        ...
        try:
            ...
            if portfolio.user_id != user_id:
                raise HTTPException(status_code=403, detail="Portfolio does not belong to user")
            ...
            if investment_type == InvestmentType.DOLLAR_BASED:
                if not investment_amount:
                    raise ValueError("Investment amount is required for dollar-based investments")
                
                # Set investment amount and calculate approx quantity for reference
                trade.investment_amount = investment_amount
                approx_quantity = Decimal(str(investment_amount)) / Decimal(str(execution_price))
                trade.quantity = approx_quantity
            else:
                if not quantity:
                    raise ValueError("Quantity is required for quantity-based investments")
            ...
            result = {
                ...
                "investment_amount": float(trade.investment_amount) if trade.investment_amount else None,
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "symbol": trade.symbol,
                    "quantity": float(trade.quantity) if trade.quantity else None,
                    "filled_quantity": float(trade.filled_quantity) if trade.filled_quantity else None,
                    "price": float(trade.price) if trade.price else None,
    ...
    async def approve_trade(
        ...
        try:
            ...
            if not minor or minor.role != UserRole.MINOR:
                raise HTTPException(status_code=400, detail="Trade is not from a minor user")
            ...
            if trade.broker_order_id:
                try:
                    ...
                    if not success:
                        raise HTTPException(
                            status_code=400,
                            detail="Failed to cancel trade with broker"
                        )
                except BrokerError as e:
    ...
    async def get_order_book(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            buy_orders = self.db.query(Trade).filter(
                Trade.symbol == symbol,
            ...
            sell_orders = self.db.query(Trade).filter(
                Trade.symbol == symbol,
                Trade.status == TradeStatus.PENDING,
    ...
    async def _get_market_price(self, symbol: str) -> float:
        ...
        try:
            ...
            raise ValueError(f"Failed to get market price: {str(e)}")
    ...
    def _validate_trade(self, trade: Trade, user: User, portfolio: Portfolio) -> None:
        ...
        if trade.order_type in [OrderType.STOP, OrderType.STOP_LIMIT] and not trade.stop_price:
    ...
    def _validate_educational_requirements(self, user_id: int, trade: Trade) -> None:
        ...
        if not self.education_service.check_user_has_permission(user_id, basic_permission):
            raise ValueError("You need to complete the basic trading education before you can trade stocks")
        
        # Check additional permissions
        for permission_type in permission_types:
            if not self.education_service.check_user_has_permission(user_id, permission_type):
                permission_name = permission_type.replace("_", " ").title()
                raise ValueError(f"You need to complete the education for {permission_name} before you can use this order type")
    ...
    async def _update_portfolio_after_trade(self, trade: Trade, portfolio: Portfolio) -> None:
        """
        Update portfolio cash balance and positions after a trade is executed.
        """
        # Get trade details
        quantity = Decimal(str(trade.filled_quantity)) if trade.filled_quantity else Decimal('0')
        price = Decimal(str(trade.average_price)) if trade.average_price else Decimal('0')
        commission = Decimal(str(trade.commission)) if trade.commission else Decimal('0')
