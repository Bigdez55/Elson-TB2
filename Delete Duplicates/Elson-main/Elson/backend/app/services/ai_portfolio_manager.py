...
class AIPortfolioManager(AdvisorService):
    ...
    def optimize_portfolio(
        ...
        cache_key = f"{self.CACHE_KEY_PREFIX}optimize:{user_id}:{method}:{risk_tolerance}"
        cached_result = self._get_cached_result(cache_key)
        if cached_result:
            logger.info(f"Using cached optimization result for user {user_id}")
            record_metric("ai_portfolio_optimization", 
        ...
        additional_symbols = ["SPY", "QQQ", "VTI", "AGG", "BND", "VEA", "VWO", "VXUS", "GLD", "VNQ"]
        ...
        if not historical_data or len(historical_data) < 30:  # Need sufficient data
            logger.error(f"Insufficient historical data for optimization: {len(historical_data)} days")
            return None
        ...
        try:
            if method == "efficient_frontier":
                ...
                result = self._optimize_black_litterman(
                    returns, risk_tolerance, min_allocation, max_allocation, symbols, portfolio
                )
    ...
    def get_market_timing_signal(
        ...
        try:
            ...
            # Get historical data
            historical_data = self._get_historical_data([symbol], 120)  # 120 days of data
            if not historical_data or len(historical_data) < 60:  # Need sufficient data
                logger.error(f"Insufficient historical data for timing signal: {len(historical_data)} days")
                return None
            ...
            prediction = self.neural_network.predict(symbol, features, prediction_horizon)
            if not prediction:
                logger.error(f"Failed to get prediction for {symbol}")
                return None
            
            # Get market regime and conditions
            market_regime = self.market_processor.detect_market_regime(historical_data, symbol)
            market_volatility = self.market_processor.calculate_volatility(historical_data, symbol, 30)
            market_momentum = self.market_processor.calculate_momentum(historical_data, symbol, 14)
            ...
            bollinger = self.market_processor.calculate_bollinger_bands(historical_data, symbol)
            ...
            if bollinger and 'lower' in bollinger and 'upper' in bollinger and 'price' in bollinger:
            ...
            signal_type = "buy" if combined_signal > 0.1 else "sell" if combined_signal < -0.1 else "hold"
            ...
            signal = MarketTimingSignal(
                ...
                signals={
                    ...
                    "bollinger": float(bollinger_signal) if bollinger_signal is not None else None,
                    "nn_prediction": float(nn_signal) if nn_signal is not None else None
                },
                market_conditions={
                    "regime": market_regime,
                    "volatility": float(market_volatility) if market_volatility is not None else None,
                    "momentum": float(market_momentum) if market_momentum is not None else None
                }
            )
            
            # Record metrics
            record_metric("market_timing_signal", 
                         (datetime.utcnow() - start_time).total_seconds(),
                         {"symbol": symbol})
    ...
    def rebalance_portfolio_ai(
        ...
        if (portfolio.last_rebalanced_at and 
            (datetime.utcnow() - portfolio.last_rebalanced_at).days < 7):
        ...
        if consider_timing and trades:
            ...
            for trade in trades:
                ...
                if timing_signal:
                    ...
                    if (trade.trade_type == "buy" and timing_signal.signal_type == "sell" and 
                        timing_signal.strength > 0.7):
                        continue
                    elif (trade.trade_type == "sell" and timing_signal.signal_type == "buy" and 
                          timing_signal.strength > 0.7):
                        continue
                ...
                trade_impact = trade.total_amount / portfolio.total_value if portfolio.total_value > 0 else 0
        ...
        # Record metrics
        record_metric("portfolio_rebalance_ai", 
                     (datetime.utcnow() - start_time).total_seconds(),
    ...
    def _get_historical_data(self, symbols: List[str], days: int = 365) -> Dict[str, List[Dict]]:
    ...
    def _calculate_returns(self, historical_data: Dict[str, List[Dict]]) -> pd.DataFrame:
    ...
    def _optimize_efficient_frontier(
        ...
        def objective_function(weights, mean_returns, cov_matrix, risk_tolerance, risk_free_rate):
            ...
            risk_adjusted_return = (1 - risk_tolerance) * (-port_variance) + risk_tolerance * port_return
    ...
    def _optimize_black_litterman(
        ...
        total_value = sum(p.quantity * p.current_price for p in positions) if positions else 0
        
        for symbol in symbols:
            if symbol in current_positions and total_value > 0:
                position = current_positions[symbol]
                market_caps[symbol] = (position.quantity * position.current_price) / total_value
            else:
        ...
        market_weights_array = np.array([market_weights.get(s, 0) for s in returns.columns])
            ...
            for symbol in symbols:
                ...
                try:
                    ...
                    if historical_data and len(historical_data) > 60:
                        features = self.market_processor.extract_features(historical_data, symbol)
                        prediction = self.neural_network.predict(symbol, features, 30)  # 30-day prediction
        ...
        for i, (symbol, view) in enumerate(views.items()):
            ...
            uncertainty = (1.0 - confidence.get(symbol, 0.5)) * 0.2  # Scale to reasonable value
        ...
        if len(views) > 0:
            ...
            try:
                ...
                er_bl = equilibrium_returns_array + np.dot(middle, (Q - np.dot(P, equilibrium_returns_array)))
        ...
        initial_weights = np.array([market_weights.get(s, 1.0/num_assets) for s in returns.columns])
    ...
    def _calculate_portfolio_metrics(
        ...
        weights_array = weights_array / np.sum(weights_array) if np.sum(weights_array) > 0 else weights_array
        ...
        expected_risk = np.sqrt(np.dot(weights_array.T, np.dot(cov_matrix, weights_array))) * np.sqrt(252)
    ...
    def _calculate_rebalance_trades(
        ...
        target_values = {symbol: total_portfolio_value * weight for symbol, weight in target_allocation.items()}
