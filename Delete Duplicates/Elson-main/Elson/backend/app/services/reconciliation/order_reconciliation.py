...
from app.services.broker.factory import get_resilient_broker, BrokerType
...
class OrderReconciliationService:
    ...
    async def reconcile_recent_orders(self, hours: int = 24) -> Dict[str, Any]:
        ...
        trades_to_reconcile = self.db.query(Trade).filter(
            and_(
        ...
        reconciliation_tasks = [
            self.reconcile_single_order(trade)
    ...
    async def reconcile_single_order(self, trade: Trade) -> Dict[str, Any]:
        ...
        try:
            ...
            if not trade.broker_order_id:
                logger.debug(f"Skipping reconciliation for trade {trade.id} with no broker_order_id")
            ...
            if self._needs_status_update(trade, broker_status):
                ...
                logger.info(f"Status mismatch for trade {trade.id}: app={trade.status.value}, broker={broker_status['status']}")
    ...
    async def _get_broker_status(self, trade: Trade) -> Dict[str, Any]:
        ...
        try:
            ...
            if status_info.get("status") in ["filled", "FILLED", "executed", "EXECUTED"]:
    ...
    def _needs_status_update(self, trade: Trade, broker_status: Dict[str, Any]) -> bool:
        ...
        broker_app_status = self._map_broker_status_to_app_status(broker_status.get("status"))
        ...
        if broker_app_status == TradeStatus.FILLED:
            ...
            if broker_filled_qty and abs(float(broker_filled_qty) - float(trade.filled_quantity or 0)) > 0.0001:
                return True
                
            if broker_avg_price and abs(float(broker_avg_price) - float(trade.average_price or 0)) > 0.0001:
                return True
    ...
    def _map_broker_status_to_app_status(self, broker_status: str) -> TradeStatus:
        ...
        if "filled" in status_lower or "executed" in status_lower or "completed" in status_lower:
        ...
        elif "pending" in status_lower or "open" in status_lower or "new" in status_lower:
            return TradeStatus.PENDING
        elif "expire" in status_lower:
            return TradeStatus.EXPIRED
        else:
            logger.warning(f"Unknown broker status: {broker_status}, defaulting to PENDING")
            return TradeStatus.PENDING
    
    async def _update_trade_from_broker(self, trade: Trade, broker_status: Dict[str, Any]) -> None:
        """
        ...
        if new_status in [TradeStatus.FILLED, TradeStatus.PARTIALLY_FILLED]:
            ...
            if previous_status != TradeStatus.FILLED and new_status == TradeStatus.FILLED:
        ...
        if previous_status != new_status and new_status in [
            TradeStatus.FILLED, TradeStatus.CANCELLED, TradeStatus.REJECTED, TradeStatus.EXPIRED
        ]:
    ...
    async def _update_portfolio_for_filled_trade(self, trade: Trade) -> None:
        ...
        if not portfolio:
            logger.error(f"Portfolio {trade.portfolio_id} for trade {trade.id} not found")
            return
        ...
        if trade.trade_type == "buy":
            ...
            if not position:
                ...
                if position.quantity > 0:
                    position.average_price = (total_value + new_value) / float(position.quantity)
            ...
            if position:
                ...
                logger.warning(f"Position for {trade.symbol} not found when processing sell trade {trade.id}")
    ...
    async def _send_status_notification(self, trade: Trade, previous_status: TradeStatus) -> None:
        """
    ...
    async def reconcile_all_active_positions(self) -> Dict[str, Any]:
        ...
        for portfolio in portfolios:
            try:
                ...
                db_positions = self.db.query(Position).filter(
                    Position.portfolio_id == portfolio.id
                ).all()
        ...
        for symbol in all_symbols:
            try:
                ...
                # Case 2: Position in DB but not in broker
                    ...
                    logger.warning(f"Position {symbol} found in DB but not in broker for portfolio {portfolio.id}")
                ...
                # Case 3: Position in broker but not in DB
                    ...
                    updated += 1
                    logger.info(f"Created missing position {symbol} for portfolio {portfolio.id}")
    ...
    def _position_needs_update(self, db_position: Position, broker_position: Dict[str, Any]) -> bool:
        """
        ...
        quantity_diff = abs(float(db_position.quantity) - float(broker_position["quantity"]))
        ...
        if "current_price" in broker_position and broker_position["current_price"]:
            price_diff_pct = abs(float(db_position.current_price) - float(broker_position["current_price"])) / float(broker_position["current_price"])
            if price_diff_pct > 0.01:  # 1% difference
                return True
                
        return False
    
    def _update_position_from_broker(self, db_position: Position, broker_position: Dict[str, Any]) -> None:
        """
