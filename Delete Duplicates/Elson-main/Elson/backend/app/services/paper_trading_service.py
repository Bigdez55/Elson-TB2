...
from app.services.broker.factory import get_broker, BrokerType
...
class PaperTradingService(TradingService):
    ...
    async def get_paper_portfolio_value(
        self,
        portfolio_id: int
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for position in positions:
                ...
                if position.quantity > 0:
                    position_data.append({
                        ...
                        "unrealized_gain_percent": float(unrealized_gain / cost_basis * 100) if cost_basis > 0 else 0,
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to get portfolio value: {str(e)}"
            )
    ...
    async def get_paper_trade_history(
        ...
        try:
            ...
            for trade in trades:
                ...
                realized_pl = None
                if trade.status == TradeStatus.FILLED and trade.filled_quantity and trade.average_price:
                    ...
                    if trade.trade_type == "sell":
                        ...
                        if position:
                            cost_basis = float(trade.filled_quantity) * float(position.average_price)
                            sale_proceeds = float(trade.filled_quantity) * float(trade.average_price)
                ...
                trade_history.append({
                    ...
                    "investment_type": trade.investment_type.value if trade.investment_type else None,
