...
class RateLimiter:
    ...
    def __init__(self, calls_per_second: float, calls_per_minute: float, calls_per_day: float):
    ...
    async def wait_if_needed(self, endpoint: str = "default") -> float:
        ...
        if wait_time > 0:
            ...
            metrics.gauge("api.rate_limit.wait_time", wait_time, tags={"endpoint": endpoint})
...
class BaseApiClient(ABC):
    ...
    async def _request(
        ...
        try:
            ...
            metrics.increment(
                "api.request.error",
                tags={"provider": self.provider_name, "endpoint": endpoint, "type": "client_error"}
            )
            ...
            metrics.increment(
                "api.request.error",
                tags={"provider": self.provider_name, "endpoint": endpoint, "type": "timeout"}
            )
            ...
            duration = time.time() - start_time
            logger.error(f"API unexpected error {request_id} ({duration:.2f}s): {str(e)}", exc_info=True)
            
            metrics.increment(
                "api.request.error",
                tags={"provider": self.provider_name, "endpoint": endpoint, "type": "unexpected"}
            )
