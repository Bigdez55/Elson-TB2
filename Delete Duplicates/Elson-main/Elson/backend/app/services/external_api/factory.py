...
class ApiClientHealth:
    ...
    def __init__(self):
        ...
        for provider in ApiProvider:
            self._health_status[provider] = True  # Assume healthy until proven otherwise
    ...
    def record_failure(self, provider: ApiProvider) -> None:
        ...
        metrics.gauge(f"api.consecutive_failures.{provider}", self._consecutive_failures[provider])
    ...
    def is_healthy(self, provider: ApiProvider) -> bool:
        ...
        if not self._health_status[provider] and time_since_check > settings.API_RETRY_INTERVAL:
            logger.info(f"API provider {provider} retry period elapsed, marking for retry")
            self._health_status[provider] = True
...
class ApiClientFactory:
    ...
    def register(self, provider: ApiProvider, client_class: Type[BaseApiClient]) -> None:
...
class ResilientApiClient:
        ...
        **kwargs
        ...
        for provider in preferred_providers:
            ...
            try:
                ...
                if not method:
                    logger.warning(f"Method {method_name} not supported by provider {provider}")
                    continue
                ...
                logger.debug(f"Successfully executed {method_name} using provider {provider}")
                ...
                logger.warning(f"Provider {provider} operation {method_name} failed: {e}")
    ...
    async def get_company_profile(self, symbol: str) -> Dict[str, Any]:
        ...
        result, provider = await self._execute_with_failover("get_company_profile", symbol)
        result["provider"] = provider  # Add provider information
        return result
