...
from app.services.external_api.factory import get_resilient_client, api_factory, ApiProvider
...
async def get_stock_quote(symbol: str, force_refresh: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        metrics.increment(
            "market_data.quote.external_api.error",
            tags={"symbol": symbol}
        )
...
async def get_historical_data(
    ...
    try:
        historical_data = await client.get_historical_data(symbol, start_date, end_date, interval)
        
        # Record success metrics
        metrics.timing(
            "market_data.historical.external_api", 
            (datetime.now() - start_time).total_seconds() * 1000,
            tags={"symbol": symbol, "provider": historical_data.get("provider", "unknown")}
        )
        ...
        # Record failure metrics
        metrics.increment(
            "market_data.historical.external_api.error",
            tags={"symbol": symbol}
        )
        
        logger.error(f"Failed to get historical data for {symbol} from all providers: {str(e)}")
        raise
...
async def get_company_profile(symbol: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Record failure metrics
        metrics.increment(
            "company_data.profile.external_api.error",
            tags={"symbol": symbol}
        )
        
        logger.error(f"Failed to get company profile for {symbol} from all providers: {str(e)}")
        raise
