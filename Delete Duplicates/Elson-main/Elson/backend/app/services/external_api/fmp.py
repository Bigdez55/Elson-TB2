...
class FMPClient(BaseApiClient):
    ...
    async def get_historical_data(
        ...
        if start_date or end_date:
            ...
            for data_point in historical_data:
                ...
                try:
                    ...
                    logger.warning(f"Invalid date format in historical data: {date_str}")
        ...
        for data_point in historical_data:
            normalized_point = {
                ...
                "adjusted_close": float(data_point.get("adjClose", 0)) if "adjClose" in data_point else None
            }
    ...
    async def get_company_profile(self, symbol: str) -> Dict[str, Any]:
        ...
        profile = {
            ...
            "pe_ratio": float(profile_data.get("pe", 0)) if profile_data.get("pe") else None,
            "dividend_yield": float(profile_data.get("lastDiv", 0)) if profile_data.get("lastDiv") else None,
            "beta": float(profile_data.get("beta", 0)) if profile_data.get("beta") else None,
            "52_week_high": float(profile_data.get("range", "0-0").split("-")[1]) if "-" in profile_data.get("range", "") else None,
            "52_week_low": float(profile_data.get("range", "0-0").split("-")[0]) if "-" in profile_data.get("range", "") else None,
            "country": profile_data.get("country"),
            "employees": int(profile_data.get("fullTimeEmployees", 0)) if profile_data.get("fullTimeEmployees") else None,
            "ceo": profile_data.get("ceo"),
            "website": profile_data.get("website"),
            "image": profile_data.get("image"),
            "price": float(profile_data.get("price", 0)),
            "change_percent": float(profile_data.get("changes", 0)),
            "change": float(profile_data.get("changes", 0) * profile_data.get("price", 0) / 100) if profile_data.get("price") else 0,
