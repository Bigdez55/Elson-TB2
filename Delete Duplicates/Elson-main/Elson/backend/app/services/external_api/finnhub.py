...
class FinnhubClient(BaseApiClient):
    ...
    async def get_stock_quote(self, symbol: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "change_percent": ((response.get("c", 0) - response.get("pc", 0)) / response.get("pc", 1)) * 100 if response.get("pc", 0) else 0,
            "timestamp": datetime.fromtimestamp(response.get("t", 0)).isoformat() if response.get("t", 0) else datetime.now().isoformat()
        }
    ...
    async def get_company_profile(self, symbol: str) -> Dict[str, Any]:
        ...
        if not response or "error" in response:
            raise ApiError(response.get("error", f"No company data found for symbol: {symbol}"))
        ...
        profile = {
            ...
            "market_cap": float(response.get("marketCapitalization", 0)) * 1000000 if response.get("marketCapitalization") else 0,
    ...
    async def get_technical_indicator(
        ...
        # Make the API request
        response = await self.get("/indicator", params={
            "symbol": symbol,
            "resolution": resolution,
            "from": start_ts,
            "to": end_ts,
