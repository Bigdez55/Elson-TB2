...
class RiskManager:
    ...
    ALLOWED_SYMBOLS_MINORS = {
        "AAPL", "MSFT", "GOOGL", "AMZN", "SPY", "VTI", "VOO", 
        "QQQ", "VXUS", "BND", "AGG", "VB", "IJR"
    }
    ...
    def __init__(self, db: Session):
        self.db = db
        self.config_dir = os.path.join(os.path.dirname(__file__), '../../../trading_engine/config')
        ...
        for profile in RiskProfile:
            ...
            if os.path.exists(config_file):
                try:
                    ...
                    logger.error(f"Error loading risk profile {profile.value}: {str(e)}")
    ...
    def check_allowed_symbols(self, user: User, symbol: str) -> RiskCheckResult:
        ...
        restricted_assets = self.get_profile_param(user, "trade_limitations.restricted_assets") or []
        ...
        is_leveraged_etf = any(x in symbol for x in ["2X", "3X", "BULL", "BEAR", "INVERSE"])
        is_crypto = symbol.startswith("CRYPTO_") or symbol in ["BTC", "ETH", "LTC"]
        
        # Check if any of these asset classes are restricted
        if (is_option and "options" in restricted_assets) or \
           (is_future and "futures" in restricted_assets) or \
           (is_leveraged_etf and "leveraged_etfs" in restricted_assets) or \
           (is_crypto and "crypto" in restricted_assets):
            profile = self.get_user_risk_profile(user)
            return RiskCheckResult(
                False,
    ...
    def check_position_limits(self, user: User, portfolio: Portfolio, trade: Trade) -> RiskCheckResult:
        """
        ...
        if trade.trade_type.lower() == "buy":
            ...
            if new_position < 0:
                return RiskCheckResult(False, f"Insufficient shares to sell. You have {current_position} shares.")
        ...
        max_position_size = self.get_profile_param(user, "position_sizing.max_position_size")
        ...
        position_pct = new_position_value / new_portfolio_value if new_portfolio_value > 0 else 0
    ...
    def check_risk_profile_limits(self, user: User, portfolio: Portfolio, trade: Trade) -> RiskCheckResult:
        """
        ...
        max_trades_per_day = self.get_profile_param(user, "trade_limitations.max_trades_per_day")
        ...
        min_holding_period_days = self.get_profile_param(user, "trade_limitations.min_holding_period_days")
        if min_holding_period_days is not None and trade.trade_type.lower() == "sell":
            # Find when this position was acquired
            most_recent_buy = self.db.query(Trade).filter(
                Trade.user_id == user.id,
...
def calculate_portfolio_volatility(portfolio: Portfolio) -> float:
    ...
    position_count = len(portfolio.positions_detail) if portfolio.positions_detail else 0
...
def calculate_portfolio_risk_metrics(portfolio: Portfolio, risk_profile: Optional[RiskProfile] = None) -> Dict[str, float]:
    """
...
def get_risk_profile_report(user: User, portfolio: Portfolio, risk_manager: RiskManager) -> Dict:
    """
    ...
    max_volatility = risk_manager.get_profile_param(user, "volatility_limits.max_portfolio_volatility") or 0.15
    max_drawdown = risk_manager.get_profile_param(user, "drawdown_limits.max_total_drawdown") or 0.12
    ...
    return {
        ...
        "warnings": warnings,
        "recommendations": generate_risk_recommendations(user, portfolio, metrics, profile, risk_manager)
    }
...
def generate_risk_recommendations(
    ...
    max_volatility = risk_manager.get_profile_param(user, "volatility_limits.max_portfolio_volatility") or 0.15
    max_position_size = risk_manager.get_profile_param(user, "position_sizing.max_position_size") or 0.10
    min_cash_allocation = risk_manager.get_profile_param(user, "exposure_limits.min_cash_allocation") or 0.05
    ...
    for position in portfolio.positions_detail:
        ...
        position_pct = position_size / portfolio.total_value if portfolio.total_value else 0
    ...
    cash_position = next((p for p in portfolio.positions_detail if p.symbol == "CASH"), None)
    cash_pct = 0
    if cash_position:
        cash_pct = cash_position.quantity / portfolio.total_value if portfolio.total_value else 0
