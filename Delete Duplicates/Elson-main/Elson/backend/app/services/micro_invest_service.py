...
from app.models.trade import (
    Trade, TradeStatus, TradeType, InvestmentType, 
    TradeSource, OrderType, RoundupTransaction
)
from app.models.user import User, UserRole
from app.models.user_settings import (
    UserSettings, RoundupFrequency, MicroInvestTarget
)
from app.models.portfolio import Portfolio
from app.schemas.micro_invest import (
    RoundupTransactionCreate, UserSettingsCreate, MicroInvestmentCreate,
    UserSettingsUpdate, RoundupTransactionUpdate, RoundupSummary,
    MicroInvestStats
)
...
class MicroInvestService:
    ...
    def __init__(
        ...
        self.investment = investment_service
        self.alert_service = alert_service
        
        # Configuration
        self.min_micro_amount = Decimal('0.01')  # Minimum 1 cent for micro-investments
        self.min_roundup_investment = Decimal('1.00')  # Minimum $1 to trigger roundup investment
        self.default_etfs = {
    ...
    def create_user_settings(
        self, settings_data: UserSettingsCreate
    ) -> UserSettings:
        ...
        settings = UserSettings(
            ...
            micro_invest_target_type=MicroInvestTarget(settings_data.micro_invest_target_type),
    ...
    def calculate_roundup(self, transaction_amount: float, multiplier: float = 1.0) -> float:
    ...
    def get_pending_roundups(self, user_id: int) -> Tuple[List[RoundupTransaction], float]:
        """Get pending roundups and total amount for a user."""
        roundups = self.db.query(RoundupTransaction).filter(
            RoundupTransaction.user_id == user_id,
            RoundupTransaction.status == "pending"
        ).order_by(RoundupTransaction.transaction_date).all()
        
        total_amount = sum(r.roundup_amount for r in roundups)
        
        return roundups, total_amount
    
    def _process_threshold_based_roundups(self, user_id: int) -> Optional[Dict[str, Any]]:
    ...
    def invest_pending_roundups(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            # Create trade
            from app.schemas.trade import DollarBasedInvestmentCreate
            ...
            investment_data = DollarBasedInvestmentCreate(
                ...
                note=f"Roundup investment (${total_amount:.2f})"
            )
            
            trade = self.investment.create_dollar_based_investment(
                user_id=user_id,
    ...
    def _get_investment_target(
        ...
        if settings.micro_invest_target_type == MicroInvestTarget.SPECIFIC_SYMBOL:
            ...
            portfolio = self.db.query(Portfolio).filter(
                Portfolio.id == settings.micro_invest_portfolio_id
            ).first()
    ...
    def process_scheduled_roundups(self) -> Dict[str, Any]:
        ...
        users_with_roundups = self.db.query(UserSettings).filter(
            UserSettings.roundup_enabled == True,
            UserSettings.micro_investing_enabled == True
        ).all()
        ...
        for settings in users_with_roundups:
            try:
                ...
                logger.error(f"Error processing roundups for user {settings.user_id}: {e}")
                result["errors"] += 1
    ...
    def get_roundup_summary(self, user_id: int) -> RoundupSummary:
        ...
        # Total roundups
        total_count = self.db.query(func.count(RoundupTransaction.id)).filter(
            RoundupTransaction.user_id == user_id
        ).scalar() or 0
        
        # Total amount
        total_amount = self.db.query(func.sum(RoundupTransaction.roundup_amount)).filter(
            RoundupTransaction.user_id == user_id
        ).scalar() or 0
        
        # Pending amount
        pending_amount = self.db.query(func.sum(RoundupTransaction.roundup_amount)).filter(
            RoundupTransaction.user_id == user_id,
            RoundupTransaction.status == "pending"
        ).scalar() or 0
        
        # Invested amount
        invested_amount = self.db.query(func.sum(RoundupTransaction.roundup_amount)).filter(
            RoundupTransaction.user_id == user_id,
            RoundupTransaction.status == "invested"
        ).scalar() or 0
        
        # Total investments
        total_investments = self.db.query(func.count(RoundupTransaction.trade_id.distinct())).filter(
            RoundupTransaction.user_id == user_id,
            RoundupTransaction.status == "invested"
        ).scalar() or 0
        
        # Last investment date
        last_investment = self.db.query(RoundupTransaction).filter(
            RoundupTransaction.user_id == user_id,
            RoundupTransaction.status == "invested"
        ).order_by(desc(RoundupTransaction.invested_at)).first()
    ...
    def get_micro_invest_statistics(self, user_id: int) -> MicroInvestStats:
        ...
        total_invested = self.db.query(func.sum(Trade.investment_amount)).filter(
            Trade.user_id == user_id,
            Trade.investment_type.in_([InvestmentType.MICRO, InvestmentType.ROUNDUP]),
            Trade.status != TradeStatus.CANCELED
        ).scalar() or 0
        
        # Total transactions
        total_transactions = self.db.query(func.count(Trade.id)).filter(
            Trade.user_id == user_id,
            Trade.investment_type.in_([InvestmentType.MICRO, InvestmentType.ROUNDUP]),
            Trade.status != TradeStatus.CANCELED
        ).scalar() or 0
        ...
        # Average transaction
        avg_transaction = total_invested / total_transactions if total_transactions > 0 else 0
        
        # Roundup specific stats
        total_roundups = self.db.query(func.count(RoundupTransaction.id)).filter(
            RoundupTransaction.user_id == user_id
        ).scalar() or 0
        
        total_roundup_amount = self.db.query(func.sum(RoundupTransaction.roundup_amount)).filter(
            RoundupTransaction.user_id == user_id,
            RoundupTransaction.status == "invested"
        ).scalar() or 0
        ...
        this_month_invested = self.db.query(func.sum(Trade.investment_amount)).filter(
            Trade.user_id == user_id,
            Trade.investment_type.in_([InvestmentType.MICRO, InvestmentType.ROUNDUP]),
            Trade.status != TradeStatus.CANCELED,
        ...
        this_month_count = self.db.query(func.count(Trade.id)).filter(
            Trade.user_id == user_id,
            Trade.investment_type.in_([InvestmentType.MICRO, InvestmentType.ROUNDUP]),
            Trade.status != TradeStatus.CANCELED,
            Trade.created_at >= this_month_start
        ...
        last_month_invested = self.db.query(func.sum(Trade.investment_amount)).filter(
            Trade.user_id == user_id,
            Trade.investment_type.in_([InvestmentType.MICRO, InvestmentType.ROUNDUP]),
            Trade.status != TradeStatus.CANCELED,
        ...
        last_month_count = self.db.query(func.count(Trade.id)).filter(
            Trade.user_id == user_id,
            Trade.investment_type.in_([InvestmentType.MICRO, InvestmentType.ROUNDUP]),
            Trade.status != TradeStatus.CANCELED,
            Trade.created_at >= last_month_start,
