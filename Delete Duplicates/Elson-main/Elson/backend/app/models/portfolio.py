from sqlalchemy import Column, Integer, Float, ForeignKey, JSON, DateTime, String, Boolean, UniqueConstraint, Numeric
...
class Portfolio(Base):
    ...
    positions = Column(JSON, default=dict)  # Stores current positions as JSON
    ...
    def get_daily_drawdown(self) -> Optional[Decimal]:
        ...
        try:
            ...
            today_trades = self._session.query(Trade).filter(
    ...
    def get_daily_trade_count(self) -> int:
        ...
        try:
            ...
            trade_count = self._session.query(Trade).filter(
                Trade.portfolio_id == self.id,
...
class Position(Base):
    ...
    __table_args__ = (
        ...
        sa.UniqueConstraint('portfolio_id', 'symbol', name='uix_position_portfolio_symbol'),
    )
    ...
    def add_quantity(self, quantity: Decimal, price: Decimal) -> None:
        ...
        # Update the position
        self.average_price = new_total_cost / new_total_quantity if new_total_quantity > 0 else Decimal('0')
