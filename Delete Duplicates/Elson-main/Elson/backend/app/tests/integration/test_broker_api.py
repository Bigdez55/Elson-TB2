...
class TestBrokerAPI:
    ...
    def mock_schwab_api(self):
        ...
        with mock.patch('app.services.broker.schwab.SchwabBroker._api_request') as mock_api:
    ...
    def mock_alpaca_api(self):
        ...
        with mock.patch('app.services.broker.alpaca.AlpacaBroker._api_request') as mock_api:
            # Configure the mock to return different responses based on call
    ...
    def _mock_schwab_api_side_effect(self, method, endpoint, **kwargs):
        ...
        if endpoint == "/accounts/test-account-123":
            return generate_mock_response("schwab", "account_info", account_id="test-account-123")
        elif endpoint == "/accounts/test-account-123/positions":
            return generate_mock_response("schwab", "positions", account_id="test-account-123")
    ...
    def _mock_alpaca_api_side_effect(self, method, endpoint, **kwargs):
        """Generate appropriate mock response based on request."""
        if endpoint == "/account":
            return generate_mock_response("alpaca", "account_info", account_id="test-account-123")
        elif endpoint == "/positions":
            return generate_mock_response("alpaca", "positions", account_id="test-account-123")
    ...
    def test_alpaca_broker_advanced_orders(self, mock_alpaca_api):
        ...
        # Test bracket order
        result = broker.place_bracket_order(trade, take_profit_price=200.0, stop_loss_price=180.0)
        assert "broker_order_id" in result
        assert result["status"] == TradeStatus.PENDING
        ...
        result = broker.place_trailing_stop(trade, trail_amount=10.0, trail_type="percent")
        assert "broker_order_id" in result
        assert result["status"] == TradeStatus.PENDING
