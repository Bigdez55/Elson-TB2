...
@pytest.fixture
def fractional_trade(db: Session, test_user: Dict[str, Any], test_portfolio: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def test_create_dollar_based_trade(authorized_client: TestClient, test_portfolio: Dict[str, Any]):
...
def test_execute_fractional_trade(
    ...
    with patch('app.services.market_data.MarketDataService.get_current_price', return_value=150.0):
...
def test_minimum_investment_amount(authorized_client: TestClient, test_portfolio: Dict[str, Any]):
...
def test_fractional_trade_in_portfolio(
    ...
    response = authorized_client.get(f"/api/v1/portfolios/{test_portfolio['id']}/positions")
    assert response.status_code == 200
...
def test_fractional_sell(
    ...
    positions_response = authorized_client.get(f"/api/v1/portfolios/{test_portfolio['id']}/positions")
