...
class SecurityMonitor:
    ...
    def record_login_attempt(self, user_id: str, ip: str, success: bool, details: Optional[Dict] = None) -> None:
        """
        ...
        with self._lock:
            ...
            self.security_events["login_attempts"] = [
                event for event in self.security_events.get("login_attempts", [])
            ...
            if not success:
                ...
                recent_failures = [
                    e for e in self.security_events.get("login_attempts", [])
                    if not e.get("success") and (e.get("user_id") == user_id or e.get("ip") == ip)
                ]
                ...
                if len(recent_failures) >= self.login_failure_threshold:
                    send_warning_alert(
                        ...
                        message=f"Detected {len(recent_failures)} failed login attempts for user {user_id} or IP {ip}",
    ...
    def record_api_key_failure(self, ip: str, endpoint: str, details: Optional[Dict] = None) -> None:
        """
        ...
        with self._lock:
            ...
            self.security_events["api_key_failures"] = [
                event for event in self.security_events.get("api_key_failures", [])
                if event.get("timestamp", 0) > hour_ago
            ]
            ...
            ip_failures = [e for e in self.security_events.get("api_key_failures", []) if e.get("ip") == ip]
            ...
            if len(ip_failures) >= self.api_failure_threshold:
                send_warning_alert(
                    ...
                    metadata={
                        ...
                        "endpoints_accessed": list(set(e.get("endpoint") for e in ip_failures)),
                        "time_window": "1 hour",
                        "latest_attempt": datetime.now().isoformat()
                    }
                )
    ...
    def record_account_lockout(self, user_id: str, ip: str, reason: str, 
    ...
    def record_token_verification_failure(self, ip: str, error_type: str, details: Optional[Dict] = None) -> None:
        """
        ...
        with self._lock:
            ...
            self.security_events["token_failures"] = [
                event for event in self.security_events.get("token_failures", [])
                if event.get("timestamp", 0) > hour_ago
            ]
            ...
            # Check if this IP has multiple failures
            ip_failures = [e for e in self.security_events.get("token_failures", []) if e.get("ip") == ip]
            ...
            if len(ip_failures) >= self.api_failure_threshold:
                send_error_alert(
                    ...
                    metadata={
                        "ip_address": ip,
                        "failure_count": len(ip_failures),
                        "error_types": list(set(e.get("error_type") for e in ip_failures)),
                        "time_window": "1 hour",
                        "latest_attempt": datetime.now().isoformat()
                    }
                )
    ...
    def record_suspicious_activity(self, activity_type: str, ip: str, 
        ...
        with self._lock:
            ...
            self.security_events["suspicious_activity"] = [
                event for event in self.security_events.get("suspicious_activity", [])
    ...
    def is_account_locked(self, user_id: str) -> Tuple[bool, Optional[timedelta]]:
        ...
        with self._lock:
            ...
            expired_users = [
                user for user, expiry in self.account_lockouts.items()
                if now > expiry
            ]
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            events_last_hour = {
                k: len([e for e in v if datetime.fromisoformat(e.get("datetime", "")) > hour_ago])
                for k, v in self.security_events.items()
            }
            
            events_last_day = {
                k: len([e for e in v if datetime.fromisoformat(e.get("datetime", "")) > day_ago])
                for k, v in self.security_events.items()
            }
    ...
    def cleanup(self) -> None:
        ...
        with self._lock:
            ...
            # This would need to be implemented in a production system with a TTL for IPs
...
def create_failed_login_check() -> ServiceCheck:
    ...
    def check_failed_logins() -> bool:
        ...
        try:
            ...
            login_failures = summary.get("events_last_hour", {}).get("login_attempts", 0)
