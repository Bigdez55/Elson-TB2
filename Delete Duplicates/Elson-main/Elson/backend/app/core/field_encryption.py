...
from typing import Any, Optional, Type, TypeVar, Union, Dict, cast
...
class EncryptedString(TypeDecorator):
    ...
    def process_bind_param(self, value: Optional[Dict[str, str]], dialect) -> Optional[str]:
        """
    ...
    def process_result_value(self, value: Optional[str], dialect) -> Optional[Dict[str, str]]:
        """
...
class EncryptedField:
    ...
    def __get__(self, instance: Any, owner: Type) -> Union[hybrid_property, Any]:
        ...
        if instance is None:
            ...
            return hybrid_property(
                fget=lambda self: self._get_decrypted_value(getattr(self, self.column_name)),
                fset=lambda self, value: setattr(self, self.column_name, self._get_encrypted_value(value)),
    ...
    def _get_decrypted_value(self, encrypted_value: Optional[Dict[str, str]]) -> Optional[str]:
        """
...
def encrypt_model_fields(model_class: Type[DeclarativeBase], fields: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    for field_name, value in fields.items():
        if hasattr(model_class, f"_{field_name}"):
            ...
            encrypted_fields[f"_{field_name}"] = encrypted_field._get_encrypted_value(value)
        else:
