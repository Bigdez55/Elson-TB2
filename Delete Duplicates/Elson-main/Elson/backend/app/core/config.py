...
class Settings(BaseSettings):
    ...
    BROKER_PRIORITY_LIST: List[BrokerEnum] = [BrokerEnum.SCHWAB, BrokerEnum.ALPACA, BrokerEnum.PAPER]
    ...
    SESSION_REFRESH_THRESHOLD_MINUTES: int = 30  # Refresh session if less than this time left
    ...
    FIRST_SUPERUSER_PASSWORD: str = "admin123" # For development only, change in production
...
def get_env_file():
    ...
    if os.environ.get("ENV_FILE"):
        ...
        if os.path.exists(env_file):
            ...
            print(f"Warning: Specified env file {env_file} not found, falling back to config lookup")
