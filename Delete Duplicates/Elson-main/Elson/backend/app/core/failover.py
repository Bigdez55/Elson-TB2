...
and takes appropriate actions based on predefined policies.
"""
...
# Create metrics
health_check_status = metrics_registry.gauge(
    'health_check_status', 
    'Current status of service health checks (1=healthy, 0=unhealthy)', 
    ['service']
)

failover_attempts = metrics_registry.counter(
    'failover_attempts_total', 
    'Number of failover attempts', 
    ['service', 'outcome']
)

time_since_last_healthy = metrics_registry.gauge(
...
class HealthCheck:
    ...
    def update_status(self, result: HealthCheckResult) -> None:
        ...
        if self.last_healthy_time:
            seconds_since_healthy = (datetime.now() - self.last_healthy_time).total_seconds()
            time_since_last_healthy.labels(service=self.service_id).set(seconds_since_healthy)
...
class KubernetesHealthCheck(HealthCheck):
    ...
    def check(self) -> HealthCheckResult:
        ...
        try:
            ...
            cmd = [
                "kubectl", "get", self.resource_type, self.resource_name,
...
class RestartServiceAction(FailoverAction):
    ...
    def __init__(
        ...
        service_id: str, 
        command: List[str],
        description: str = "Restart service"
    ):
    ...
    def execute(self) -> Tuple[bool, str]:
        ...
        try:
            logger.info(f"Executing restart command for {self.service_id}: {' '.join(self.command)}")
            process = subprocess.run(
                self.command,
                capture_output=True,
                text=True,
            ...
            return True, f"Service {self.service_id} restarted successfully: {process.stdout}"
...
class PerformDatabaseFailoverAction(FailoverAction):
    ...
    def execute(self) -> Tuple[bool, str]:
        ...
        try:
            logger.info(f"Initiating failover for database {self.service_id}: " +
            ...
            return True, f"Database failover completed successfully: now using {self.standby_host}:{self.standby_port}"
...
class GracefulDegradationAction(FailoverAction):
    ...
    def execute(self) -> Tuple[bool, str]:
        ...
        try:
            logger.info(f"Enabling graceful degradation for {self.service_id}: mode={self.degradation_mode}")
            ...
            return True, f"Enabled degraded mode ({self.degradation_mode}) for {self.service_id}"
...
class FailoverPolicy:
    ...
    def should_failover(self) -> bool:
        ...
        if self.last_failover_time:
            ...
            if elapsed < self.cooldown_period:
                logger.info(f"Failover for {self.service_id} in cooldown ({elapsed:.1f}s / {self.cooldown_period}s)")
                return False
    ...
    def execute_failover(self) -> Tuple[bool, str]:
        ...
        logger.warning(f"Executing failover for {self.service_id} after {self.health_check.consecutive_failures} consecutive failures")
        ...
        for action in self.actions:
            ...
            if not success:
                ...
                logger.info(f"Failover action succeeded for {self.service_id}: {message}")
        ...
        if overall_success:
            ...
            result_message = f"Failover for {self.service_id} failed (some actions failed)"
    ...
    def check_and_failover(self) -> Tuple[bool, str]:
        ...
        return False, f"Service {self.service_id} is unhealthy but failover threshold not reached ({self.health_check.consecutive_failures}/{self.failure_threshold})"
    ...
    def execute_recovery(self) -> Tuple[bool, str]:
        ...
        for action in self.recovery_actions:
            ...
            if not success:
                ...
                logger.info(f"Recovery action succeeded for {self.service_id}: {message}")
        
        if overall_success:
            self.in_failover = False
            result_message = f"Recovery for {self.service_id} completed successfully"
        else:
            result_message = f"Recovery for {self.service_id} failed (some actions failed)"
...
class FailoverManager:
    ...
    def start(self) -> None:
        ...
        logger.info(f"Failover manager started with check interval {self.check_interval}s")
    ...
    def get_service_status(self) -> Dict[str, Dict[str, Any]]:
        ...
        for service_id, policy in self.policies.items():
            ...
            status[service_id] = {
                ...
                "last_check_time": health_check.last_check_time.isoformat() if health_check.last_check_time else None,
                "last_healthy_time": health_check.last_healthy_time.isoformat() if health_check.last_healthy_time else None,
                "in_failover": policy.in_failover,
                "last_failover_time": policy.last_failover_time.isoformat() if policy.last_failover_time else None,
...
def init_failover_manager(alert_manager: AlertManager, check_interval: int = 30) -> FailoverManager:
    ...
    failover_manager = FailoverManager(alert_manager, check_interval)
...
def _register_kubernetes_service_policies(manager: FailoverManager) -> None:
    ...
    if hasattr(settings, "TRADING_ENGINE_NAMESPACE"):
        ...
        actions = [
            ...
            RestartServiceAction(
                service_id="trading_engine",
                command=["kubectl", "rollout", "restart", "deployment/trading-engine", 
                         "-n", settings.TRADING_ENGINE_NAMESPACE]
            ),
    ...
    if hasattr(settings, "MARKET_DATA_NAMESPACE"):
        ...
        actions = [
            ...
            RestartServiceAction(
                service_id="market_data",
                command=["kubectl", "rollout", "restart", "deployment/market-data", 
                         "-n", settings.MARKET_DATA_NAMESPACE]
            ),
...
def _register_local_service_policies(manager: FailoverManager) -> None:
    ...
    if hasattr(settings, "TRADING_ENGINE_HOST"):
        trading_engine_health_check = HttpHealthCheck(
            service_id="trading_engine",
            service_type=ServiceType.TRADING_ENGINE,
    ...
    if hasattr(settings, "MARKET_DATA_HOST"):
        market_data_health_check = HttpHealthCheck(
            service_id="market_data",
            service_type=ServiceType.MARKET_DATA,
