...
class FeatureEngineering:
    ...
    def add_moving_averages(self, df: pd.DataFrame, windows: List[int] = [5, 10, 20, 50, 200]) -> pd.DataFrame:
    ...
    def add_bollinger_bands(self, df: pd.DataFrame, window: int = 20, num_std: float = 2.0) -> pd.DataFrame:
        """Add Bollinger Bands"""
    ...
    def add_candlestick_patterns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['hammer'] = ((body_size <= 0.3 * (df['high'] - df['low'])) & 
                        ...
                        (upper_wick <= 0.2 * body_size)).astype(int)
        
        # Shooting Star (small body at the bottom, long upper wick)
        df['shooting_star'] = ((body_size <= 0.3 * (df['high'] - df['low'])) & 
                              (upper_wick >= 2 * body_size) & 
                              (lower_wick <= 0.2 * body_size)).astype(int)
...
class SentimentFeatures:
    ...
    def generate_features(self, news_df: pd.DataFrame) -> pd.DataFrame:
        ...
        max_score = max(abs(df['sentiment_score'].max()), abs(df['sentiment_score'].min()))
        ...
        agg_df['positive_ratio'] = agg_df['positive_count'] / agg_df['sentiment_score_count']
        agg_df['negative_ratio'] = agg_df['negative_count'] / agg_df['sentiment_score_count']
