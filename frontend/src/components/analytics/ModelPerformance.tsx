...
const VolatilityDashboard: React.FC = () => {
  ...
  const prepareWinRateChartData = () => {
    ...
    // Filter out regimes with no data and cast to RegimePerformance
    return Object.entries(performance)
      .filter(([regime, data]) => regime !== 'summary' && 'sample_count' in data && (data as RegimePerformance).sample_count > 0)
      .map(([regime, data]) => {
        const regimeData = data as RegimePerformance;
        return {
          name: regime,
          winRate: (regimeData.win_rate * 100).toFixed(2),
          sampleCount: regimeData.sample_count,
          avgReturn: regimeData.avg_return.toFixed(2),
                              ...
                              .map(([regime, data]) => {
                                ...
                                return (
                                  <TableRow key={regime}>
                                    ...
                                    <TableCell align="right">{formatPercent(regimeData.win_rate)}</TableCell>
                                    <TableCell align="right">{regimeData.avg_return.toFixed(2)}%</TableCell>
                                    <TableCell align="right">{regimeData.sample_count}</TableCell>
                                  </TableRow>
                                );
                              })}
                          </TableBody>
