...
export const RecurringInvestmentForm: React.FC<RecurringInvestmentFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Extract API-relevant fields, excluding UI-only fields
      const {
        useSpecificDay,
        specificWeekday,
        specificMonthDay,
      ...
      const submissionData = {
        ...
        portfolio_id: String(formState.portfolio_id),
        // Add metadata with scheduling preferences if enabled
        metadata: useSpecificDay ? {
          specific_day: formState.frequency === 'weekly'
            ? String(Number(specificWeekday) - 1) // Convert 1-7 to 0-6 format
            : specificMonthDay === 'last'
              ? '-1'
              : specificMonthDay
        } : undefined
      };
