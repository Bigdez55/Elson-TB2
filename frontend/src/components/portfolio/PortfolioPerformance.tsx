              ...
              <div key={asset.symbol} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className={`text-right ${asset.pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <div className="text-xs">{formatPercentage(asset.pnlPercent)}</div>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="h-4 bg-gray-200 rounded-full overflow-hidden flex">
              {allocation.map((asset, index) => (
                <div
                  ...
                  title={`${asset.symbol}: ${formatPercentage(asset.percentage)}`}
                />
              ))}
            </div>
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {allocation.map((asset, index) => (
                <div key={asset.symbol} className="flex items-center space-x-2 text-sm">
                  ...
                  <span>{asset.symbol} ({formatPercentage(asset.percentage)})</span>
                </div>
              ))}
            </div>
          </div>
