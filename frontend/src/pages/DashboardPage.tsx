...
const DashboardPage: React.FC = () => {
  ...
  // Transform portfolio performance data for charts - no mock data fallbacks
  const hasPerformanceData = performanceData?.daily_returns && performanceData.daily_returns.length > 0;
  const portfolioData = hasPerformanceData ? {
    labels: performanceData.daily_returns.map(d =>
      new Date(d.date).toLocaleDateString('en-US', { weekday: 'short' })
    ),
    portfolio: performanceData.daily_returns.map(d => d.portfolio_value),
    benchmark: performanceData.daily_returns.map(d =>
      (performanceData.daily_returns[0]?.portfolio_value || 0) * (1 + d.return * 0.8)
    ),
  } : null;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="bg-gray-900 rounded-xl p-6 shadow-md lg:col-span-2">
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="px-2 py-1 bg-purple-900 bg-opacity-60 rounded-full text-xs text-purple-200">
              Coming Soon
            </span>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700/50">
              <div className="flex items-center mb-3">
                <div className="h-8 w-8 rounded-full bg-green-900/50 flex items-center justify-center">
                  <svg className="h-4 w-4 text-green-400/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                  </svg>
                </div>
                <div className="ml-3">
                  <h4 className="text-gray-400 font-medium">Tax Loss Harvesting</h4>
                  <p className="text-gray-500 text-xs">AI-powered tax optimization</p>
                </div>
              </div>
              <p className="text-gray-500 text-sm">Automated tax-saving strategies will analyze your portfolio for optimization opportunities.</p>
            </div>
            ...
            <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700/50">
              <div className="flex items-center mb-3">
                ...
                <div className="ml-3">
                  <h4 className="text-gray-400 font-medium">Smart Portfolio Analysis</h4>
                  <p className="text-gray-500 text-xs">AI-driven insights</p>
                </div>
              </div>
              <p className="text-gray-500 text-sm">Get personalized recommendations based on your trading patterns and market conditions.</p>
            </div>
            ...
            <div className="bg-gray-800/50 p-4 rounded-lg border border-gray-700/50">
              <div className="flex items-center mb-3">
                ...
                <div className="ml-3">
                  <h4 className="text-gray-400 font-medium">Risk Assessment</h4>
                  <p className="text-gray-500 text-xs">Portfolio risk monitoring</p>
                </div>
              </div>
              <p className="text-gray-500 text-sm">Real-time risk analysis and alerts to help protect your investments.</p>
            </div>
          </div>
        </div>
      </div>
